%% 生命周期模型求解与模拟 (VFI方法)
% 描述: 
% 本脚本求解并模拟了一个包含内生个人养老金储蓄决策的生命周期模型。
% 模型特点:
% - Epstein-Zin 递归效用
% - 持久性和暂时性收入冲击
% - 工作期和退休期
% - 无风险和风险资产投资
% - 个人养老金账户 (PPS)，缴费可抵扣工资税
% - 工资税 (tau_y) 和养老金提取税 (tau_q)
%
% 结构:
...
file_path = fullfile(save_path, 'vfi_results.mat');
...
vfi_results = solve_model_vfi(cS);
    ...
    fprintf('模型求解结果已保存到 %s\n', fullfile(save_path, 'vfi_results.mat'));
else
load(file_path)
end
...
function cS = setup_parameters()
    ...
    cS.rp = 1.02;     % 个人养老金账户(PPS)回报率

    % --- E. 养老金与税收 ---
    cS.ret_fac = 0.6827;      % 退休后基础养老金 (替代率)
    cS.tau_y = 0.0;          % 工资税率
    cS.tau_q = 0.0;          % 个人养老金提取税率
    % cS.Q_max 将在下方通过新函数计算
    cS.pension_rate = 1 / (cS.td - cS.tr); % 退休后年金化给付比率

    % --- F. 数值求解参数 ---
    cS.n_shocks = 5;      % 离散化的随机冲击节点数
    cS.ncash = 31;        % 现金持有量网格数
    cS.nfund = 31;        % 养老金账户网格数
    cS.maxcash = 100;     % 现金网格上限 (归一化)
    cS.mincash = 0.25;    % 现金网格下限 (归一化)
    cS.maxfund = 20;     % 养老金网格上限 (归一化)
    ...
    % *** 新增: 调用函数计算归一化的 Q_max ***
    absolute_q_max_yuan = 12000; % 政策规定：每年12000元上限
    anchor_income_yuan  = 60000; % 假设：18岁初始年收入为6万元
    % cS.Q_max = calculate_q_max(absolute_q_max_yuan, anchor_income_yuan, cS.f_y);
    cS.Q_max =99999;
    % 收入过程网格
    ...
    for t = (cS.tn-1):-1:(cS.tr-cS.tb+1)
        ...
        parfor i_fund = 1:cS.nfund
            ...
            for i_cash = 1:cS.ncash
                ...
                % 决策变量 x = [消费比例 c_rate, 风险资产比例 alpha]
                x0 = [0.5, 0.5]; 
                lb = [1e-6, 0];
                ub = [1, 1];
                
                obj_fun = @(x) fun_valuefunc_retired(x, gcash_val, gfund_val, V_next, cS, t);
                % 此处无非线性约束
                [policy, fval] = fmincon(obj_fun, x0, [], [], [], [], lb, ub, [], options);

                % 存储绝对消费值和风险资产比例
                local_C(i_cash) = policy(1) * gcash_val; 
    ...
    for t = (cS.tr - cS.tb):-1:1
        ...
        parfor i_fund = 1:cS.nfund
            ...
            for i_cash = 1:cS.ncash
                ...
                % 决策变量: x = [消费比例 c_rate, 风险资产比例 alpha, 养老金缴费占储蓄比例 q_rate]
                x0 = [0.5, 0.5, 0.1]; 
                lb = [0.01, 0, 0];
                ub = [1, 1, min(1,cS.Q_max/gcash_val)];

                obj_fun = @(x) fun_valuefunc_work(x, gcash_val, gfund_val, V_next, cS, t);
                % A = [1,0,1]; % c_rate + q_rate <=1
                % *** 移除非线性约束，因为它总能被满足 ***
                [policy, fval] = fmincon(obj_fun, x0, [], [], [], [], lb, ub, [], options);

                % 将优化得到的比例转换回绝对值进行存储
                
                pension_contrib_abs = policy(3) * gcash_val;
                consumption_abs = policy(1) *(gcash_val-pension_contrib_abs+cS.tau_y*pension_contrib_abs);

                local_C(i_cash) = consumption_abs;
                local_A(i_cash) = policy(2);
                local_Q(i_cash) = pension_contrib_abs;
                local_V(i_cash) = -fval;
            end
            temp_C(:, i_fund) = local_C;
            temp_A(:, i_fund) = local_A;
...
%                VFI - 退休期值函数
%  =====================================================================
function v = fun_valuefunc_retired(x, cash, fund, V_next, cS, t)
    % x = [消费比例 c_rate, 风险资产比例 alpha]
    c_rate = x(1);
    alpha = x(2);

    consumption = c_rate * cash;
    if consumption <= 1e-8, v = 1e10; return; end

    sav = cash - consumption;
    if sav < 0, v = 1e10; return; end % 不应发生，因为 c_rate <= 1

    % Epstein-Zin 瞬时效用项
    u_term = (1 - cS.beta) * (consumption^cS.psi_1);

    % 养老金给付 (税后)
    pension_payout = (1 - cS.tau_q) * fund * cS.pension_rate;

    % 下一期现金持有量 (归一化)
    % 注意：退休后归一化因子不变，gyp=1
    ret_fac_norm = cS.ret_fac; % 假设退休金是相对于不变的 P_K
    ...
    % 插值获取下一期值函数
    int_V = interp2(cS.gfund, cS.gcash, V_next, fund_1, cash_1, 'spline');
    int_V(int_V <= 0) = 1e-9;
...
%                VFI - 工作期值函数
%  =====================================================================
function v = fun_valuefunc_work(x, cash, fund, V_next, cS, t)
    % x = [绝对消费 C, 风险资产比例 alpha, 绝对养老金缴费 Q]
    c_rate = x(1);
    alpha = x(2);
    pension_rate = x(3);
  
    pension_contrib = pension_rate * cash;
    consumption = c_rate*(cash-pension_contrib+cS.tau_y*pension_contrib);
...
function simulate_model(results, cS)
    ...
    simS_norm = zeros(cS.tn, nsim);
    simB_norm = zeros(cS.tn, nsim);
    simQ_Cash_ratio = zeros(cS.tn, nsim); % 新增: 用于存储 q/cash 比率
    ...
    last_work_u = simY_norm(work_periods, :);
    norm_base_pension = cS.ret_fac * last_work_u;
    simY_norm(work_periods+1:cS.tn, :) = repmat(norm_base_pension, cS.td-cS.tr+1, 1);
    ...
    for t = 1:cS.tn
        ...
        norm_cash_clipped = max(min(norm_cash, cS.gcash(end)), cS.gcash(1));
        norm_fund_clipped = max(min(simF_norm(t, :), cS.gfund(end)), cS.gfund(1));
        
        % 插值获取最优决策 (这些已经是满足约束的绝对值)
        simC_norm(t, :) = interp2(cS.gfund, cS.gcash, C_policy(:,:,t)', norm_fund_clipped, norm_cash_clipped, 'spline');
        simA(t, :)      = interp2(cS.gfund, cS.gcash, A_policy(:,:,t)', norm_fund_clipped, norm_cash_clipped, 'spline');
        simQ_norm(t, :) = interp2(cS.gfund, cS.gcash, Q_policy(:,:,t)', norm_fund_clipped, norm_cash_clipped, 'spline');
        
        simA(t, :)      = max(min(simA(t, :), 1), 0);
        
        % *** 核心修正: 直接使用VFI策略，计算剩余储蓄 ***
        if t <= work_periods % 工作期
            % 安全检查，防止插值错误导致消费和缴费超过现金持有量
            ...
            simQ_norm(t, :) = simQ_norm(t, :) .* scale_factor;
            
            % 计算 q/cash 比率用于绘图
            simQ_Cash_ratio(t, :) = simQ_norm(t, :) ./ norm_cash;
            simQ_Cash_ratio(t, isnan(simQ_Cash_ratio(t,:))) = 0;
        ...
        if t < cS.tn
            ...
            if t <= work_periods
                ...
                simF_norm(t+1, :) = simF_norm(t, :) * (1 - cS.pension_rate);
            end
        end
    end
    ...
    meanTotalW = mean(simW_norm + simF_norm, 2);
    meanLiquidW = mean(simW_norm, 2);
    meanF = mean(simF_norm, 2);
    meanA = mean(simA, 2);
    ...
    subplot(2,2,1);
    plot(ages, meanTotalW, 'b-', 'LineWidth', 2, 'DisplayName', 'Total Wealth (w+f)');
    hold on;
    ...
    hold on;
    % 计算 q/cash 的均值 (mean of ratios)
    meanQ_Cash_ratio = nanmean(simQ_Cash_ratio, 2);
    plot(ages(1:work_periods), meanQ_Cash_ratio(1:work_periods), 'g-', 'LineWidth', 2, 'DisplayName', 'Pension Contrib. (q/cash)');
    xline(cS.tr, '--', 'Retirement', 'LineWidth', 1.5);
    hold off;
    title('Mean Portfolio and Pension Choices'); xlabel('Age'); ylabel('Share');
    legend('show', 'Location', 'best'); grid on; xlim([cS.tb, cS.td]); ylim([-0.05, 1.05]);
    
    subplot(2,2,3);
    plot(ages, meanF, 'c-', 'LineWidth', 2, 'DisplayName', 'Pension Fund (f)');
    xline(cS.tr, '--', 'Retirement', 'LineWidth', 1.5);
    hold off;
    title('Mean Pension Fund Dynamics (Normalized)'); xlabel('Age');
    legend('show', 'Location', 'northwest'); grid on; xlim([cS.tb, cS.td]);
    
    subplot(2,2,4);
    % 计算 (w+f)/y 的均值 (mean of ratios)
    simWY_norm = (simW_norm + simF_norm) ./ simY_norm;
    simWY_norm(isinf(simWY_norm) | isnan(simWY_norm)) = NaN;
    meanWY_norm = nanmean(simWY_norm, 2);
    plot(ages, meanWY_norm, 'LineWidth', 2);
    xline(cS.tr, '--', 'Retirement', 'LineWidth', 1.5);
    title('Mean Wealth-to-Income Ratio ((w+f)/y)'); xlabel('Age');
    grid on; xlim([cS.tb, cS.td]);

    sgtitle('Lifecycle Simulation Results (Normalized, VFI)', 'FontSize', 16, 'FontWeight', 'bold');
    
    output_filename = fullfile(cS.save_path, 'vfi_simulation_results_normalized.png');
