...
direct_simu = false;
save_path = 'result/sensitivity/EpsteinZin/';
%% 模型求解和模拟
if ~direct_simu
...
psi = 1/gamma;     % Epstein-Zin的跨期替代弹性
        ...
        parfor i2 = 1:nfund
            ...
            for i3 = 1:ncash
                ...
                [policy, fval, exitflag] = fmincon(@(x) fun_valuefunc_retired(x, gcash(i3), gfund(i2), squeeze(V(:, i2,t+1))...
                    , gret, rf, ret_fac, pension_pay, gamma, beta, psi_1, psi_2, theta, gcash, gfund, survprob(t), weig, t), ...
                    x0, [], [], [], [], lb, ub, [], options);
...
% Epstein-Zin效用函数
u = (x(1) * cash+1e-07).^psi_1;  % Epstein-Zin效用
...
int_V_values = max(interp2(gfund, gcash, nextV, fund_1_values, cash_1_values,  'spline'),0);

% 计算加权和
auxVV = sum(weig .* (survprob * (int_V_values .* gyp_value).^(1-gamma)),'all');
