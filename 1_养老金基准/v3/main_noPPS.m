...
file_path = fullfile(save_path, 'vfi_results_nopps_solo.mat');
...
for t = (cS.tr - cS.tb):-1:1
    ...
    for i_cash = 1:cS.ncash
        ...
        if i_cash > 1
            ...
            x0 = [gcash_val * 0.5, 1-1e-06]; % C, alpha, Q
        end
...
% 平滑处理
% results.A_policy(find(abs(cS.gcash-results.C_policy-(1-cS.tau_y)*results.Q_policy)<1e-04))=1;
end
%% =====================================================================
%                VFI - 退休期值函数
%  =====================================================================
...
% results.A_policy(find(abs(cS.gcash-results.C_policy)<1e-05))=1;
...
output_filename = fullfile(cS.save_path, 'vfi_simulation_results_normalized_noPPS.png');
