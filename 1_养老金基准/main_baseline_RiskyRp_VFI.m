...
direct_simu = false;
save_path = 'result/sensitivity/RiskyRp/';
%% 模型求解和模拟
if ~direct_simu
...
Q_Alpha = zeros(ncash, nfund, tn);  % 养老金风险投资比例 - 新增
...
% 终止期 - 扩展到三维
    for i1 = 1:ncash
        for i2 = 1:nfund
            ...
            Q_Alpha(i1, i2, tn) = 0.0;  % 最后一期不购买养老金风险投资
...
for i1 = 1:(tr - tb)
    ...
    fprintf('工作期求解进度: %d/%d\n', i1, tr - tb);
        ...
        temp_Q_Alpha = zeros(ncash, nfund);
        temp_V = zeros(ncash, nfund);  
        ...
        parfor i2 = 1:nfund
            ...
            temp_Q_Alpha_i2 = zeros(ncash, 1);
            temp_V_i2 = zeros(ncash, 1);
            
            for i3 = 1:ncash
                if i1==1
                     x0 = [ C(i3,i2,t+1), A(i3,i2,t+1), 0.5, 0.5];
                else
                x0 = [ C(i3,i2,t+1), A(i3,i2,t+1), Q(i3,i2,t+1), 0.5];
                end
                lb = [0, 0, 0, 0];
                ub = [1, 1, 1, 1];
            ...
            temp_A(:, i2) = temp_A_i2;
            temp_Q(:, i2) = temp_Q_i2;
            temp_Q_Alpha(:, i2) = temp_Q_Alpha_i2;
            temp_V(:, i2) = temp_V_i2;
        end
            % 将临时结果复制到主变量
        C(:, :, t) = temp_C;
...
% writematrix(Q_Alpha(:, 1, :), fullfile(save_path, 'Q_Alpha_slice.csv'));
...
model_results.Q_Alpha = Q_Alpha;  % 增加养老金风险投资比例
model_results.V = V;
...
%% 数值模拟
% 调用模拟函数
model_results = load(fullfile(save_path, 'model_results.mat'));
fprintf('开始进行数值模拟...\n');
...
function simulate_model(model_results,save_path)
    ...
    Q_Alpha = model_results.Q_Alpha;  % 养老金风险投资比例 - 新增
    ...
    meanQ_Alpha = zeros(tn, 1);  % 平均养老金风险投资比例 - 新增
    meanWY = zeros(tn, 1);
    ...
    simQ_Alpha = zeros(tn, nsim);  % 模拟养老金风险投资比例 - 新增
    ...
    for t = 1:tn
        ...
        simA_t = zeros(1, nsim);
        simQ_t = zeros(1, nsim);
        simQ_Alpha_t = zeros(1, nsim);  % 养老金风险投资比例 - 新增
       ...
       simQ_Alpha_t = interp2(gfund, gcash, Q_Alpha(:,:,t), fund_t, cash_t, 'spline');  % 养老金风险投资比例 - 新增
        ...
        simQ_Alpha_t = min(max(simQ_Alpha_t, 0), 1);  % 养老金风险投资比例范围约束为0-1 - 新增
    ...
    meanQ_Alpha = mean(simQ_Alpha, 2);  % 养老金风险投资比例 - 新增
    ...
    % 创建表格保存模拟结果
    sim_results_table = table(meanC, meanC_pct, meanY, meanW, meanS, meanB, meanF, meanP, meanQ, meanQ_Alpha, meanWY, meanalpha, meanGPY);
    writetable(sim_results_table, fullfile(save_path, 'simulation_results.csv'));
    
    % 保存原始模拟数据用于进一步分析
    save(fullfile(save_path, 'raw_simulation.mat'), 'simC', 'simA', 'simY', 'simW', 'simS', 'simB', 'simF', 'simP', 'simQ', 'simQ_Alpha', 'simR', 'simGPY', '-v7.3');
    ...
    plot([tb:td],meanQ_Alpha, 'LineWidth', 2);
    xline(tr, '--', '退休年龄', 'LineWidth', 1.5);
    hold off;    
    legend('消费比例 (c)', '风险资产配置比例 (\alpha)', '养老金购买比例 (q)', '养老金风险投资比例 (q\_\alpha)', 'Location', 'best');
    xlabel('年齡');
    ylabel('比例');
    title('消费、风险资产配置和养老金相关决策比例');
    ...
    legend('养老基金余额 (F)', '养老金给付 (P)', 'Location', 'best');
    xlabel('期数');
    ylabel('数值');
    title('养老基金余额和养老金给付');
    ...
    legend('养老金风险投资比例 (q\_\alpha)', 'Location', 'best');
    xlabel('年齡');
    ylabel('比例');
    title('养老金风险投资比例');
    
    grid on;
    saveas(gcf, fullfile(save_path, 'pension_risk_allocation_plot.png'));
    saveas(gcf, fullfile(save_path, 'pension_risk_allocation_plot.fig'));
...
%
% 参数:
% x - 决策变量 [消费比例, 风险资产投资比例, 个人养老金购买比例, 养老金风险投资比例]
% cash - 手中现金
% fund - 养老基金账户余额
% gyp - 劳动收入增长率
% nextV - 下期值函数
...
% 计算养老基金的投资回报
pension_return = rp * (1 - x(4)) + gret_value * x(4);

cash_1_values = portfolio_return .* sav_values + (1-tau) * yh_value;
% 修改养老基金的回报计算方式
fund_1_values = (fund + pension_contrib) .* pension_return ./ gyp_value + tau * yh_value;
