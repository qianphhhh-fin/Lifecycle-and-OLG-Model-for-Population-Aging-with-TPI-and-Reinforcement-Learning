...
for i1 = 1:(td - tr)
    ...
    for i3 = 1:ncash
        ...
        for i3 = 1:ncash
            x0 = [0.8, 0.5];
            lb = [0, 0];
            ub = [1, 1];  % 消费比例的上界改为1（而不是0.999*gcash(i3)）
            ...
            [policy, fval, exitflag] = fmincon(@(x) evaluate_policy_retired_pct(x, gcash(i3), V(:, t+1),...
...
for i1 = 1:(tr - tb)
    ...
    for i3 = 1:ncash
        ...
        for i3 = 1:ncash
            ...
            % 优化函数，使用fmincon模拟Python的minimize函数
            [policy, fval, exitflag] = fmincon(@(x) evaluate_policy_work(x, gcash(i3), gyp(:, :, t), V(:, t+1),...
                yh, gret, rf, gamma, beta, psi_1, psi_2, theta, gcash, survprob(t), n, weig), ...
                x0, [], [], [], [], lb, ub, [], options);
...
save(fullfile(save_path, 'cocco.mat'), 'model_results');
%% 数值模拟
% 调用模拟函数
% fprintf('开始进行数值模拟...\n');
...
function simulate_model(model_results)
    ...
    meanCs = zeros(tn, 1);
    meanWs = zeros(tn, 1);
    simPY = zeros(tn, nsim);  % 持久性收入冲击p_t
    simGPY = zeros(tn, nsim); % 持久性收入增长率
    simY = zeros(tn, nsim);   % 总收入
    simC = zeros(tn, nsim);
    simW = zeros(tn, nsim);
    simS = zeros(tn, nsim);
    simB = zeros(tn, nsim);
    ...
    simR = zeros(tn, nsim);
    eps_y = zeros(1, 1);
    simTY = zeros(tn, nsim);  % 修改为二维数组，保存每期每个模拟的临时收入冲击
    eps_r = zeros(1, 1);
    cash = zeros(tn, nsim);
    ...
    for i1 = 1:(nsim/2) % 另外一半模拟完全对称
        ...
        % 生成第一期临时收入冲击并保存
        temp_ty = randn(1);
        simTY(1, i1) = exp(temp_ty * smay); % 保存临时收入冲击u_t
        simTY(1, nsim/2 + i1) = exp(-temp_ty * smay);
        
        simY(1, i1) = simTY(1, i1);
        simY(1, nsim/2 + i1) = simTY(1, nsim/2 + i1);
        ...
        for i2 = 2:(tr-tb)
            ...
            % 生成每期临时收入冲击并保存
            temp_ty = randn(1);
            simTY(i2, i1) = exp(temp_ty * smay);
            simTY(i2, nsim/2 + i1) = exp(-temp_ty * smay);
            
            simY(i2, i1) = simTY(i2, i1);
            simY(i2, nsim/2 + i1) = simTY(i2, nsim/2 + i1);
        end
    end
    ...
    for t = 1:tn
        ...
        simC(t, :) = interp1(gcash, C(:, t), cash_t, 'spline', 'extrap');  % 现在直接获取消费比例
        simA(t, :) = interp1(gcash, A(:, t), cash_t, 'spline', 'extrap');
        
        % 确保约束条件满足
        simC(t, :) = max(min(simC(t, :), 0.9999), 0);  % 消费比例范围约束为0-1
        simA(t, :) = min(max(simA(t, :), 0), 1);
        
        % 计算各种模拟变量
        simC_pct(t, :) = simC(t, :);  % 现在simC已经是消费比例，直接赋值
    ...
    meanTY = mean(simTY, 2);  % 计算平均临时收入冲击
    meanCash = mean(cash, 2); % 计算平均现金持有量
    meanPY = mean(simPY, 2);  % 计算平均持久性收入冲击
    ...
    % 创建表格保存模拟结果
    sim_results_table = table(age, meanC, meanC_pct, meanY, meanW, meanS, meanB, meanWY, meanalpha, meanGPY, meanTY, meanCash, meanPY);
    writetable(sim_results_table, 'result_cocco_matlab_PFI/simulation/simulation_results.csv');
    
    % 保存原始模拟数据用于进一步分析
    save('result_cocco_matlab_PFI/simulation/raw_simulation.mat', 'simC', 'simA', 'simY', 'simW', 'simS', 'simB', 'simR', 'simGPY', 'simTY', 'cash', 'simPY', '-v7.3');
    
    % 绘制决策变量：消费比例和资产配置
    figure(1);
    plot(age, meanC_pct, 'LineWidth', 2);
    hold on;
    plot(age, meanalpha, 'LineWidth', 2);
    hold off;
    ...
    % 绘制状态变量：现金和财富
    figure(2);
    plot(age, meanCash, 'LineWidth', 2);
    hold on;
    plot(age, meanW, 'LineWidth', 2);
    plot(age, meanY, 'LineWidth', 2);
    xline(tr, '--', '退休年龄', 'LineWidth', 1.5);
    hold off;
    legend('现金持有量 (Cash)', '财富 (Wealth)', '收入 (Income)', 'Location', 'best');
    xlabel('年龄');
    ylabel('数量');
    title('状态变量：现金和财富 (PFI方法)');
    grid on;
    
    % 绘制外生冲击：临时收入冲击和持久性收入
    figure(3);
    plot(age, meanTY, 'LineWidth', 2);
    hold on;
    plot(age, exp(meanPY), 'LineWidth', 2);  % 转换为水平值便于比较
    plot(age, meanGPY, 'LineWidth', 2);
    xline(tr, '--', '退休年龄', 'LineWidth', 1.5);
    hold off;
    legend('临时收入冲击 (u_t)', '持久性收入水平 (exp(p_t))', '持久性收入增长率 (G_t)', 'Location', 'best');
    xlabel('年龄');
    ylabel('数量');
    title('外生冲击：收入冲击过程 (PFI方法)');
    grid on;
    
    % 绘制储蓄和投资组合
    figure(4);
    plot(age, meanS, 'LineWidth', 2);
    hold on;
    plot(age, meanB, 'LineWidth', 2);
    plot(age, meanS + meanB, 'LineWidth', 2);
    xline(tr, '--', '退休年龄', 'LineWidth', 1.5);
    hold off;
    legend('风险投资 (S)', '无风险投资 (B)', '总储蓄 (S+B)', 'Location', 'best');
    xlabel('年龄');
    ylabel('数量');
    title('投资组合: 风险资产和无风险资产 (PFI方法)');
    grid on;
    
    % 保存所有图像
    saveas(figure(1), 'result_cocco_matlab_PFI/simulation/results_decisions.png');
    saveas(figure(1), 'result_cocco_matlab_PFI/simulation/results_decisions.fig');
    
    saveas(figure(2), 'result_cocco_matlab_PFI/simulation/results_states_cash_wealth.png');
    saveas(figure(2), 'result_cocco_matlab_PFI/simulation/results_states_cash_wealth.fig');
    
    saveas(figure(3), 'result_cocco_matlab_PFI/simulation/results_shocks.png');
    saveas(figure(3), 'result_cocco_matlab_PFI/simulation/results_shocks.fig');
    
    saveas(figure(4), 'result_cocco_matlab_PFI/simulation/results_portfolio.png');
    saveas(figure(4), 'result_cocco_matlab_PFI/simulation/results_portfolio.fig');
    
    fprintf('数值模拟完成，结果已保存到result_cocco_matlab_PFI/simulation文件夹\n');
    fprintf('已添加状态变量随年龄变化的图表，共4张图表\n');
end
...
% 
% auxVV = 0;
% consumption = policy(1);  % 消费绝对值 = 消费比例 * 现金
% sav = cash - policy(1);    % 储蓄 = (1-消费比例) * 现金
% 
% % 效用函数
...
function v = evaluate_policy_retired_pct(policy, cash, nextV, gret, rf, ret_fac, gamma, beta, psi_1, psi_2, theta, gcash, survprob, weig)
% 对退休期间的给定策略进行评估，计算其值函数
%
% 参数:
% policy - 策略 [消费比例, 风险资产投资比例]
...
auxVV = 0;
consumption = policy(1) * cash;  % 消费绝对值 = 消费比例 * 现金
sav = (1 - policy(1)) * cash;    % 储蓄 = (1-消费比例) * 现金
...
for i6 = 1:n
    for i7 = 1:n
        for i8 = 1:n
            % 计算储蓄值 - 使用消费比例
            sav_values(i6, i7, i8) = ((1 - x(1)) * cash) / gyp(i6, i8);
