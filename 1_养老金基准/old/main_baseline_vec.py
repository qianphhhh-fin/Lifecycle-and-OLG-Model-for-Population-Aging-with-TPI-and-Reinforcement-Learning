...
def evaluate_policy_retired(policy, cash, fund, nextV, nextF, gret, rf, ret_fac, pension_pay, gamma, beta, psi_1, psi_2, theta, gcash, gfund, survprob, weig):
    ...
    # 下期的cash-on-hand
    portfolio_return = rf * (1 - policy[1]) + gret * policy[1]
    cash_1_values = portfolio_return * sav + ret_fac + pension_pay
    ...
    cash_idx = np.searchsorted(gcash, cash_1_values) - 1
    cash_idx = np.maximum(0, np.minimum(cash_idx, len(gcash) - 2))
    cash_weight = (cash_1_values - gcash[cash_idx]) / (gcash[cash_idx + 1] - gcash[cash_idx])

    # 双线性插值
    v0 = nextV[cash_idx]
    v1 = nextV[cash_idx + 1]

    int_V_values = v0 * (1 - cash_weight) + v1 * cash_weight
    ...
    return v
# from main_baseline import evaluate_policy_retired
...
def cocco_fun_valuefunc_work(x, cash, fund, gyp, nextV, nextF, yh, gret, rf, rp, tau, gamma, beta, psi_1, psi_2, theta, gcash, gfund, survprob, n, weig):
    ...
    # 遍历所有可能的状态
    fund_idx = np.searchsorted(gfund, fund_1_values) - 1
    fund_idx = np.maximum(0, np.minimum(fund_idx, len(gfund) - 2))    
    fund_weight = (fund_1_values - gfund[fund_idx]) / (gfund[fund_idx + 1] - gfund[fund_idx])
    cash_idx = np.searchsorted(gcash, cash_1_values) - 1
    cash_idx = np.maximum(0, np.minimum(cash_idx, len(gcash) - 2))
    cash_weight = (cash_1_values - gcash[cash_idx]) / (gcash[cash_idx + 1] - gcash[cash_idx])
    # 双线性插值
    ...
    int_V_values = (1 - cash_weight) * (1 - fund_weight) * v00 + \
    ...
    int_F_values = (1 - cash_weight) * (1 - fund_weight) * f00 + \
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mode', type=str, choices=['solve', 'simulate', 'both'],
        default='solve', help='运行模式：求解(solve)，模拟(simulate)，或两者都运行(both)')
