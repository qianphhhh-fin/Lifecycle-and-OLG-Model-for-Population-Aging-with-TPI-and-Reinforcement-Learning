...
secd(:,1) = f_spline(gcash(:,1),V(:,tn),ncash,1.0); % 给定cash的grid以及对应最后一期的值函数值，计算每个grid处的导数
...
% ======================== *内生状态变量cash ==============================
   for i3=1:ncash 
      ...
      if i3==1 % 
         ...
         minc = C(i3-1,t); % 由于C(cash)是一个增函数，故令最小minc等于上一个grid的cash的C
         if i3<10 
            maxc = minc + (gcash(i3,1) - gcash(i3-1,1)); % maxc不需要等于gcash(i3,1)(上限太大), 只需要增加两个cash grid多出来的部分就好
        else
...
%       minmaxcash(i3,1) = minc;
%       minmaxcash(i3,2) = maxc;
      ...
      loggc(:,i3,i1) = [gc(1);gc(end)]; % 记录用
 % 开始迭代
 % ========================  *控制变量c ===================================
      for i4=1:nc 
        % u=(1.0-delta)*(gc(i4,1)^psi_1) ;
        u = gc(i4,1)^(1-rho)/(1-rho);
...
% ========================  *外生状态变量:只有资产收益的随机项 =============
                 for i8=1:n % 随机资产收益率的grid
                    ...
                    % int_V = interp1(gcash(:,1), V(:,t+1), cash_1, 'linear'); % matlab自带的插值，无需利用f_spline.m手动计算二次导数
                    auxVV= auxVV + weig(i8,1)*survprob(t,1)*(int_V); % 累加auxVV得到EV
                 end 
%              end       
%           end        
% 得到所有可能的control var的grid下的V
            auxV(i5,i4) = u+delta*(auxVV);    %1/(1-1/psi), Epstein-Zin效用形式
        end
      end
   ...
   secd(:,1) = f_spline(gcash(:,1),V(:,t),ncash,1.0); % 给定cash的grid以及对应t期的值函数值，更新每个grid处的导数
end
            ...
            for i6=1:n
               for i8=1:n
                  for i7=1:n
                     ...
                     auxVV  = auxVV+nweig1(i6,i7,i8)*survprob(t,1)*((int_V*gyp(i6,i8,t))); % 新的scaled value function
                  end 
               end 
            end 
            auxV(i5,i4) = u+delta*(auxVV);    
         end 
      end 
...
for t=1:tn
    for i1=1:nsim
       ...
       % simC(t,i1) = max(min(simC(t,i1),1),0);
       % simA(t,i1) = max(min(simA(t,i1),1),0);
       sav = cash(t,i1)-simC(t,i1); % 用于投资的金额
       simS(t,i1) = simA(t,i1)*sav; % 风险投资额
       simS(t,i1) = min(simS(t,i1),sav);
       simB(t,i1) = sav-simS(t,i1); % 无风险投资额
       if (t<tn) 
          simW(t+1,i1) = (simB(t,i1)*r+simS(t,i1)*simR(t,i1))/simGPY(t+1,i1); % 下期一开始的(scaled)财富（拿到下期工资收入前）
       end
    end
end 
