...
# 退休期间进行策略评估的函数
def evaluate_policy_retired(policy, cash, nextV, gret, rf, ret_fac, gamma, beta, psi_1, psi_2, theta, gcash, survprob, weig):
    """
    ...
    # 下期的cash-on-hand
    cash_1 = (rf * (1 - policy[1]) + gret * policy[1]) * sav + ret_fac
...
# 工作期间进行策略评估的函数
def evaluate_policy_work(policy, cash, gyp, nextV, yh, gret, rf, gamma, beta, psi_1, psi_2, theta, gcash, survprob, n, weig):
    """
    ...
    # 计算所有可能的储蓄值
    sav_values = (cash - policy[0]) / gyp[i6_grid, i8_grid]
...
def solve_model():
    ...
    for i1 in tqdm(range(1, td - tr + 1), desc="退休期求解进度"):
        ...
        for iter_count in range(max_iterations):
            # 策略评估 - 计算当前策略的值函数
            V_new = np.zeros(ncash)
            for i3 in range(ncash):
                policy = np.array([C[i3, t], A[i3, t]])
                V_new[i3] = -evaluate_policy_retired(policy, gcash[i3], V[:, t+1], gret, rf, ret_fac, gamma, beta, psi_1, psi_2, theta, gcash, survprob[t], weig)
            
            # 策略改进 - 基于当前值函数更新策略
            C_new = np.zeros(ncash)
            A_new = np.zeros(ncash)
            ...
            for i3 in range(ncash):
                ...
                # 优化函数
                result = minimize(
                    lambda x: evaluate_policy_retired(x, gcash[i3], V[:, t+1], gret, rf, ret_fac, gamma, beta, psi_1, psi_2, theta, gcash, survprob[t], weig),
            ...
            # 计算策略变化，检查收敛
            policy_change = np.max(np.abs(C_new - C[:, t])) + np.max(np.abs(A_new - A[:, t]))
    ...
    for i1 in tqdm(range(1, tr - tb + 1), desc="工作期求解进度"):
        ...
        for iter_count in range(max_iterations):
            ...
            for i3 in range(ncash):
                policy = np.array([C[i3, t], A[i3, t]])
                V_new[i3] = -evaluate_policy_work(policy, gcash[i3], gyp[:, :, t], V[:, t+1], yh, gret, rf, gamma, beta, psi_1, psi_2, theta, gcash, survprob[t], n, nweig1)
    ...
    # 保存结果
    os.makedirs('result_cocco_python_PFI', exist_ok=True)
    pd.DataFrame(C).to_excel('result_cocco_python_PFI/C.xlsx')
    pd.DataFrame(A).to_excel('result_cocco_python_PFI/A.xlsx')
    pd.DataFrame(V).to_excel('result_cocco_python_PFI/V.xlsx')
    pd.DataFrame(gcash).to_excel('result_cocco_python_PFI/gcash.xlsx')

    print("模型求解完成，结果已保存到result_cocco_python_PFI文件夹")
...
def simulate_model(model_results=None):
    ...
    if model_results is None:
        try:
            # 检查文件是否存在
            if not os.path.exists('result_cocco_python_PFI/C.xlsx'):
                print("未找到模型求解结果文件，请先运行模型求解")
                return
                
            # 从文件中读取模型结果
            C = pd.read_excel('result_cocco_python_PFI/C.xlsx', index_col=0).values
            A = pd.read_excel('result_cocco_python_PFI/A.xlsx', index_col=0).values
            gcash = pd.read_excel('result_cocco_python_PFI/gcash.xlsx', index_col=0).values.flatten()
    ...
    sim_results.to_excel('result_cocco_python_PFI/simulation_results.xlsx')
    # 绘制结果
    ...
    plt.xlabel('Period')
    plt.ylabel('Value')
    plt.title('Consumption Ratio and Portfolio Allocation (PFI)')
    plt.savefig('result_cocco_python_PFI/results_plot.png')
    plt.show()
    print("数值模拟完成，结果已保存到result_cocco_python_PFI文件夹")
