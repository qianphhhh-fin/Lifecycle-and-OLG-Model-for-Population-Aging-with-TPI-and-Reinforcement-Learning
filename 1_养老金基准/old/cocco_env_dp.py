...
class CoccoEnv(gym.Env):
    ...
    def __init__(self, params): # 
        ...
        super(CoccoEnv, self).__init__()
         ...
         # 设置默认参数
        self.scale_inc = 1.5 # 用于缩放收入的系数
        self.tb = 20 # 96#20  # 初始年龄
        self.tr = 23 # 98#61  # 退休年龄
        self.td = 25  # 最大年龄
    ...
    0.82935, 0.81282, 0.79485, 0.77543, 0.75458, 0.7324, 0.70893, 0.68424]
        # survprob_values = list(np.ones(101))
        ...
        self.action_space = spaces.Box(low=np.array([1e-3, -1]),\
                                       high=np.array([100, 2]), shape=(2,), dtype=float)
        
        # 状态空间: [现金, 年龄, 永久收入]
        self.observation_space = spaces.Box(
            low=np.array([0.25, self.tb, -1e3]),
            high=np.array([50.0, self.td, 1e3]),
            shape=(3,),
            dtype=float
        )             
        self.state = None
    ...
    def step(self, action):
        ...
        # Unpack the action
        consum = action[0] # 消费数量
        risky_pct = np.clip(action[1],0,1) # 风险资产比例 

 
        cash, age, perm_shock = self.state 
...
# === 状态转移方程（退休前） ===       
        if self.age < self.tr:
            ...
            self.cash = (self.cash - consum*np.exp(self.f(self.age) + perm_shock)) * (risky_pct * (1 + rand_return) \
            ...
            info = {'age':np.array([self.age]),'normalized_income':self.wage/np.exp(self.f(self.age+1) + new_perm_shock),\
                    'raw_consumption':consum*np.exp(self.f(self.age) + perm_shock),\
...
# === 状态转移方程（退休后） ===   
            ...
            self.wage = np.exp(log_rand_wage) # 下一期工资
            # 当期到手现金
            self.cash = (self.cash - consum*np.exp(self.last_f + self.last_perm_shock)) * (risky_pct * (1 + rand_return) \
                                                  + (1-risky_pct) * (1 + self.rf)) + self.wage #  raw cash
            new_cash = self.cash/np.exp(self.last_perm_shock + self.last_f) # normalized cash

                                               
            # print([new_cash,self.age,'retired'])
            info = {'age':np.array([self.age]),'normalized_income':self.ret_fac,'raw_consumption':consum*np.exp(self.last_f + self.last_perm_shock),\
        ...
        if self.age < self.tr:
            # reward = -((consum*np.exp(self.f(self.age) + perm_shock))**(1-self.gamma))/10
            reward = np.log(consum*np.exp(self.f(self.age) + perm_shock))
            # reward = -((consum*np.exp(self.f(self.age) + perm_shock))**(1-self.gamma))             
        else:
            # reward = -((consum*np.exp(self.last_perm_shock + self.last_f))**(1-self.gamma))/10
            reward = np.log(consum*np.exp(self.last_perm_shock + self.last_f))
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        self.wage = rand_wage
        self.cash = self.init_cash*np.exp(self.f(self.age) + perm_shock) + self.wage # raw cash
        ...
        self.state[2] = np.array([perm_shock],dtype=float)
...
if __name__=="__main__":
    ...
    # ===== 记录每个agent的年龄和原始消费 =====
    # 创建一个数组用于存储每个年龄段的标准化收入,行数为年龄数,列数为agent数
    raw_consumption_by_age = np.zeros((len(unique_age),len(all_trajectories)))
    # 遍历每个agent
    for agent_idx in range(len(all_trajectories)):
        # 获取当前agent的年龄和收入数据
        agent_age = all_trajectories[agent_idx]['age']
        agent_consumption = np.array([info['raw_consumption'] for info in all_trajectories[agent_idx]['infos']])

        # 遍历该agent的每个时间步
        for t in range(len(agent_age)):
            # 找到当前年龄在unique_age中的索引
            age_idx = np.where(unique_age == agent_age[t])[0][0]
            # 将收入填入对应位置
            raw_consumption_by_age[age_idx, agent_idx] = agent_consumption[t]
            
    # 将未填充的位置设为nan
    raw_consumption_by_age[raw_consumption_by_age == 0] = np.nan
    import pandas as pd   
    # 创建DataFrame保存标准化收入数据
    df_income = pd.DataFrame(raw_consumption_by_age, 
                           columns=[f'Agent_{i+1}' for i in range(raw_consumption_by_age.shape[1])])
    df_income.index = unique_age
    df_income.index.name = '年龄'  
    # 保存到Excel
    df_income.to_excel(os.path.join(current_path, 'result/raw_consumption_by_age.xlsx'))

    df_utility = -(df_income)**(1-env.gamma)*50 + 10   
    
    # 计算每个agent的终身效用
    agent_utility = pd.DataFrame(score_history, \
                                                   index=[f'Agent_{i+1}' for i in range(len(score_history))]).T
    df_utility = pd.concat([df_utility, agent_utility], axis=0)
    df_utility.to_excel(os.path.join(current_path, 'result/utility_by_age.xlsx')) 


    # ===== 画出x轴为年龄，y轴为action的图 =====
    # 将所有轨迹的动作合并为一个数组
    action_by_age = np.concatenate([traj['actions'] for traj in all_trajectories])
    # 创建2个数组用于存储每个年龄段的action0和action1,行数为年龄数,列数为agent数
    action0_by_age = np.full((len(unique_age),len(all_trajectories)),np.nan)
    action1_by_age = np.full((len(unique_age),len(all_trajectories)),np.nan)
    # 遍历每个agent,将action0和action1填入对应年龄的数组中
    for agent_idx in range(len(all_trajectories)):
        # 获取当前agent的年龄和动作数据
        agent_age = all_trajectories[agent_idx]['age']
        agent_actions = all_trajectories[agent_idx]['actions']
        ...
        for t in range(len(agent_age)):
            ...
            age_idx = np.where(unique_age == agent_age[t])[0][0]
            # 将动作填入对应位置
            action0_by_age[age_idx, agent_idx] = agent_actions[t][0]
            action1_by_age[age_idx, agent_idx] = agent_actions[t][1]
            



    # 保存为excel
    df_action0 = pd.DataFrame(action0_by_age, columns=[f'Agent_{i+1}' for i in range(action0_by_age.shape[1])])
    df_action0.index = unique_age
    df_action0.index.name = '年龄'
    df_action0.to_excel(os.path.join(current_path, 'result/action0_by_age.xlsx'))

    df_action1 = pd.DataFrame(action1_by_age, columns=[f'Agent_{i+1}' for i in range(action1_by_age.shape[1])])
    df_action1.index = unique_age
    df_action1.index.name = '年龄'
    df_action1.to_excel(os.path.join(current_path, 'result/action1_by_age.xlsx'))
    ...
    plt.plot(unique_age, np.nanmean(action0_by_age,axis=1),label='消费比例')
    plt.plot(unique_age, np.nanmean(action1_by_age,axis=1),label='风险资产比例')
    plt.legend()
    plt.savefig(os.path.join(current_path, 'result/lifecycle_plot_cocco.png'))
