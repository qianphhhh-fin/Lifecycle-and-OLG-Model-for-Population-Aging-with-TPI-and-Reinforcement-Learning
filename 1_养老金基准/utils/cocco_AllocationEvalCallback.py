...
class EvalCallback(EventCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            if self.verbose >= 1:
                print(f"Eval num_timesteps={self.num_timesteps}, " f"episode_reward={mean_reward:.4f} +/- {std_reward:.4f}")
            ...
            if mean_reward >= self.best_mean_reward:
                ...
                if self.best_model_save_path is not None:
                    # self.model.save(os.path.join(self.best_model_save_path, f"best_model_{self.n_evals}"))
                ...
                plt.title(f"{runs}: Best Model {self.n_evals}: Timesteps={self.num_timesteps}, " f"episode_reward={mean_reward:.4f} ± {std_reward:.4f}")
                plt.xlabel('Age')
                plt.legend()
                plt.grid(True)
            ...
            # else:
                ...
                # plt.plot(mean_actions[:,0], mean_actions[:,2], label='riksy') 
                # runs = self.best_model_save_path.split('_')[-1]  
                # plt.title(f"{runs}: Model {self.n_evals}: Timesteps={self.num_timesteps}, " f"episode_reward={mean_reward:.4f} ± {std_reward:.4f},  Best={self.best_mean_reward:.4f}")
                # plt.xlabel('Age')
                # plt.legend()
                # plt.grid(True)
                # self.logger.record("trajectory/figure", Figure(figure, close=True), exclude=("stdout", "log", "json", "csv"))
