...
class PensionFundEnv(gym.Env):
    ...
    0.82935, 0.81282, 0.79485, 0.77543, 0.75458, 0.7324, 0.70893, 0.68424]
        ...
        self.action_space = spaces.Box(low=np.array([0.05, -0.2]),\
    ...
    def step(self, action):
        ...
        tda_pct = np.float64(0) # 购买个人养老金比例
        tda_risky_pct = np.float64(0) # 个人养老金中购买风险资产比例
...
# === 状态转移方程（退休后） ===   
        ...
        # print(norm_consum)
        # if np.isnan(reward) or np.isnan(self.state).any():
        #     raise ValueError("Reward或state中包含NaN值")
        self.age += 1
        self.wage = new_wage    
        # Determine if the episode is done
        # 如果完成，则返回同样的state，在计算时game结束的下一期的值函数无论如何都为0（不影响
        # 随机死亡or超过最大年龄       
        self.done = True if (self.age > self.td) or (np.random.choice([True,False], 1,  p=[1-self.survprob_dict[self.age-1],self.survprob_dict[self.age-1]])) else False
...
if __name__=="__main__":
    ...
    env = gym.make('pensionfund-v3-simplified', params=params)
    ...
    print(f'模拟 {n_agents} 个agent共耗时: {elapsed_time:.2f} 秒')
    #     pbar.update(1)
    # pbar.close()
