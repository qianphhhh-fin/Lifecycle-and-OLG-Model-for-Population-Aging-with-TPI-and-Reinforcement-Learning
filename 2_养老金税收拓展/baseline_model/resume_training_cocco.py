...
from utils.AllocationEvalCallback_cocco import EvalCallback
...
model_name = 'cocco-v0_run14//'
log_dir = '../autodl-tmp/models//sac//'

  
vec_env = make_vec_env("cocco-v0",seed=1, n_envs=88, env_kwargs={'params':None},monitor_kwargs={'discount_factor': distf})
...
model.learning_starts = 10_000 # 重新训练不需要/使用已有policy生成
...
model.tau = 0.01
model.gradient_steps = 11
# model.ent_coef = 0.001
# model.action_noise = OrnsteinUhlenbeckActionNoise(mean=np.zeros(vec_env.action_space.shape[0]), sigma=0.5* np.ones(vec_env.action_space.shape[0]))
...
# create eval env
eval_env = make_vec_env("cocco-v0",seed=3687851522, n_envs=1, env_kwargs={'params': None},\
