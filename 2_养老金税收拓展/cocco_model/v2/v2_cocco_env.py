    ...
    **[修正版 v2]** 此版本修复了 step 函数返回的 info 字典，以支持评估回调。
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        info = {
            ...
            "risky_share": 0.0,
            "normalized_coh": self.X/np.exp(self.f_y[0] + self.P),
        }
    ...
    def step(self, action):
        ...
        reward = np.maximum(utility, -50.0)
        ...
        if self.age <= self.tr:
            ...
            gyp = np.exp(self.f_y[next_age_idx] + self.P)
        else:
            Y_next = self.ret_fac * np.exp(self.f_y[self.tr - self.tb] + self.P_retirement)
            gyp = np.exp(self.f_y[self.tr - self.tb] + self.P_retirement)
