    ...
    **[修正版 v2]** 此版本修复了 step 函数返回的 info 字典，以支持评估回调。
    ...
    def __init__(self, disable_random_death: bool = False):
        ...
        self.observation_space = spaces.Box(
            low=np.array([0.0, -np.inf, -np.inf, -1.0]),
    ...
    def _get_obs(self):
        ...
        return np.array([self.X_norm, self.P, np.log(self.gyp), normalized_age], dtype=np.float32)
    ...
    def step(self, action):
        ...
        if self.gamma == 1: 
            ...
            utility = ((C/self.gyp)**(1 - self.gamma)) / (1 - self.gamma) + 1
        reward = np.maximum(utility, -50)
