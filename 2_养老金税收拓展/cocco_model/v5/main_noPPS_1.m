...
cS.shock_grid = grid;
% 对于i.i.d冲击, 概率就是转移矩阵的任意一行
cS.shock_weig = weig_matrix(1, :)'; 

% 风险资产回报率网格
cS.gret = cS.rf + cS.mu + cS.shock_grid * cS.sigr;

% 状态转移概率 (三维冲击)
% 这个将在 value function 中直接计算, 不再预计算 nweig1
...
% 收入过程网格 (简化为一维向量)
cS.yh_grid = exp(cS.shock_grid * cS.smay); % 暂时冲击 exp(u_t)
cS.yp_grid = exp(cS.shock_grid * cS.smav); % 持久冲击增长 exp(z_t)

% gyp 将在 VFI 循环中根据需要构建
cS.gyp = zeros(cS.n_shocks, cS.tn - 1); % 总收入增长 (持久部分)

% 工作期收入增长
work_periods = cS.tr - cS.tb;
for t = 1:work_periods
    G_t = cS.f_y(t+1) / cS.f_y(t); % 确定性增长
    cS.gyp(:, t) = G_t .* cS.yp_grid;
end
...
options = optimoptions('fmincon', 'Display', 'off', 'Algorithm', 'interior-point', ...
    ...
    'OptimalityTolerance', 1e-15, 'StepTolerance', 1e-15, ...
...
for t = (cS.tn-1):-1:(cS.tr-cS.tb+1)
    ...
    for i_cash = 1:cS.ncash
        ...
        if i_cash > 1 && local_C(i_cash-1) > 0
            x0 = [local_C(i_cash-1)*cS.gcash(i_cash-1)/gcash_val, local_A(i_cash-1)];
        end
...
% x = [消费 C, 风险资产比例 alpha]
alpha = max(0,min(1,x(2)));
consumption = max(0,min(1,x(1))) * cash;
...
% 使用 ndgrid 清晰地构建三维冲击网格
% yz_shocks: 持久收入冲击, yh_shocks: 暂时收入冲击, ret_shocks: 回报冲击
[yz_shocks, yh_shocks, ret_shocks] = ndgrid(cS.gyp(:,t), cS.yh_grid, cS.gret);

% 构建三维概率权重网格
[w_yz, w_yh, w_ret] = ndgrid(cS.shock_weig, cS.shock_weig, cS.shock_weig);
prob_grid = w_yz .* w_yh .* w_ret;

% 下一期归一化状态变量
portfolio_return = cS.rf * (1 - alpha) + ret_shocks * alpha;
cash_1 = portfolio_return .* (liquid_sav ./ yz_shocks) + (1-cS.tau_y) .* yh_shocks;
...
term_inside_exp = (yz_shocks.^(1 - cS.gamma)) .* int_V;
expected_term = sum(prob_grid .* term_inside_exp, 'all');
...
for t = 1:cS.tn
    ...
    % 插值获取最优决策
    simC_prop(t,:)= C_interp_obj(norm_cash);
    simC_prop(t, :) = max(min(simC_prop(t, :), 1), 0);
    simC_norm(t, :) = simC_prop(t, :) .* norm_cash;   
