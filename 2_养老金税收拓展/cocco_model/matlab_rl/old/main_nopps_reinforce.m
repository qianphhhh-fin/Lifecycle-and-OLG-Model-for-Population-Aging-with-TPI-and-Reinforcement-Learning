% =========================================================================
%               生命周期模型求解与模拟 (REINFORCE 深度强化学习方法)
% =========================================================================
% 描述:
% 本脚本使用 REINFORCE (Monte Carlo Policy Gradient) 算法求解生命周期模型。
% REINFORCE 是最基础的策略梯度算法，它直接通过蒙特卡洛模拟的回报 (Return)
% 来更新策略网络 (Actor)，不使用值网络 (Critic)。
%
% 优点: 算法简单，容易实现，是验证环境和奖励函数是否可学习的良好基准。
% 缺点: 梯度估计方差高，可能导致训练不稳定和收敛速度慢。
%
% 结构:
% 1. setup_parameters: 初始化所有模型和 DRL 训练参数。
% 2. solve_model_reinforce: 搭建并训练 Actor 神经网络。
...
% --- 主流程控制器 ---
fprintf('开始执行生命周期模型 (REINFORCE 深度强化学习)...\n');

% 1. 设置所有模型参数
cS = setup_parameters();
save_path = cS.save_path;
file_path = fullfile(save_path, 'reinforce_results_crra_nopps_solo.mat');


% 2. 求解模型 (训练 REINFORCE 网络)
fprintf('\n===== 步骤 2: 求解模型 (REINFORCE 训练) =====\n');
if ~exist(file_path,'file')
    drl_results = solve_model_reinforce(cS);
    if ~exist(save_path, 'dir'), mkdir(save_path); end
    save(file_path, 'drl_results', 'cS');
    fprintf('REINFORCE 模型训练结果已保存到 %s\n', file_path);
else
    load(file_path, 'drl_results', 'cS');
    fprintf('已从 %s 加载预训练的 REINFORCE 模型。\n', file_path);
end
...
cS.drl.exploration_std = [0.3; 0.5];% 探索噪声
cS.drl.wealth_scale = 10.0;           % 归一化财富的缩放因子
cS.drl.income_scale = 2.0;            % 归一化收入的缩放因子
...
cS.yh = zeros(cS.n_shocks, cS.n_shocks);
cS.yp = zeros(cS.n_shocks, cS.n_shocks);
...
cS.gyp = zeros(cS.n_shocks, cS.n_shocks, cS.tn - 1);
...
%                       2. REINFORCE 求解器函数
%  =====================================================================
function results = solve_model_reinforce(cS)

% --- 1. 创建 Actor 网络 ---
actor = createActorNetwork(cS);

% 为 Adam 优化器初始化一阶和二阶矩估计
actor_m = []; actor_v = [];

% --- 2. 初始化训练历史和实时监控图 ---
fprintf('开始 REINFORCE 训练...\n');
history.episode_rewards = zeros(1, cS.drl.num_episodes);
history.mean_actor_loss = zeros(1, cS.drl.num_episodes);
history.mean_return = zeros(1, cS.drl.num_episodes); % 监控回报
history.mean_c_ratio = zeros(1, cS.drl.num_episodes);
history.mean_alpha = zeros(1, cS.drl.num_episodes);

% 创建图形窗口和子图
fig = figure('Name', 'REINFORCE Training Monitor', 'Position', [100, 100, 1200, 800]);
subplot_handles.rewards = subplot(2, 2, 1);
title('Episode Rewards'); xlabel('Episode'); ylabel('Total Undiscounted Reward'); grid on; hold on;
subplot_handles.loss = subplot(2, 2, 2);
title('Mean Actor Loss'); xlabel('Episode'); ylabel('Loss'); grid on; hold on;
subplot_handles.returns = subplot(2, 2, 3);
title('Mean Episode Return (Normalized)'); xlabel('Episode'); ylabel('Return'); grid on; hold on;
subplot_handles.actions = subplot(2, 2, 4);
title('Mean Actions'); xlabel('Episode'); ylabel('Ratio/Share'); grid on; hold on;

% 初始化绘图句柄
plot_handles.reward_ma = plot(subplot_handles.rewards, NaN, NaN, 'b-', 'LineWidth', 2, 'DisplayName', 'Reward (MA)');
plot_handles.actor_loss = plot(subplot_handles.loss, NaN, NaN, 'g-', 'DisplayName', 'Actor Loss (MA)');
plot_handles.returns = plot(subplot_handles.returns, NaN, NaN, 'k-', 'DisplayName', 'Return (MA)');
...
legend(subplot_handles.actions,'show');
...
for episode = 1:cS.drl.num_episodes
    ...
    for t = 1:cS.tn
        ...
        % 从离散冲击中随机抽样
        u_idx = randi(cS.n_shocks);
        z_idx = randi(cS.n_shocks);
        r_idx = randi(cS.n_shocks);
        
        u_shock_val = cS.yh(u_idx, 1);
        ...
        gpy = 1.0; 
        if t < cS.tn
            yp_val = cS.yp(z_idx, 1); 
            gret_val = cS.gret(r_idx);
            if t < work_periods
                G_t = cS.f_y(t+1) / cS.f_y(t); 
                gpy = G_t * yp_val;
            end
    ...
    % 标准化回报以降低方差 (这是REINFORCE的关键技巧)
    if std(returns) > 1e-6
        normalized_returns = (returns - mean(returns)) / std(returns);
    else
        normalized_returns = returns - mean(returns);
    end
    ...
    [actor_grads, actor_loss] = ...
        dlfeval(@computeReinforceGradients, actor, batch_states, dlarray(normalized_returns, 'CB'), ...
    ...
    % -- C. 记录和更新监控图 --
    history.episode_rewards(episode) = sum(rewards_to_process); % 记录当期未经折现的总奖励
    history.mean_actor_loss(episode) = extractdata(actor_loss);
    history.mean_return(episode) = mean(normalized_returns);
    ...
    if mod(episode, update_freq) == 0 || episode == cS.drl.num_episodes
        ...
        ma_return = movmean(history.mean_return(1:episode), [reward_window-1, 0]);
        ...
        set(plot_handles.returns, 'XData', 1:episode, 'YData', ma_return);
        set(plot_handles.c_ratio, 'XData', 1:episode, 'YData', ma_c_ratio);
        set(plot_handles.alpha, 'XData', 1:episode, 'YData', ma_alpha);
        
        drawnow;
        
        fprintf('Episode: %d | Avg Reward (MA): %.4f | Avg Loss (MA): %.4f\n', ...
            episode, ma_rewards(end), ma_actor_loss(end));
    end
end

fprintf('REINFORCE 训练完成。\n');
results.actor = actor;
end
...
%                  REINFORCE - 网络创建与梯度计算
% =====================================================================
function actor = createActorNetwork(cS)
% 创建 Actor 网络
actor_layers = [
...
function [actor_grads, actor_loss] = computeReinforceGradients(actor, states, returns, action_means, action_samples, exploration_std)
    % REINFORCE loss = - E[log(pi(a|s)) * G_t]
    % G_t 是经过标准化的回报
    
    log_probs = -0.5 * sum(((action_samples - action_means) ./ exploration_std).^2, 1);
    
    actor_loss = -mean(log_probs .* returns);
...
simW_norm = zeros(cS.tn + 1, nsim); 
simC = zeros(cS.tn, nsim);          
simA = zeros(cS.tn, nsim);          
simCash = zeros(cS.tn, nsim);
...
for t = 1:cS.tn
    ...
    simC(t, :) = min(simC(t, :), cash_on_hand * 0.9999); 
...
figure('Position', [100, 100, 1200, 800], 'Name', 'REINFORCE Simulation Results');
...
sgtitle('Lifecycle Simulation Results (REINFORCE)', 'FontSize', 16, 'FontWeight', 'bold');

output_filename = fullfile(cS.save_path, 'drl_simulation_results_reinforce.png');
