...
def main():
    ...
    N_EPISODES = 1000
    DISABLE_RANDOM_DEATH = True # 在有随机死亡的完整环境中测试
    ...
    C_policy_prop = vfi_results['C_policy']
    A_policy = vfi_results['A_policy']
    norm_cash_grid = cS['gcash'].flatten()
    
    vfi_policies = {'c_prop': [], 'a': []}
    tn = C_policy_prop.shape[1]
    for t in range(tn):
        c_interp = interp1d(norm_cash_grid, C_policy_prop[:, t], kind='cubic', bounds_error=False, fill_value="extrapolate")
        a_interp = interp1d(norm_cash_grid, A_policy[:, t], kind='cubic', bounds_error=False, fill_value="extrapolate")
        vfi_policies['c_prop'].append(c_interp)
    ...
    for i_episode in range(N_EPISODES):
        ...
        while not is_done:
            ...
            c_prop_t = vfi_policies['c_prop'][t_idx](norm_cash_on_hand_t)
