    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0 and self.num_timesteps>self.model.learning_starts:
            ...
            if mean_reward >= self.best_mean_reward:
                ...
                plt.plot(mean_actions[:,0], mean_actions[:,3], label='basic_pension')  
                plt.plot(mean_actions[:,0], mean_actions[:,4], label='tda_pct')  
                plt.plot(mean_actions[:,0], mean_actions[:,5], label='riksy_tda')  
