...
def run_simulation(raw_env, n_sim, policy_type, rl_model=None, norm_env=None, vfi_policies=None):
    ...
    for i in range(n_sim):
        ...
        while not is_done:
            ...
            trajectory['c_prop'].append(info['absolute_consumption'] / (wealth_t + income_t + 1e-8))
...
def evaluate_and_compare(args):
    ...
    x_grid = cS['x_grid'].flatten()
    
    vfi_policies = {'c': [], 'a': []}
    tn = C_policy.shape[1]
    for t in range(tn):
        c_interp = interp1d(x_grid, C_policy[:, t], kind='linear', bounds_error=False, fill_value="extrapolate")
        a_interp = interp1d(x_grid, A_policy[:, t], kind='linear', bounds_error=False, fill_value="extrapolate")
...
class Args:
    def __init__(self):
        self.rl_model = "models/cocco_sac_curriculum/cocco_sac_run_4/best_model/best_model.zip"
        self.stats = "models/cocco_sac_curriculum/cocco_sac_run_4/best_model/vecnormalize_best.pkl"
