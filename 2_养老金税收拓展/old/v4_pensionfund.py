...
register(
    id='pensionfund-v4',                                # call it whatever you want
    entry_point='v4_pensionfund:PensionFundEnv', # module_name:class_name
)
  

#         # 根据邹建文和杨继生（2024）表1
#         # 25-60岁 城镇居民收入：mean:5.2603 std:4.575
...
class PensionFundEnv(gym.Env):
    ...
    def __init__(self, params): # 
        ...
        # 参照邹建文、杨继生(2024)表3城镇列
        # 临时收入冲击 
        self.py = 0.370 # 混合正态分布1
        self.smay_1 = np.sqrt(1.364) # 混合正态分布1的标准差
        self.smay_2 = np.sqrt(0.0002)  # 混合正态分布2的标准差
        # 永久收入冲击
        self.pv = 0.286 # 混合正态分布1
        self.muv_1 = -0.335 # 混合正态分布1的均值
        self.smav_1 = np.sqrt(0.0603)   # 混合正态分布1的标准差
        self.smav_2 = np.sqrt(0.0609)  # 混合正态分布2的标准差
        self.rho = 0.801 # 永久收入冲击的AR

        # 初始值
        self.smav_0 = 0.0740 # 持续收入冲击
        self.sa_0 = 0 # 个体效应,暂时设为0

        self.aa = (-2.170042 + 2.700381)+7.44
    ...
    0.82935, 0.81282, 0.79485, 0.77543, 0.75458, 0.7324, 0.70893, 0.68424]
        ...
        self.action_space = spaces.Box(low=np.array([0.05, -0.2, -0.2, -0.2]),\
                                       high=np.array([1.5, 1.5, 1.5, 1.5]), shape=(4,), dtype=float)
...
# === 状态转移方程（退休前） ===       
        if self.age < self.tr:
            ...
            # 收入过程，混合正态分布
            y = np.random.binomial(1,self.py,1)[0] # 临时收入冲击属于混合正态分布1的概率           
            u_t = np.random.normal(0, self.smay_1)*y + np.random.normal(0, self.smay_2) * (1-y)  

            v = np.random.binomial(1,self.pv,1)[0] # 永久收入冲击属于哪个混合正态分布
            muv_2 = -self.pv*self.muv_1/(1-self.pv) # 邹建文、杨继生（2024）：令self.pv*self.muv_1 ＋(1-self.pv)*self.muv_2＝０， 由此只需估计muv_1就可以
            z_t = np.random.normal(self.muv_1, self.smav_1)*v  + np.random.normal(muv_2, self.smav_2) * (1-v)  


            new_perm_shock = self.rho * perm_shock + z_t # 工资持续性冲击
            ...
            log_rand_wage = self.f(self.age+1) + new_perm_shock + self.ind_a +  u_t   
            new_wage = np.exp(log_rand_wage) # 下一期工资
            if np.isnan(new_wage):
                raise
            # 养老金风险收益
            # 生成独立的标准正态随机变量   
...
# === 状态转移方程（退休后） ===   
            ...
            log_rand_wage = self.last_perm_shock + self.last_f + self.ind_a + np.log(self.ret_fac)   # ret_fac是退休后固定支付的工资,为最后一期工作期工资的比例
            new_wage = np.exp(log_rand_wage) # 下一期工资
            ...
            info = {'age':np.array([self.age]),'basic_income':self.wage,\
                    ...
                    'normalized_cash':(new_cash)/np.exp(self.last_perm_shock + self.last_f + self.ind_a),\
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        # 第一份随机工资收入
        z_t = np.random.normal(0, self.smav_0)
        self.ind_a = np.random.normal(0, self.sa_0) # 个体效应
        perm_shock = z_t # 工资持续性冲击
        # perm_shock = max(z_t,0) # 工资持续性冲击
        
        log_rand_wage = self.f(self.age) + perm_shock + self.ind_a 
        ...
        info = {'age':self.age,'basic_income':self.wage,\
                          'state':self.state,
