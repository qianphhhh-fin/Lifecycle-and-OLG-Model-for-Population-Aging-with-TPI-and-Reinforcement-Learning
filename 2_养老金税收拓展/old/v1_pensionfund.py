...
register(
    id='pensionfund-v1',                                # call it whatever you want
    entry_point='v1_pensionfund:PensionFundEnv', # module_name:class_name
)
...
class PensionFundEnv(gym.Env):
    ...
    def __init__(self, params): # 
        ...
        self.init_cash = 0.2  # 初始现金

        self.tda_tax = 0.03 # 个人养老金税率
        self.fund_excess_return = 0.04  # 养老金风险投资超额收益
        self.fund_riskfree_return = 0.02  # 养老金无风险收益
        self.tau = 0.1 # 工资中扣除的基本养老金比例
    ...
    0.82935, 0.81282, 0.79485, 0.77543, 0.75458, 0.7324, 0.70893, 0.68424]
        ...
        # ===================== 状态空间：=====================
        # 现金余额，工资，永久收入冲击, 个人养老金余额，年龄
        self.observation_space = spaces.Box(
            low=np.array([0, 0, -1e3, 0, self.tb]),
            high=np.array([1e3, 1e3, 1e3, 1e3, self.td]),
            shape=(5,),
            dtype=float
        )
    ...
    def step(self, action):
        ...
        cash,wage, perm_shock,pen_balance,_ = self.state
...
# === 状态转移方程（退休前） ===       
        if self.age < self.tr:
            ...
            # 下一期到手现金
            # 在缴纳阶段tda免税          
            tax = self.taxation(wage) # 当期应纳税额
            tax_refund = self.taxation(wage) - self.taxation(wage - pen_amount) # 购买养老金的节税额
            # if tax>0:
            #     raise
            cash = cash + wage - tax + tax_refund - pen_amount # 可用现金
            # if cash<=0:
            #     raise
            ...
            info = {'age':np.array([self.age]),'basic_income':wage,\
                    ...
                    'normalized_cash':(new_cash + new_wage - self.taxation(new_wage))/np.exp(self.f(self.age+1) + new_perm_shock),\
...
# === 状态转移方程（退休后） ===   
            ...
            info = {'age':np.array([self.age]),'basic_income':wage,\
                    ...
                    'normalized_cash':(new_cash+new_wage)/np.exp(self.last_perm_shock + self.last_f),\
        ...
        self.state[1] = new_wage
        self.state[2] = new_perm_shock
        self.state[3] = new_pen_balance  
        self.state[4] = self.age 
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        # self.state['cash'] = np.array([self.int_cash],dtype=float)
        # self.wage = np.array([rand_wage],dtype=float) # 第一份工资
        # self.state['perm_shock'] = np.array([perm_shock],dtype=float)
        # self.state['pension_balance'] = np.array([0],dtype=float) 
        # self.state['norm_age'] = np.array([-1],dtype=float)
        self.state[0] = np.array([new_cash],dtype=float)
        self.state[1] = np.array([new_wage],dtype=float)
        self.state[2] = np.array([perm_shock],dtype=float)
        self.state[3] = np.array([0],dtype=float) 
        self.state[4] = np.array([self.age],dtype=float)
        
        # self.state = np.array([rand_wage,perm_shock,-1],dtype=np.float32)
        info = {'age':self.age,'basic_income':new_wage,\
                          'state':self.state,
                          'normalized_cash':(new_cash + new_wage-self.taxation(new_wage))/np.exp(self.f(self.age) + perm_shock),
                          'status':'born'}
...
if __name__=="__main__":
    ...
    env = gym.make('pensionfund-v1', params=params)
