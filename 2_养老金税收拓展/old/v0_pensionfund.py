...
class PensionFundEnv(gym.Env):
    ...
    0.82935, 0.81282, 0.79485, 0.77543, 0.75458, 0.7324, 0.70893, 0.68424]
        ...
        self.observation_space = spaces.Box(
            low=np.array([0, -1e3, 0, self.tb]),
            high=np.array([1e3, 1e3, 1e3, self.td]),
            shape=(4,),
            dtype=float
        )
...
# === 状态转移方程（退休前） ===       
        if self.age < self.tr:
            ...
            tda_pct = min(tda_pct, (self.pension_limit/cash))
            ...
            # 下一期到手现金
            # 在缴纳阶段tda免税          
            tax = self.taxation(self.wage) # 下期应纳税额
            tax_refund = self.taxation(previous_wage) - self.taxation(previous_wage - pen_amount) # 购买养老金的节税额
            # if tax>0:
            #     raise
...
# === 状态转移方程（退休后） ===   
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        self.state[0] = np.array([cash],dtype=float)
        self.state[1] = np.array([perm_shock],dtype=float)
        self.state[2] = np.array([0],dtype=float) 
        self.state[3] = np.array([self.age],dtype=float)
        
        # self.state = np.array([rand_wage,perm_shock,-1],dtype=np.float32)
        info = {'age':self.age,'basic_income':rand_wage,\
                          'state':self.state,
                          'normalized_cash':cash/np.exp(self.f(self.age) + perm_shock),
                          'status':'born'}
