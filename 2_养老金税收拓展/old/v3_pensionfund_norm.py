...
class PensionFundEnv(gym.Env):
    ...
    def __init__(self): # 
        ...
        super(PensionFundEnv, self).__init__()
         ...
         # 设置默认参数
        self.scale_inc = 2
        self.reward_lb = -0.5 # 单期reward的下限
        self.scale_reward = 0.5
...
# === 状态转移方程（退休后） ===   
        ...
        # print(reward)
        if (reward <self.reward_lb):
            reward = self.reward_lb     
        ...
        if (self.age > self.td):
            ...
            return self.normalize_obs(self.state), reward, self.done, False, info
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        return self.normalize_obs(self.state), info
    ...
    def normalize_obs(self,obs):
        # obs_rms_mean = np.array([4.543013847370008, -0.00011678215522098111, 0.8264708139469078,0]) # [5.932814062561221, 0.001149662208315307, 19.927408263970737, -0.01624865359787772]
        # obs_rms_var = np.array([2.32940339498445, 0.04631466511138381, 6.536882534081683, 1]) # [13.412597988834666, 0.15746419961010705, 1187.7825988146735, 0.08285733411645112]
        obs_rms_mean = np.array([0,0,0,0])
        obs_rms_var = np.array([1,1,1,1])
        epsilon = 1e-06
        return (obs - obs_rms_mean) / np.sqrt(obs_rms_var)
...
if __name__=="__main__":
    ...
    env = gym.make('pensionfund-v3-norm')
    ...
    # 存储所有agent的轨迹
    # 初始化一个空列表用于存储所有观测值
    all_observations = np.zeros((1, env.observation_space.shape[0]))  
    ...
    for agent in range(n_agents):
        # 重置环境获取初始状态
        obs, info = env.reset()
        # 初始化当前agent的观测值数组，形状为(1, obs_dim)
        agent_obs = np.zeros((1, len(obs)))  
        agent_obs[0] = obs  # 存储初始观测值
        ...
        while not done:
            ...
            # 将新观测值添加到当前agent的观测值数组中
            agent_obs = np.vstack((agent_obs, obs.reshape(1, -1)))
        
        # 将当前agent的观测值数组添加到总列表中
        all_observations = np.vstack((all_observations,agent_obs))
        pbar.update(1)
    ...
    obs_var = np.var(all_observations, axis=0)
    
    # 打印每列的统计信息
    print("观测值各列统计信息：")
    for i, (mean, var) in enumerate(zip(obs_mean, obs_var)):
        print(f"第{i}列 - 均值: {mean:.4f}, 方差: {var:.4f}")
    
    ''' 结果
    第0列 - 均值: 9.9867, 方差: 48.3938
    第1列 - 均值: 0.0042, 方差: 0.1587
    第2列 - 均值: 84.7792, 方差: 2912.9999
    第3列 - 均值: -0.0046, 方差: 0.0864
    '''
