...
def run(penlimit,env_id):
    ...
    # params = {'pension_limit': 1,'sigma_eta':0.27}
    
    vec_env = make_vec_env("pensionfund-v5-rural",seed=1, n_envs=88,  env_kwargs={'params':None}, monitor_kwargs={'discount_factor': distf})
    #状态变量：财富，上期工资持续冲击，基本养老金个人账户余额, 城乡保缴费年限, 个人养老金账户余额，年龄，
    vec_env = VecNormalize(vec_env, norm_obs=True, norm_reward=True,norm_obs_index_mask=[3,5]) #  新定义env , norm_obs_index_mask=[3]
    ...
    eval_env = make_vec_env("pensionfund-v5-rural",seed=3687851522, n_envs=1, env_kwargs={'params':None},\
...
def parse_args(args=None):
    ...
    # 通用参数:
    parser.add_argument('--envid', default="pensionfund-v5-rural", type=str, metavar='ENVID', help="环境 (默认: %(default)s)")
    parser.add_argument('--penlimit', default=1.0, type=float, metavar='PENLIMIT', help="养老金购买上限 (默认: %(default)s)")
