...
class VecNormalize(VecEnvWrapper):
    ...
    def __init__(
        ...
        norm_obs_index_mask: Optional[list[str]] = None,
    ):
        VecEnvWrapper.__init__(self, venv)

        self.norm_obs = norm_obs
        self.norm_obs_keys = norm_obs_keys
        self.norm_obs_index_mask = norm_obs_index_mask
    ...
    def _normalize_obs(self, obs: np.ndarray, obs_rms: RunningMeanStd) -> np.ndarray:
        ...
        """
        result = np.clip((obs - obs_rms.mean) / np.sqrt(obs_rms.var + self.epsilon), -self.clip_obs, self.clip_obs)
        if self.norm_obs_index_mask != None:
            if len(result.shape) == 1:
                result[self.norm_obs_index_mask] = obs[self.norm_obs_index_mask] # 不需要normalize的维度
            else:
                result[:,self.norm_obs_index_mask] = obs[:,self.norm_obs_index_mask] # 不需要normalize的维度
        return result
    ...
    def _unnormalize_obs(self, obs: np.ndarray, obs_rms: RunningMeanStd) -> np.ndarray:
        ...
        """
        result = (obs * np.sqrt(obs_rms.var + self.epsilon)) + obs_rms.mean
        if self.norm_obs_index_mask != None:
            result[:,self.norm_obs_index_mask] = obs[:,self.norm_obs_index_mask] # 不需要normalize的维度 
        return result
