...
classdef distribution
    methods (Static)
        ...
        function Dist = get_ss_dist_no_pps(polS, paramS, cS, Z_ss_norm, bequest_per_capita)
            % =========================================================================
            ...
            if mass_newborns > 1e-9
                beq_per_newborn = bequest_per_capita * sum(Z_ss_norm) / mass_newborns;
            else
                beq_per_newborn = 0;
            end
        ...
        function Dist_path = simulate_dist_forward(dist0, Pol_path, cS, paramS, bequest_per_newborn_hat_path)
            % =========================================================================
            % == 函数: simulate_dist_forward (v4.0 - 新生儿会计与循环修正版)
            % == 核心修正:
