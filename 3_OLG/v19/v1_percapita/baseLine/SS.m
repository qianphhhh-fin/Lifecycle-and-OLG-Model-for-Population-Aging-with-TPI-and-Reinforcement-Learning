...
classdef SS
    methods (Static)
        ...
        function [F_error, ss, Dist, polS, valS] = system_of_equations(x_guess, Z_ss_norm, cS, paramS, params_ext)
            ...
            % == 版本: [v1.7 - 生产与遗赠会计双重修正版]
            % ==
            % == 核心修改:
            % ==   - [!!! 关键BUG修复 !!!] 修正了宏观计算流程。现在使用家庭供给的
            % ==     劳动 (L_supply_agg)，而不是虚拟的L_required_for_PAYG，来
            % ==     计算总产出(Y)和价格(r, w)。这是恢复宏观一致性的关键。
            % ==   - [!!! 遗赠会计修正 !!!] 在计算遗赠市场出清误差 (error_bq) 时，
            % ==     将从微观聚合得到的原始遗赠总量 (Bequest_generated_agg)
            % ==     手动除以 (1+n_period) 以进行正确的BGP调整。
            % ==   - [!!! 劳动市场出清修正 !!!] 劳动市场的出清条件现在直接
            % ==     比较家庭供给的劳动 (L_supply_agg) 和厂商在均衡价格下
            % ==     需求（或使用）的劳动（也是 L_supply_agg）。
            % =========================================================================
            r_guess = x_guess(1);
            w_guess = x_guess(2);
            bq_guess = x_guess(3); % 这是人均、BGP调整后的遗赠
            tr_guess = x_guess(4); % 这是人均转移支付
            b_hat_guess = x_guess(5); % 这是人均养老金

            % --- 流程 1: 微观聚合 ---
            % 基于猜测的价格、转移支付和养老金，计算家庭的最优决策和加总的宏观供给量
            ...
            L_supply_agg = macro_state.L_hat; % 这是家庭部门供给的总劳动
            K_supply_agg = macro_state.K_private_hat; % 这是家庭部门供给的总资本
            ...
            % [!!! 核心修正: 使用家庭供给的劳动 L_supply_agg 来计算生产 !!!]
            % 首先，计算公共资本存量 K_g
            Kg_Y_ratio_factor = cS.I_g_to_Y_ratio_ss / (g_total_period + cS.ddk_g);
            % 联立 Y = A * Kp^alpha * Kg^gamma * L^(1-alpha-gamma) 和 Kg = factor * Y 求解
            ...
            % 基于供给的资本和劳动，以及计算出的公共资本，得到厂商引致的价格
            ...
            % [!!! 核心修正: 对聚合的原始总量进行BGP调整 !!!]
            % macro_state.Bequest_generated_agg 是未经调整的期末原始总量
            bequest_generated_adj = macro_state.Bequest_generated_agg / (1 + n_period);
            bq_new = bequest_generated_adj / mass_total; % 计算出新的人均、BGP调整后遗赠
            ...
            total_pension_contrib_hat = cS.theta_path(end) * w_guess * L_supply_agg; % 使用猜测的w，因为这是家庭的预期
            error_b_hat = total_pension_contrib_hat - total_pension_paid_hat; % PAYG收支平衡
            ...
            if nargout > 1
                ...
                % [!!! 会计接口修正 !!!]
                % Bequest_generated_agg 存储 BGP调整后的总量
                ss.Bequest_generated_agg = bequest_generated_adj; 
                % Bequest_distributed_agg 是同义词，也存调整后的量
                ss.Bequest_distributed_agg = bequest_generated_adj; 
                % 为转轨路径求解器，额外存储【未经】调整的原始总量
                ...
                ss.b_hat = b_hat_guess; % 存储均衡的养老金水平
            end
        end        
        ...
        function display_national_accounts(ss, cS, paramS, Z_ss_norm, report_filename)
            ...
            % ==   - [!!!] 在报告最终均衡误差时，将 ss 中存储的总量(遗赠、转移支付)
            % ==     转换回人均量，以匹配 system_of_equations 的输入要求。
            % =========================================================================
            ...
            for ia = 1:cS.aD_new
                ...
                % 注意: payg_contrib_grid 仅对工作年龄有效，但为简化代码，此处对所有年龄计算
                % 由于退休后 labor_income 为0, payg_contrib_grid 也为0，不影响结果
            ...
            if pps_active
                fprintfAndLog(fileID, '###                 稳 态 国 民 经 济 核 算 报 告 (v8.0 - 带PPS, PAYG分离版)              ###\n');
            else
                fprintfAndLog(fileID, '###                       稳 态 国 民 经 济 核 算 报 告 (v8.0 - PAYG分离版)                      ###\n');
            end
            fprintfAndLog(fileID, '=========================================================================================================\n');
