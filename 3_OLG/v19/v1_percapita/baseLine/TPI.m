...
classdef TPI
    methods (Static)
        ...
        function [target_paths, errors, Pol_path, Dist_path, aggr_supply] = calculate_paths_and_errors(r_path, w_hat_path, bequest_hat_pc_path, TR_hat_pc_path, b_hat_path, ss0, ssF, cS, paramSF, valF, polF, dist0)
            % =========================================================================
            % ==          TPI核心计算引擎 (v6.0 - 遗赠会计完全重构版)
            % ==
            % == 核心修改:
            % ==  - [!!! 跨期会计修正 !!!] 彻底修正了遗赠的跨期和总量/人均会计逻辑。
            % ==  - 1. 计算【每个新生儿收到的资产路径】(bequest_per_newborn_hat_path):
            % ==       - t=1 的新生儿收到来自稳态 ss0 的遗赠。使用 ss0 中新增的
            % ==         `Bequest_gen_hat_raw_ss` (未经BGP调整的原始总量) 来确保无缝衔接。
            % ==       - t>1 的新生儿收到来自 t-1 期末产生的遗赠总量。
            % ==  - 2. 将此新生儿资产路径传递给修正后的 `simulate_dist_forward`。
            % ==  - 3. 构造【新的目标遗赠路径】(target_paths.bequest_hat_pc_path):
            % ==       - t>1 的目标人均遗赠，由 t-1 期末产生的【人均】遗赠供给决定。
            % ==  - 4. 计算【误差】时，将 t>1 的目标路径与 t>1 的猜测路径进行比较。
            % =========================================================================
            T = cS.T_sim;
            ...
            % t=1的新生儿，接收来自初始稳态ss0的遗赠
            % 使用未经BGP调整的原始总量，确保会计一致
            if mass_newborns_path(1) > 1e-9
                bequest_per_newborn_hat_path(1) = ss0.Bequest_gen_hat_raw_ss / mass_newborns_path(1);
            else
                bequest_per_newborn_hat_path(1) = 0;
            end
            
            % 在这里，我们预先计算 t>1 时新生儿的资产。这需要一个临时的、
            % 基于当前猜测的遗赠总量路径。
            total_pop_path = sum(cS.Z_path_raw, 1);
            Bequest_hat_total_path_guess = bequest_hat_pc_path .* total_pop_path; % 总量 = 人均 * 总人口
            if T > 1
                 bequest_per_newborn_hat_path(2:T) = Bequest_hat_total_path_guess(1:T-1) ./ mass_newborns_path(2:T);
            end
            ...
            % [!!! 核心修正: 构造新的目标遗赠路径 !!!]
            target_paths.bequest_hat_pc_path = zeros(1, T);
            target_paths.bequest_hat_pc_path(1) = bequest_hat_pc_path(1); % t=1的目标保持不变
            if T > 1
                % t>1的目标，由t-1的供给决定
                target_paths.bequest_hat_pc_path(2:T) = aggr_supply.Bequest_gen_hat_pc_path(1:T-1);
            end
            ...
            errors.tr = max(abs(target_paths.TR_hat_pc_path(2:T) - TR_hat_pc_path(2:T)));
            
            % [!!! 核心修正: 比较 t>1 的目标和猜测 !!!]
            errors.beq = max(abs(target_paths.bequest_hat_pc_path(2:T) - bequest_hat_pc_path(2:T)));
