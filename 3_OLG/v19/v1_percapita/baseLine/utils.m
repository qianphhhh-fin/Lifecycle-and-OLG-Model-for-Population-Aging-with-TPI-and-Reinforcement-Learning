classdef utils
    methods (Static)
        ...
        function [Z_out,Z_out_raw, A_path, cS] = generate_exo_paths(cS, graph_flag)
            ...
            % == 版本: [v_BGP_consistent_sim_v2 - 人口恒等式修正版]
            % ==
            % == 核心修改:
            % ==   - [!!! 关键逻辑修正 !!!] 修正了人口前向模拟的核心恒等式。
            % ==   - 旧方法使用 Newborns(t+1) = Pop(t) * CBR(t) 的近似，这在
            % ==     逻辑上不严谨，混淆了粗出生率和动态出生率。
            % ==   - 新方法使用精确的人口恒等式: Pop(t+1) = Survivors(t+1) / (1-CBR(t+1))
            % ==     这确保了给定CBR数据，模拟出的人口路径在每个时间点都严格满足
            % ==     人口平衡方程，保证了内部的逻辑一致性。
            % =========================================================================

            fprintf('\n--- 启动外生路径生成器 (v_BGP_consistent_sim_v2) ---\n');
            ...
            for t = 1:max_sim_periods
                ...
                if t < max_sim_periods
                    ...
                    if ~any(annual_indices)
                        cbr_target_period = birth_rate_path_annual_full(end); % 超出范围则用最后一个值
                    else
                        cbr_target_period = mean(birth_rate_path_annual_full(annual_indices));
                    end

                    % 每个时期内的总出生人口，应该等于该时期总人口乘以该时期的出生率。
                    % 我们近似认为一个时期(time_Step年)的总出生率是年出生率的time_Step倍
                    cbr_target_model_period = cbr_target_period * cS.time_Step;

                    % c. 使用核心恒等式计算下一期的总人口
                    % Pop(t+1) = Survivors(t+1) / (1 - CBR(t+1))
