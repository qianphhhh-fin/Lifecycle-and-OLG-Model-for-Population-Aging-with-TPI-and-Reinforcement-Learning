...
% == 版本: [v2.5 - dist0 构造根本性修正版]
% ==
% == 目的:
% ==   - [!!! 关键BUG修复 !!!] 彻底修正了构造初始绝对分布 (dist0_abs)
% ==     的会计逻辑。
% ==   - 旧逻辑错误地将一个基于旧人口结构的分布乘以新的人口向量。
% ==   - 新逻辑首先通过除以旧的人口分布(Z0_norm)来提取条件分布P(k,e|a)，
% ==     然后再乘以新的人口绝对数量(Z_path_raw(:,1))，确保了会计的
% ==     严格一致性。这是修复TPI投资缺口的关键。
% =========================================================================
clear; close all; clear classes;
addpath(pwd);
fprintf('=== 受控人口动态稳态求解脚本 (v2.5 - dist0 构造根本性修正版) ===\n\n');

%% --- 1. 实验设置与参数初始化 ---
fprintf('--- 1. 实验设置与参数初始化 ---\n');
output_filename = 'SS/data_for_diff_transition.mat';
max_sim_periods = 500; % 延长模拟期数以确保财富分布收敛
convergence_tol = 1e-10; % 收敛容忍度
...
% [!!! 核心会计逻辑修正 !!!]
% 1. 获取初始稳态的【条件分布】P(k, e | a)。
%    这通过将初始稳态的绝对分布(dist0_norm)除以其对应的人口结构(Z0_norm)来实现。
%    为避免除以零，对Z0_norm中的零值进行处理。
Z0_norm_reshaped = reshape(Z0_norm, [1,1,1,cS.aD_new]);
Z0_norm_safe = Z0_norm_reshaped;
Z0_norm_safe(Z0_norm_safe < 1e-12) = 1; % 将零或极小的人口组替换为1，因为dist0_norm中对应项也为0
dist_cond_ss0 = dist0_norm ./ Z0_norm_safe;

% 2. 将这个条件分布乘以转轨期第一期(t=1)的【绝对人口数量】，得到t=1期初的绝对分布。
