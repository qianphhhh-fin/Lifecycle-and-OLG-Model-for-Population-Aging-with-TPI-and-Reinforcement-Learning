...
function [val_age, pol_age] = VFI_by_age_PPS(a_idx, vPrime_kkppse_next, M_age, b_age_val, paramS_age, cS)
    % =========================================================================
    % == 函数: VFI_by_age_PPS (版本 v1.9 - 外生劳动记录版)
    % == 核心修正:
    % ==   - [!!!] 新增 'l' 字段用于记录外生劳动供给，以供聚合函数使用。
    % ==   - 移除了所有关于劳动负效用的计算。
    % =========================================================================
    nk_search_grid = 150;
    ...
    parfor task_idx = 1:num_tasks
        ...
        [~, util_c_mat] = utils.CES_utility(c_choices_mat, cS.sigma, cS); % [!!!] 移除了劳动负效用
    ...
    % =========================================================================
    % == 函数: VFI_by_age (版本 v2.2 - 外生劳动记录版)
    % == 核心修正:
    % ==   - [!!!] 新增 'l' 字段用于记录外生劳动供给，以供聚合函数使用。
    % ==   - 移除了所有关于劳动负效用的计算，因为劳动是外生的。
    % =========================================================================
    nk_search_grid = 150;
