...
classdef SS
    methods (Static)
        function [ss, Dist, polS, valS] = solve_steady_state(cS, paramS, params_ext, verbose)
            % =========================================================================
            ...
            options = optimoptions('lsqnonlin', 'Display', verbose, 'FunctionTolerance', 1e-12, 'StepTolerance', 1e-12, 'MaxIterations', 200);
...
% --- SS.m ---
        function [F_error, ss, Dist, polS, valS] = system_of_equations(x_guess, Z_ss_norm, cS, paramS, params_ext)
            ...
            % ==
            % == 核心修改:
            % ==   - 引入了更直接、更稳健的资本市场出清条件，与TPI的聚合口径对齐。
            % ==   - 清晰地区分 K_supply_t (来自聚合分布) 和 K_supply_tplus1_raw (来自聚合策略)。
            % ==   - 误差项 error_K 直接反映了 K_t = K_{t+1}_raw / (1+g) 是否成立。
            % ==   - ss结构体中的资本 K_private_hat 被明确定义为当期资本供给 K_t。
            % =========================================================================
            r_guess = x_guess(1);
            w_guess = x_guess(2);
            bq_guess = x_guess(3);
            ...
            if nargout > 1
                ...
                ss.K_private_hat = K_supply_agg_t; % [!!!] 核心：使用来自当期分布的资本
