...
function [target_paths, errors, Pol_path, Dist_path, aggr_supply] = calculate_paths_and_errors(r_path, w_hat_path, Bequest_hat_total_path, TR_hat_pc_path, b_hat_path, ss0, ssF, cS, paramSF, valF, polF, dist0)
            % =========================================================================
            % ==          TPI核心计算引擎 (v5.7 - 增强接口最终版)
            % ==
            % == 核心修改:
            % ==  - 直接使用由增强版稳态求解器ss0提供的、会计口径完全一致的
            % ==    “期末原始总量” (`ss0.Bequest_gen_hat_raw_ss`) 来计算
            % ==    t=1期新生儿的初始财富，消除了所有手动调整。
            % =========================================================================
            T = cS.T_sim;
            ...
            % [!!! 核心修正: 直接使用增强接口提供的原始总量 !!!]
            bequest_per_newborn_hat_path(1) = ss0.Bequest_gen_hat_raw_ss / mass_newborns_path(1);
            
            if T > 1
                 bequest_per_newborn_hat_path(2:T) = Bequest_hat_total_path(1:T-1) ./ mass_newborns_path(2:T);
            end
            ...
            total_bequest_gen_from_sim_raw = aggr_supply.Bequest_gen_hat_pc_path .* total_pop_path;
            target_paths.Bequest_hat_total_path = total_bequest_gen_from_sim_raw;
            ...
            norm_factor = max(abs(Bequest_hat_total_path(2:T)));
            if norm_factor < 1e-9, norm_factor = 1; end
            errors.beq = max(abs(target_paths.Bequest_hat_total_path(2:T) - Bequest_hat_total_path(2:T))) / norm_factor;
