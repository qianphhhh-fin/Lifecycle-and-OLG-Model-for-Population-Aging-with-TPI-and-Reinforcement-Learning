classdef utils
    methods (Static)
        ...
        function cS = ParameterValues()
            ...
            cS.cbr_last_year = 2023;            % 联合国出生率数据使用的最后年份
            ...
            cS = utils.generate_mortality_path(cS); % 生成死亡率数据
...
function [Z_out,Z_out_raw, A_path, cS] = generate_exo_paths(cS, graph_flag)
            ...
            % == 版本: [v_BGP_consistent_sim - BGP逻辑一致版]
            % ==
            % == 核心修改:
            % ==   1. [!!!] 修正了人口前向模拟的核心逻辑，使其与BGP定义严格一致。
            % ==   2. 废除了“新生儿=总人口*出生率”的静态近似，改为使用
            % ==      “总人口(t+1) = 新生儿(t+1) + 幸存者(t+1)”的动态恒等式。
            % ==   3. 新生儿数量根据给定的出生率数据计算，而总人口路径和增长率
            % ==      则由模拟内生决定，确保了整个路径的内部自洽性。
            % =========================================================================

            fprintf('\n--- 启动外生路径生成器 (v_BGP_consistent_sim) ---\n');
            ...
            initial_pop_aggregated = utils.aggregate_population(initial_pop_row, num_age_groups_all, cS.time_Step);
            ...
            for t = 1:max_sim_periods
                ...
                if current_year <= cS.mortality_years(end)
                    raw_survival_data_t = 1 - interp1(cS.mortality_years, cS.mortality_path_data, current_year, 'linear', 'extrap');
                else
                    raw_survival_data_t = 1 - cS.mortality_path_data(end,:); 
                end
                raw_survival_data_t(end) = 0; 
                ...
                if t < max_sim_periods
                    ...
                    % a. 计算下一期的幸存者 (绝对数量)
                    survivors_tp1 = Z_path_raw(1:end-1, t) .* s_path_all_sim(1:end-1, t);
                    
                    % b. 根据当期总人口和出生率，计算下一期的新生儿 (绝对数量)
                    total_pop_t = sum(Z_path_raw(:, t));
                    period_start_idx_in_annual = (t-1)*cS.time_Step + 1;
                    period_end_idx_in_annual = t*cS.time_Step;
                    
                    % 确保我们不会超出预先计算的年度出生率路径
                    if period_end_idx_in_annual > length(birth_rate_path_annual_full)
                        avg_birth_rate_period = birth_rate_path_annual_full(end);
                    else
                        avg_birth_rate_period = mean(birth_rate_path_annual_full(period_start_idx_in_annual:period_end_idx_in_annual));
                    end
                    
                    % 新生儿总数是一个时期内(time_Step年)积累的结果
                    new_entrants_tp1 = total_pop_t * avg_birth_rate_period * cS.time_Step;
            ...
            A_path = utils.generate_tfp_path(cS, annual_years_vec, model_sim_years);
            ...
            if graph_flag
                ...
                utils.create_exogenous_dynamics_plots(cS, Z_out, A_path_annual_for_plot, ...
        ...
        function cS = calcaulte_theta_payg_path(cS, graph_flag)
            ...
            % --- 1. 定义关键参数的【起点】与【终点】 ---
            theta_urban_initial = 0.24;   % 初始值: 基于2023年 16%+8% 的法定费率
        ...
        function Z_theory = compute_theoretical_ss_dist(s_pathV_ss, n_ss_annual, time_Step, aD_new)
