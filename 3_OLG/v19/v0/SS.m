...
function [F_error, ss, Dist, polS, valS] = system_of_equations(x_guess, Z_ss_norm, cS, paramS, params_ext)
            ...
            % ==
            % == 核心修改:
            % ==   - 在函数末尾、打包最终ss结构体时，新增了一个模块。
            % ==   - 该模块手动地、从最终的稳态分布中，重新聚合出【未经BGP调整】
            % ==     的“期末原始总量”（如 K_p_hat_eop_raw）。
            % ==   - 这些新变量被附加到ss结构体中，专门供转轨路径求解器使用，
            % ==     以确保从稳态到转轨的会计口径完全一致，同时不影响稳态
            % ==     自身的求解和市场出清验证。
            % =========================================================================
            r_guess = x_guess(1);
            w_guess = x_guess(2);
            ...
            L_required_for_PAYG = total_pension_paid_hat / (cS.theta_path(end) * w_guess);
            ...
            term2 = L_required_for_PAYG^(1 - cS.alpha - cS.gamma);
            term3 = Kg_Y_ratio_factor^cS.gamma;
            Y_hat_calc = (term1 * term2 * term3)^(1 / (1 - cS.gamma));
            K_g_hat_calc = Kg_Y_ratio_factor * Y_hat_calc;

            prices_new = firm.get_prices_at_t(macro_state.K_private_hat, K_g_hat_calc, L_required_for_PAYG, cS);
            r_new = prices_new.r_mkt_t;
            w_new = prices_new.w_hat_t;

            BQ_new = macro_state.Bequest_generated_agg;
            
            PAYG_contributions_agg = cS.theta_path(end) * w_new * L_required_for_PAYG;
            General_Tax_Revenue_calc = macro_state.Total_Tax_Revenue - PAYG_contributions_agg;
            Public_Capital_Return_calc = prices_new.Y_hat_t - (w_new * L_required_for_PAYG) - ((r_new + cS.ddk) * macro_state.K_private_hat);
            ...
            error_L = L_required_for_PAYG - L_supply_agg;

            F_error = [error_r; error_w; error_BQ; error_TR; error_L];
            ...
            if nargout > 1
                ...
                for ia = 1:cS.aD_new
                    ...
                    if cS.pps_active, k_prime_slice = k_prime_slice + polS(ia).kpps_prime; end
                    Kp_eop_raw = Kp_eop_raw + sum(k_prime_slice .* mass_ia_abs, 'all');
                    
                    prob_death = (1 - cS.s_pathV(ia));
                    Bq_gen_raw = Bq_gen_raw + sum(k_prime_slice .* mass_ia_abs, 'all') * prob_death;
                end
                ss.K_p_hat_eop_raw_ss = Kp_eop_raw;
                ss.Bequest_gen_hat_raw_ss = Bq_gen_raw;
                ss.TR_hat_raw_ss = TR_new * (1 + n_period); % 假设TR与Bequest有类似调整
            end
        end        
        ...
        function display_national_accounts(ss, cS, paramS, Z_ss_norm, report_filename)
            ...
            % == 版本: [v7.9 - 养老金福利(b_hat)求解版]
            % ==
            % == 核心修改:
            % ==   - 修正了对 system_of_equations 的调用，使用新的均衡变量向量。
            % ==   - 在PAYG核算部分，直接使用ss中存储的均衡 b_hat 值。
            % =========================================================================
            ...
            if pps_active
                fprintfAndLog(fileID, '###                 稳 态 国 民 经 济 核 算 报 告 (v7.9 - 带PPS, PAYG分离版)              ###\n');
            else
                fprintfAndLog(fileID, '###                       稳 态 国 民 经 济 核 算 报 告 (v7.9 - PAYG分离版)                      ###\n');
            end
            fprintfAndLog(fileID, '=========================================================================================================\n');
            ...
            % [!!! 核心修正 !!!] 修正函数调用
            final_errors = SS.system_of_equations([ss.r_mkt, ss.w_hat, ss.Bequest_distributed_agg, ss.TR_distributed_agg, ss.b_hat], Z_ss_norm, cS, paramS, struct('A',1.0));
            ...
            fprintfAndLog(fileID, '   5. 劳动供需平衡 (L_req - L_supply) .... : %15.4e\n', final_errors(5));
        ...
        function Dist = get_final_dist(ss, cS, paramS, Z_ss_norm)
            % =========================================================================
            % == 函数: get_final_dist
            % == 版本: [v1.2 - 养老金福利(b_hat)求解版]
            % ==
            % == 核心修改:
            % ==   - 修正了对 system_of_equations 的调用，使用新的均衡变量向量，
            % ==     其中包含了均衡养老金福利 ss.b_hat。
            % =========================================================================
            x_eq_final = [ss.r_mkt, ss.w_hat, ss.Bequest_distributed_agg, ss.TR_distributed_agg, ss.b_hat];
