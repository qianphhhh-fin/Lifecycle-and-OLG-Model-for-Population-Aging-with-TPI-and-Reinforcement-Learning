...
function [val_age, pol_age] = VFI_by_age_PPS(a_idx, vPrime_kkppse_next, M_age, b_age_val, paramS_age, cS)
            % =========================================================================
            % == 函数: VFI_by_age_PPS (版本 v2.5 - 修正插值函数坐标维度错误)
            % == 核心修正与确认:
            % ==   - [!!! 错误修正 !!!] 解决了 griddedInterpolant 的坐标输入维度不匹配问题。
            % ==   - [!!! 解决方案 !!!] 使用 reshape 和 repmat 将下一期PPS资产选择
            % ==     (kpps_prime_hat_choices) 显式地扩展成一个与常规资产选择张量
            % ==     (k_prime_choices_tens) 大小完全相同的张量 (kpps_prime_tens)。
            % ==   - 统一使用这个新创建的 kpps_prime_tens 进行后续的插值和财富计算，
            % ==     确保了维度的一致性。
            % =========================================================================
            nk_search_grid = 20;
            ...
            for task_idx = 1:num_tasks
                ...
                % 5. 构建选择张量并进行向量化计算
                % a. 创建PPS缴费率选择向量
                pps_rate_choices_vec = linspace(0, 1, n_pps_rate_grid);

                % b. 计算不同PPS缴费率下的缴费额和下一期PPS资产
                pps_contrib_hat_choices = labor_income_hat * pps_rate_choices_vec; % -> 1 x n_pps_rate_grid
                kpps_hat_end_of_period_choices = kpps_hat_current * (1 + M_age.r_mkt_t) + pps_contrib_hat_choices - pps_withdrawal_hat;
                kpps_prime_hat_choices = max(cS.kppsMin, kpps_hat_end_of_period_choices / growth_factor_bgp); % -> 1 x n_pps_rate_grid

                % c. 计算不同PPS缴费率下的手头现金 (CoH)
                ...
                cash_on_hand_hat_mat = base_coh_hat_vec - pps_contrib_hat_choices; % -> nk x n_pps_rate_grid

                % d. 构建 k_prime 和消费的联合选择张量
                ...
                % [!!! 核心修正 !!!] 创建一个与 k_prime_choices_tens 大小完全相同的 kpps_prime_tens 张量
                kpps_prime_reshaped = reshape(kpps_prime_hat_choices, [1, 1, n_pps_rate_grid]);
                ...
                if a_idx < cS.aD_new
                    ...
                    util_bequest_tens = cS.phi_bequest * (total_wealth_prime_tens.^(1-cS.sigma))./(1-cS.sigma);
                    ...
                    total_wealth_prime_tens = k_prime_choices_tens + kpps_prime_tens;
                    future_value_tens = discount_factor_V_prime * utils.bequest_utility(total_wealth_prime_tens, cS);
                end

                total_value_tens = util_c_tens + future_value_tens;

                % 6. 寻找最优决策组合
                total_value_mat_2d = reshape(total_value_tens, cS.nk, nk_search_grid * n_pps_rate_grid);
                [best_val_final_vec, best_joint_idx_vec] = max(total_value_mat_2d, [], 2);

                [best_k_prime_idx, best_pps_rate_idx] = ind2sub([nk_search_grid, n_pps_rate_grid], best_joint_idx_vec);
    ...
    for ie = 1:cS.nw_expanded
        ...
        function run_vfi_test()
            % =========================================================================
            % == 测试函数: run_vfi_test (V2.1 - 整合命令行汇报与可视化)
            % == 核心变更:
            % ==   - 恢复了对特定年龄点(ages_to_check)的命令行表格汇报功能。
            % ==   - 在命令行汇报之后，继续执行数据提取和绘图，提供完整的
            % ==     生命周期可视化。
            % ==   - 结合了数值检查和图形直觉，提供了更全面的测试反馈。
            % ==
            % == 如何运行:
            % ==   1. 确保 household.m 和 utils.m 在MATLAB路径中。
            % ==   2. 运行一个启动器脚本，其中包含命令: household.run_vfi_test()
            % =========================================================================
            ...
            % 
            % % Case 1: No PPS
            % lc_data1 = struct('c', zeros(1, cS.aD_new), 'k', zeros(1, cS.aD_new), 'l', zeros(1, cS.aD_new));
            % for a = age_vec
            %     lc_data1.c(a) = mean(polS1(a).c, 'all');
            %     lc_data1.k(a) = mean(polS1(a).k_prime, 'all');
            %     lc_data1.l(a) = mean(polS1(a).l, 'all');
            % end
            % 
            % % Case 2: With PPS
            % lc_data2 = struct('c', zeros(1, cS.aD_new), 'k', zeros(1, cS.aD_new), 'kpps', zeros(1, cS.aD_new), 'l', zeros(1, cS.aD_new));
            % for a = age_vec
            %     lc_data2.c(a) = mean(polS2(a).c, 'all');
            %     lc_data2.k(a) = mean(polS2(a).k_prime, 'all');
            %     lc_data2.kpps(a) = mean(polS2(a).kpps_prime, 'all');
            %     lc_data2.l(a) = mean(polS2(a).l, 'all');
            % end
            % 
            % % --- 5. 可视化生命周期策略函数 ---
            % fprintf('\n正在生成生命周期剖面图...\n');
            % 
            % % 图 1: 无 PPS 的情况
            % figure('Name', 'Life-Cycle Profiles (Without PPS)');
            % sgtitle('生命周期策略函数平均走势 (无PPS)', 'FontSize', 14, 'FontWeight', 'bold');
            % 
            % subplot(2,1,1);
            % hold on;
            % plot(age_vec, lc_data1.c, 'LineWidth', 2, 'DisplayName', '消费 (c)');
            % plot(age_vec, lc_data1.k, '--', 'LineWidth', 2, 'DisplayName', '常规储蓄 (k'')');
            % hold off;
            % title('消费与资产');
            % xlabel('年龄 (a)');
            % ylabel('平均水平');
            ...
            % 
            % subplot(2,1,2);
            % plot(age_vec, lc_data1.l, 'LineWidth', 2, 'Color', [0.8500 0.3250 0.0980], 'DisplayName', '劳动供给 (l)');
            % title('劳动供给');
            % xlabel('年龄 (a)');
            % ylabel('平均水平');
            % xlim([1, cS.aD_new]);
            % ylim([0, max(lc_data1.l)*1.1 + 1e-6]);
            % grid on;
            % legend('show', 'Location', 'best');
            % 
            % % 图 2: 有 PPS 的情况
            % figure('Name', 'Life-Cycle Profiles (With PPS)');
            % sgtitle('生命周期策略函数平均走势 (有PPS)', 'FontSize', 14, 'FontWeight', 'bold');
            % 
            % subplot(2,1,1);
            % hold on;
            % plot(age_vec, lc_data2.c, 'LineWidth', 2, 'DisplayName', '消费 (c)');
            % plot(age_vec, lc_data2.k, '--', 'LineWidth', 2, 'DisplayName', '常规储蓄 (k'')');
            % plot(age_vec, lc_data2.kpps, ':', 'LineWidth', 2, 'DisplayName', 'PPS储蓄 (kpps'')');
            % hold off;
            % title('消费与资产');
            % xlabel('年龄 (a)');
            % ylabel('平均水平');
            ...
            % 
            % subplot(2,1,2);
            % plot(age_vec, lc_data2.l, 'LineWidth', 2, 'Color', [0.8500 0.3250 0.0980], 'DisplayName', '劳动供给 (l)');
            % title('劳动供给');
            % xlabel('年龄 (a)');
            % ylabel('平均水平');
            % xlim([1, cS.aD_new]);
            % ylim([0, max(lc_data2.l)*1.1 + 1e-6]);
            % grid on;
            % legend('show', 'Location', 'best');
            % 
            % fprintf('\n============================================\n');
            % fprintf('==       household.VFI_solver 测试结束      ==\n');
            % fprintf('============================================\n');
        end
