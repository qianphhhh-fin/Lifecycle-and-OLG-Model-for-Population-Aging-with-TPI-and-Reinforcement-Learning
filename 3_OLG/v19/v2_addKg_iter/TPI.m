...
function [target_paths, errors, Pol_path, Dist_path, aggr_supply] = calculate_paths_and_errors(r_path, w_hat_path, bequest_gen_raw_pc_path_guess, TR_hat_pc_path, b_hat_path, K_g_hat_path_pe_guess, ss0, ssF, cS, paramSF, valF, polF, dist0)
    % =========================================================================
    % ==          TPI核心计算引擎 (v8.7 - K_g目标路径重构修正版)
    % ==
    % == 核心修正:
    % ==   - [!!! 稳定性增强 !!!] 修改了公共资本K_g目标路径的计算逻辑，
    % ==     解除了单次迭代内的递归依赖。
    % ==   - 新方法分为两步：
    % ==     1. 基于本轮计算的Y路径，一次性计算出完整的公共投资路径 I_g_path。
    % ==     2. 基于初始K_g(1)和完整的I_g_path，一次性、从前向后递推出
    % ==        整个target_Kg路径。
    % ==   - 这与OG-Core等成熟模型的实践更为一致，旨在提高TPI的收敛平滑性。
    % =========================================================================
    T = cS.T_sim;
    ...
    bequest_gen_raw_total_path_guess_t_minus_1 = bequest_gen_raw_pc_path_guess_t_minus_1 .* [total_pop_t0, total_pop_path(1:T-1)];
    ...
    % 首先，基于新计算的Y路径，得到完整的公共投资I_g路径 (hat单位)
    I_g_hat_total_path_calc = cS.I_g_to_Y_ratio_ss * Y_hat_total_path_calc;
    
    % 然后，基于初始K_g(1)和完整的I_g路径，一次性递推出整个目标K_g路径
    target_paths_K_g_hat_total(1) = K_g_hat_total_path_guess(1); % K_g(1)由初始猜测（或上轮迭代结果）确定
