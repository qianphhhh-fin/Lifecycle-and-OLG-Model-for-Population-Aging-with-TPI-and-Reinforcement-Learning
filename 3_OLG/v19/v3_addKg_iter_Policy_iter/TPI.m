...
function [target_paths, errors, Pol_path, Dist_path, aggr_supply] = calculate_paths_and_errors(r_path, w_hat_path, Bequest_gen_hat_path_raw_guess, TR_hat_pc_path, b_hat_path, K_g_hat_path_pe_guess, Pol_path_guess, ss0, ssF, cS, paramSF, valF, polF, dist0)
    % =========================================================================
    % ==          TPI核心计算引擎 (v9.0 - 修正theta_path缺失问题)
    % ==
    % == 核心修正:
    % ==   - 在构造传递给家庭求解器的 pathS 结构体时，增加了
    % ==     `pathS.theta_path = cS.theta_path;` 这一行。
    % ==   - 这修复了 household.backward_hh 函数因找不到 theta_path 字段
    % ==     而导致的崩溃问题。
    % =========================================================================
    T = cS.T_sim;
    ...
    pathS.theta_path = cS.theta_path; % [!!! 核心修正 !!!] 添加缺失的theta_path

    % --- 2. 向后求解家庭最优策略 ---
    [Pol_path, ~] = household.backward_hh(pathS, cS, paramSF, valF, polF);

    % --- 3. 计算每个新生儿收到的遗赠，用于分布模拟 ---
    Bequest_gen_hat_path_raw_guess_t_minus_1 = [ss0.Bequest_gen_hat_raw_ss, Bequest_gen_hat_path_raw_guess(1:T-1)];
    ...
    bequest_per_newborn_hat_path(non_zero_newborns_mask) = ...
        Bequest_gen_hat_path_raw_guess_t_minus_1(non_zero_newborns_mask) ./ mass_newborns_path(non_zero_newborns_mask);
    ...
    % --- 5c. 计算目标遗赠【总量】路径 ---
    target_paths.Bequest_gen_hat_path_raw = aggr_supply.Bequest_gen_hat_path;
    ...
    errors.beq = max(abs(target_paths.Bequest_gen_hat_path_raw(update_range_flow) - Bequest_gen_hat_path_raw_guess(update_range_flow)));
