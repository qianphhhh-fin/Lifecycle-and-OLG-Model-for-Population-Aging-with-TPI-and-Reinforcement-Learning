...
% --- 在第2节 "构造或加载初始猜测路径" 之后，初始化策略函数猜测 ---
fprintf('\n--- 2c. 构造策略函数初始猜测路径 ---\n');
Pol_path_iter = cell(1, T);
% 使用终期稳态的策略函数作为所有时期的初始猜测
for t = 1:T
    Pol_path_iter{t} = polF; 
end
fprintf('   ✅ 已使用终态策略函数 (polF) 初始化 Pol_path_iter。\n');
...
for iter = 1:max_iter
    ...
    [target_paths, errors, Pol_path_calc, ~, ~] =  TPI.calculate_paths_and_errors(...
    ...
    K_g_hat_path_iter(update_range_state) = (1 - damping.k_g) * K_g_hat_path_iter(update_range_state) + damping.k_g * target_paths.K_g_hat_path(update_range_state);
     ...
     % [!!! 核心修改 !!!] 更新策略函数路径
    damp_pol = 0.7; % 策略函数的阻尼因子可以设置得略高一些 (更新慢一些)
    for t = 1:(T-1) % 最后一期通常是稳态，不需要更新
        pol_old_t = Pol_path_iter{t};
        pol_new_t = Pol_path_calc{t};
        
        % 对策略结构体中的每个字段进行加权平均
        pol_old_t.c = (1 - damp_pol) .* pol_old_t.c + damp_pol .* pol_new_t.c;
        pol_old_t.k_prime = (1 - damp_pol) * pol_old_t.k_prime + damp_pol * pol_new_t.k_prime;
        if cS.pps_active
            pol_old_t.kpps_prime = (1 - damp_pol) * pol_old_t.kpps_prime + damp_pol * pol_new_t.kpps_prime;
        end
        % 注意：劳动 'l' 和税收 'tax_regular' 是其他决策的结果，通常不需要直接平滑，
        % 它们会随着 c, k_prime 的更新而更新。但如果为了极致的稳定，也可以平滑。
        
        Pol_path_iter{t} = pol_old_t;
    end
