...
classdef distribution
    ...
    % =========================================================================
    % == 函数: get_ss_dist_no_pps (v1.4 - BGP一致性修正版)
    % == 核心修正:
    % ==   - [!!! 关键BUG修复 !!!] 移除了在代际演化中对生存率 s_pathV(ia)
    % ==     的重复计算。
    % ==   - 理由: 当一个外生的、理论上正确的稳态人口分布 Z_ss_norm 被
    % ==     传入时，该分布已经内含了所有生存率和人口增长率的信息。
    % ==     此函数的核心任务是计算给定人口结构下，资产的条件分布。
    % ==     再次乘以生存率会导致“双重计算”死亡效应，从而得到一个与BGP
    % ==     不一致的错误资产分布。
    % =========================================================================
    nk = cS.nk;
    nw = cS.nw_expanded;
        ...
        for ik = 1:nk
            for ie = 1:nw
                ...
                for ie_next = 1:nw
                    ...
                    % 使用原始质量进行分配，因为生存效应已包含在Z_ss_norm中
    ...
    % =========================================================================
    % == 函数: get_ss_dist_pps (v1.4 - BGP一致性修正版)
    % == 核心修正:
    % ==   - [!!! 关键BUG修复 !!!] 与 get_ss_dist_no_pps 的修改一致，
    % ==     移除了在代际演化中对生存率 s_pathV(ia) 的重复计算，以确保
    % ==     计算出的资产分布与BGP的要求一致。
    % =========================================================================
    nk = cS.nk;
    nkpps = cS.nkpps;
