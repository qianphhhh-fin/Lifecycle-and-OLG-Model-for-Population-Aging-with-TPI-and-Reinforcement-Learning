...
classdef TPI
    ...
    % =========================================================================
    % ==          TPI核心计算引擎 (v9.15 - 退休年龄路径修正版)
    % ==
    % == 核心修改:
    % ==   - [!!! 关键BUG修复 !!!] 修正了计算总养老金福利时划分退休人口的逻辑。
    % ==   - 旧逻辑: 使用固定的 cS.aR_new 来计算所有时期的退休人口总数。
    % ==   - 新逻辑: 在对时间 t 的循环内部，使用该时期特定的退休年龄索引
    % ==     aR_t = cS.aR_new_path(t) 来划分工作和退休人口。
    % ==   - 这确保了PAYG系统平衡的计算与家庭在VFI中面对的时变退休年龄完全一致，
    % ==     是解决NIPA投资缺口问题的关键。
    % =========================================================================
    T = cS.T_sim;
    nH = cS.nTypes;
    ...
    for h = 1:nH
        ...
        cS_h.aR_new_path = cS.aR_new_path; % 确保路径被传递
    ...
    for t = 1:T
        ...
        for h = 1:nH
            L_path_supply_h_t = TPI.get_labor_supply_path_for_type(h, {Dist_path_h{h}(:,:,:,:,t)}, cS, paramSF, aR_t);
            
            total_contrib_t = total_contrib_t + cS.theta_path_h(h,t) * target_paths.w_hat_path(t) * L_path_supply_h_t;
            
            % === 核心修正: 使用 aR_t 来计算当期的退休人口质量 ===
    ...
    errors.total = max([errors.r, errors.w, errors.adj, errors.beq, errors.tr, errors.k_g]);
        ...
        function L_path_h = get_labor_supply_path_for_type(h, Dist_path_h, cS, paramS, aR_t_period)
            % 辅助函数: 专门为某个类型 h 计算其劳动供给路径
            % [修改 v1.1] 增加输入参数 aR_t_period，以使用当期的退休年龄
            ...
            for t = 1:T
                ...
                % 如果是多期计算，aR_t_period应该是路径；如果是单期，它是一个标量
                if length(aR_t_period) > 1
                    aR_now = aR_t_period(t);
                else
                    aR_now = aR_t_period;
                end

                for ia = 1:aR_now % 使用当期的退休年龄作为循环上界
...
function check_transition_NIPA(results, cS, ss0, ssF)
        ...
        function TR_total_hat_path = calculate_endogenous_TR_path(r_path, w_hat_path, K_p_hat_path, L_path, Y_hat_path, Dist_path_h, Pol_path_h, cS, paramS)
            % =========================================================================
            % == 函数: calculate_endogenous_TR_path (v2.1 - 退休年龄路径修正版)
            % == 核心修改:
            % ==   - [!!! 关键BUG修复 !!!] 在计算从总税收中剥离的PAYG缴费时，
            % ==     使用了随时间变化的退休年龄路径 aR_new_path(t)。
            % ==   - 旧逻辑: 使用固定的 cS.aR_new，导致计算出的“一般税收”不准。
            % ==   - 新逻辑: 在每个时期t，都使用正确的 aR_t = cS.aR_new_path(t)
            % ==     作为划分工作年龄的上界，确保了转移支付 TR 的计算与
            % ==     家庭面对的真实环境完全一致。
            % =========================================================================
            T = cS.T_sim;
            nH = cS.nTypes;
            ...
            for t=1:T
                ...
                for h = 1:nH
                    ...
                    % === 核心修正: 使用 aR_t 来计算当期的PAYG缴费总额 ===
                    for ia = 1:aR_t % 只对工作年龄人口计算
                        mass_ia_abs_h = dist_t_abs_h(:,:,:,ia);
                        if sum(mass_ia_abs_h, 'all') < 1e-30, continue; end % 如果该年龄组没人，跳过
