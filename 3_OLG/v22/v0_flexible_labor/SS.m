classdef SS
    methods (Static)
        ...
        function [ss, Dist, polS, valS] = solve_steady_state(cS, paramS, params_ext, verbose, is_bgp_ss)
            ...
            'MaxIterations', 200); %  ,'Algorithm','levenberg-marquardt'
        ...
        function [F_error, ss, Dist, polS, valS] = system_of_equations(x_guess, Z_ss_norm, cS, paramS, params_ext, is_bgp_ss)
            ...
            % ==
            % == 核心修改:
            % ==   - [!!! 关键修正 !!!] 修正了 General_Tax_Revenue_calc 的计算。
            % ==   - 由于 household.VFI_by_age_PPS 已被修正，其输出的 tax_regular
            % ==     包含了 PAYG 缴费。
            % ==   - 因此，此处必须从聚合得到的 Total_Tax_Revenue_total 中
            % ==     减去 PAYG_contributions_total，以得到正确的一般政府税收。
            % ==   - 此修改使稳态求解器的核算逻辑与 v22_heter_payg 基准完全一致。
            % =========================================================================
            nH = cS.nTypes;
        ...
        function report_struct = display_national_accounts(ss, cS, paramS, Z_ss_norm, report_filename, print_flag, is_bgp_ss)
            ...
            % ==
            % == 核心修改:
            % ==   - [!!! 关键修正 !!!] 修正了“一般税收”和“PAYG缴费”的聚合逻辑，
            % ==     以完全对齐 v22_heter_payg 基准模型的会计方法。
            % ==   - 步骤1: 聚合从家庭策略中得到的、包含PAYG缴费的总税收
            % ==     (Total_Tax_Revenue_agg)。
            % ==   - 步骤2: 基于内生劳动供给决策(pol_ia_h.l)，独立聚合PAYG缴费。
            % ==   - 步骤3: 在循环外，通过 (总税收 - PAYG缴费) 计算出正确的
            % ==     一般税收 (General_Tax_Revenue_total)。
            % ==   - 此修改确保了报告的准确性和模型内部的会计一致性。
            % =========================================================================
            ...
            PAYG_formula_benefits_agg_h = zeros(nH, 1);
            Total_Tax_Revenue_agg = 0; % [修改] 用于聚合包含PAYG的总税收
            C_agg_total = 0;
            L_hat_total = 0;
            ...
            for h = 1:nH
                ...
                for ia = 1:cS.aD_new
                    ...
                    % [核心修正] 总税收聚合
                    Total_Tax_Revenue_agg = Total_Tax_Revenue_agg + sum(pol_ia_h.tax_regular .* mass_dist_ia_h, 'all');
                    
                    if ia <= cS_h.aR_new
                        % 内生劳动供给聚合
                        le_grid_slice = reshape(paramS.leGridV, [1, 1, cS.nw_expanded]);
                        le_mat = repmat(le_grid_slice, [cS.nk, cS.nkpps, 1]);
                        total_effective_labor_grid = cS_h.ageEffV_new(ia) .* le_mat .* pol_ia_h.l;
                        L_hat_total = L_hat_total + sum(total_effective_labor_grid .* mass_dist_ia_h, 'all');
                        
                        % 基于内生劳动供给的PAYG缴费聚合
                        labor_income_grid_from_l = ss.w_hat * total_effective_labor_grid;
            ...
            General_Tax_Revenue_total = Total_Tax_Revenue_agg - PAYG_contributions_agg_total;
            PAYG_benefits_agg_total = sum(PAYG_benefits_agg_h);
            PAYG_formula_benefits_agg_total = sum(PAYG_formula_benefits_agg_h);
            ...
            if print_flag
                ...
                fprintfAndLog(fileID, '###            稳 态 国 民 经 济 核 算 报 告 (v9.8 - 弹性劳动核算对齐版)           ###\n');
                fprintfAndLog(fileID, '=========================================================================================================\n');
                ...
                gov_revenue_final = General_Tax_Revenue_total + ss.Public_Capital_Return;
        ...
        function polS_by_type = get_final_polS(ss, cS, paramS, Z_ss_norm)
            ...
            % ==
            % == 核心修改:
            % ==   - [!!!] 在为每个家庭类型 h 准备参数 cS_h 时，增加了
            % ==     cS_h.nu = cS.nu_h(h) 这一行。
            % ==   - 这确保了在报告阶段重新计算最终策略时，VFI求解器能够
            % ==     接收到正确的、特定类型的劳动供给弹性参数。
            % =========================================================================
            nH = cS.num_hh_types;
