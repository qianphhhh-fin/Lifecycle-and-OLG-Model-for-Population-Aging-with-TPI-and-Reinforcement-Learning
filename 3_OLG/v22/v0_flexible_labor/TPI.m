...
classdef TPI
    methods (Static)
        ...
        function [target_paths, errors, Pol_path_h, Dist_path_h, aggr_supply] = calculate_paths_and_errors(r_path, w_hat_path, bequest_gen_raw_pc_path_guess, ...
                TR_hat_pc_path, adj_path_guess, K_g_hat_path_pe_guess, ss0, ssF, cS, paramSF, valF_h, polF_h, dist0_h)
            % =========================================================================
            % ==          TPI核心计算引擎 (v9.21 - 劳动聚合调用修正版)
            % ==
            % == 核心修改:
            % ==   - [!!!] 修正了对 get_labor_supply_path_for_type 的调用。
            % ==   - 现在调用一个更简洁、更稳健的 get_labor_supply_path_for_type
            % ==     版本，该版本不再需要家庭类型索引 h 作为参数。
            % ==   - 这与 get_labor_supply_path_for_type 函数本身的简化相匹配，
            % ==     从根本上消除了之前版本中的索引和冗余代码问题。
            % =========================================================================
            T = cS.T_sim;
            nH = cS.nTypes;
            ...
            for h = 1:nH
                ...
                pathS_h.b_hat_path = adj_path_guess .* b_hat_formula_path_h(h, :);
                pathS_h.g_A_path = g_A_path_period(1:T);
                pathS_h.theta_path = cS.theta_path_h(h, :);
                
                cS_h = cS; 
                cS_h.ageEffV_new = cS.ageEffV_new_h(:,h);
                cS_h.aR_new_path = cS.aR_new_path;
                
                [Pol_path_h{h}, ~] = household.backward_hh(pathS_h, cS_h, paramSF, valF_h{h}, polF_h{h}, h);
            end
            ...
            L_supply_path_by_type = zeros(nH, T);
            for h = 1:nH
                % [!!! 核心修正 !!!] 调用简化的、不再需要 h 作为参数的函数
                L_supply_path_by_type(h, :) = TPI.get_labor_supply_path_for_type(Dist_path_h{h}, Pol_path_h{h}, cS);
            end

            target_paths.adj_path = zeros(1, T);
            ...
            for t = 1:T
                ...
                for h = 1:nH
                    L_path_supply_h_t = L_supply_path_by_type(h, t);
                    total_contrib_t = total_contrib_t + cS.theta_path_h(h,t) * target_paths.w_hat_path(t) * L_path_supply_h_t;
                ...
                if total_formula_benefits_hat_path(t) > 1e-9
                    target_paths.adj_path(t) = total_pension_pot_hat_path(t) / total_formula_benefits_hat_path(t);
                else
                    target_paths.adj_path(t) = 1.0; 
                end
            end
            ...
            if T > 1
               bequest_pc_new_guess(1:T-1) = bequest_gen_raw_new_path(1:T-1) ./ total_pop_path(1:T-1); 
               bequest_pc_new_guess(T) = bequest_pc_new_guess(T-1);
            end
            ...
            errors.adj = max(abs(target_paths.adj_path(update_range_flow) - adj_path_guess(update_range_flow))); 
            ...
            errors.total = max([errors.r, errors.w, errors.adj, errors.beq, errors.tr, errors.k_g]);
        end

        function L_path_h = get_labor_supply_path_for_type(Dist_path_single_h, Pol_path_single_h, cS)
            % =========================================================================
            % == 函数: get_labor_supply_path_for_type (v3.1 - 简化修正版)
            % == 核心修改:
            % ==   - [!!! 关键BUG修复 !!!] 移除了函数内部所有冗余且引发错误的代码。
            % ==   - 函数签名被简化，不再需要家庭类型索引 h 作为输入。
            % ==   - 函数的核心逻辑现在非常清晰：仅聚合传入的策略 pol.l，
            % ==     该策略已经包含了所有类型特定的最优决策。
            % =========================================================================
            T = size(Dist_path_single_h, 5);
            L_path_h = zeros(1, T);

            for t = 1:T
                L_t_h = 0;
                dist_t_abs_h = Dist_path_single_h(:,:,:,:,t);
                pol_t_h = Pol_path_single_h{t}; 
        ...
        function TR_total_hat_path = calculate_endogenous_TR_path(r_path, w_hat_path, K_p_hat_path, L_path, Y_hat_path, Dist_path_h, Pol_path_h, cS, paramS)
            % =========================================================================
            % == 函数: calculate_endogenous_TR_path (v2.4 - 税收核算对齐版)
            % == 核心修改:
            % ==   - [!!! 关键修正 !!!] 此函数的逻辑被完全重构，以匹配 v22_heter_payg
            % ==     基准模型的会计核算方法，并适配弹性劳动供给。
            % ==   - 步骤 1: 聚合包含 PAYG 的总税收 (total_tax_hat_t_raw)。
            % ==   - 步骤 2: 单独聚合 PAYG 缴费 (payg_contrib_hat_t_raw)，
            % ==     此聚合基于内生劳动决策 pol.l。
            % ==   - 步骤 3: 从总税收中减去PAYG缴费，得到正确的一般税收。
            % ==   - 步骤 4: 基于正确的一般税收，计算政府总收入和转移支付。
            % =========================================================================
            T = cS.T_sim;
            ...
            for t=1:T
                ...
                for h = 1:cS.nTypes
                    ...
                    for ia = 1:cS.aD_new
                        ...
                        if ia <= aR_t
                            ...
                            le_mat_for_income = repmat(le_grid_slice, [cS.nk, cS.nkpps, 1]);
                            labor_income_grid_from_l = w_hat_path(t) * cS_h.ageEffV_new(ia) .* le_mat_for_income .* pol_ia_t_h.l;
                            payg_contrib_grid = cS_h.theta_path * labor_income_grid_from_l;
                            payg_contrib_hat_t_raw = payg_contrib_hat_t_raw + sum(payg_contrib_grid .* mass_ia_abs_h, 'all');
                        end
                    end
                end
                ...
                Public_Capital_Return_t_level = Y_t_level - w_hat_path(t) * cS.A_path(t) * L_path(t) - (r_path(t) + cS.ddk) * K_p_t_level;
        ...
        function check_transition_NIPA(results, cS, ss0, ssF)
            % =========================================================================
            % == 函数: check_transition_NIPA (版本 v3.2 - 弹性劳动深度诊断版)
            % == 核心修改:
            % ==   - [!!! 关键诊断增强 !!!] 深度诊断部分现在同时检查 t=1 时的
            % ==     【劳动供给L】和【资本存量K】与初始稳态 ss0 是否一致。
            % ==   - 增加了 L_path 的获取和 L_hat_path 的计算。
            % ==   - 打印 ss0.L_hat 与 L_hat_path(1) 的对比，这是定位投资缺口的
            % ==     关键步骤。
            % ==   - 标准诊断报告中增加了劳动供给L的显示，以提供更全面的信息。
            % =========================================================================

            fprintf('   正在启动过渡路径国民账户一致性检验 (v3.2 - 弹性劳动深度诊断版)...\n');
            T = cS.T_sim;

            % --- 1. 获取所有流量和存量路径 (Level) ---
            Y_path = results.Y_path;
            C_path = results.C_path;
            L_path = results.L_path; % [!!! 弹性劳动修正 !!!]
            ...
            total_pop_path = sum(cS.Z_path_raw, 1);
            K_p_hat_path_pc = K_p_path ./ cS.A_path ./ total_pop_path;
            L_hat_path_pc = L_path ./ total_pop_path;
            Y_hat_path_pc = Y_path ./ cS.A_path ./ total_pop_path;
            C_hat_path_pc = C_path ./ cS.A_path ./ total_pop_path;

            fprintf('      [存量 Stock]\n');
            fprintf('         初始稳态 ss0.K_private_hat .......... : %15.6f (来自稳态求解器)\n', ss0.K_private_hat);
            fprintf('         t=1 期初资本 K_p_hat_pc(1) ............ : %15.6f (来自TPI聚合dist0)\n', K_p_hat_path_pc(1));
            fprintf('         t=2 期初资本 K_p_hat_pc(2) ............ : %15.6f (来自TPI聚合dist(2))\n', K_p_hat_path_pc(2));

            fprintf('      [关键流量 Flow]\n');
            fprintf('         初始稳态 ss0.L_hat .................... : %15.6f (来自稳态求解器)\n', ss0.L_hat);
            fprintf('         t=1 劳动供给 L_hat_pc(1) .............. : %15.6f (来自TPI聚合dist0)\n', L_hat_path_pc(1));
            fprintf('         t=1 产出 Y_hat_pc(1) .................... : %15.6f\n', Y_hat_path_pc(1));
            fprintf('         t=1 消费 C_hat_pc(1) .................... : %15.6f\n', C_hat_path_pc(1));
            fprintf('         I_p_from_K_law(1)_level ............... : %15.6f\n', I_p_from_K_law(1));
            fprintf('         I_p_from_NIPA(1)_level ................ : %15.6f\n', I_p_from_NIPA(1));

            k_diff = K_p_hat_path_pc(1) - ss0.K_private_hat;
            l_diff = L_hat_path_pc(1) - ss0.L_hat;
            fprintf('      [诊断]\n');
            fprintf('         K_p_hat(1) 与 ss0.K_private_hat 的差异 : %.4e (%.4f%%)\n', k_diff, (k_diff/ss0.K_private_hat)*100);
            fprintf('         L_hat(1) 与 ss0.L_hat 的差异 .......... : %.4e (%.4f%%)\n', l_diff, (l_diff/ss0.L_hat)*100);

            if abs(k_diff / ss0.K_private_hat) > 1e-4 || abs(l_diff / ss0.L_hat) > 1e-4
                fprintf('         >>> 警告: t=1的期初存量/流量与初始稳态严重不符！这很可能是问题的根源。\n');
                fprintf('             请重点检查: main_run_SS.m 中 dist0 的构造是否正确反映了t=1的人口结构。\n');
            else
                fprintf('         >>> 信息: t=1的期初变量与初始稳态基本一致。问题可能出在 K(2)或L(2) 的计算上。\n');
            end
            ...
            if max_rel_gap > 1e-4
                ...
                print_gap_at_t = @(t) fprintf(...
                    ...
                    t, Y_path(t), C_path(t), I_g_path(t), G_path(t), L_path(t), ...
        ...
        function Pol_path_h_guess = update_policy_guess(Pol_path_h_guess, new_Pol_path_h, damping_factor_pol, cS)
            % =========================================================================
            % ==             【新增】政策函数猜测更新器 v1.1
            % ==
            % == 核心修正:
            % ==   - 增加了对年龄 a_idx 的内层循环。
            % ==   - 这是因为 Pol_path_h_guess{h}{t} 是一个包含 aD_new 个元素的
            % ==     结构体数组，必须逐个元素进行更新，以避免逗号分隔列表错误。
            % =========================================================================
            T = cS.T_sim;
            nH = cS.nTypes;
            ...
            for h = 1:nH
                for t = 1:T
                    ...
                    for a_idx = 1:cS.aD_new
                        % 直接访问特定年龄的结构体
                        current_policy_struct = Pol_path_h_guess{h}{t}(a_idx);
                        fields = fieldnames(current_policy_struct);

                        for i = 1:length(fields)
                            fieldName = fields{i};
                            field_value = current_policy_struct.(fieldName);

                            % 现在 field_value 是单个值(矩阵/标量)，isnumeric 可以正常工作
                            if isnumeric(field_value)
                                old_val = field_value;
                                new_val = new_Pol_path_h{h}{t}(a_idx).(fieldName);

                                % 应用凸组合 (阻尼更新)
                                updated_val = (1 - damping_factor_pol) * old_val + damping_factor_pol * new_val;

                                % 将更新后的值写回结构体数组的正确位置
                                Pol_path_h_guess{h}{t}(a_idx).(fieldName) = updated_val;
                            end
                        end
                    end
                end
        ...
        function plot_transition_with_demographics(results, cS, ssF, title_suffix)
            ...
            % --- 2. 准备BGP参考路径 (level) ---
            ssF_Kp_line = (ones(1, T) * ssF.K_private_hat) .* cS.A_path .* sum(cS.Z_path_raw, 1);
            ssF_Y_line = (ones(1, T) * ssF.Y_from_production_hat) .* cS.A_path.* sum(cS.Z_path_raw, 1);
            ssF_C_line = (ones(1, T) * ssF.C_agg) .* cS.A_path.* sum(cS.Z_path_raw, 1);
