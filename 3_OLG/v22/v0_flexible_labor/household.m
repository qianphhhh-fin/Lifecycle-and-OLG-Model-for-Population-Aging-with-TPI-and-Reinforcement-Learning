...
classdef household
    methods (Static)
        ...
        function [val_age, pol_age] = VFI_by_age_PPS(a_idx, vPrime_kkppse_next, M_age, b_age_val, paramS_age, cS)
            % =========================================================================
            % == 函数: VFI_by_age_PPS (版本 v5.5 - 税收核算对齐版)
            % == 核心修改:
            % ==   - [!!! 关键修正 !!!] 修正了 pol_age.tax_regular 的计算方式。
            % ==   - 为了与 v22_heter_payg 基准模型保持完全的会计一致性，
            % ==     现在 tax_regular 被定义为包含 PAYG 缴费在内的总税负。
            % ==     即 tax_regular = 一般税 + PAYG缴费。
            % ==   - 此修改是解决宏观层面不收敛问题的根本。
            % =========================================================================
            nk_search_grid = 150;
            ...
            % 4. 构建劳动供给选择网格并计算劳动负效用
            is_working_age = (a_idx <= cS.aR_new);
            if is_working_age
                l_choices_vec = cS.lGridV;
                n_l_choices = length(l_choices_vec);
            else
                l_choices_vec = 0; % 退休后劳动为0
                n_l_choices = 1;
            end
            l_choices_reshaped = reshape(l_choices_vec, [1, 1, 1, 1, 1, n_l_choices]);
            
            chi_age = cS.chi_pathV(a_idx);
            util_labor_tens = -chi_age * (l_choices_reshaped.^(1 + cS.nu)) / (1 + cS.nu);
            
            % 5. 计算依赖于劳动选择的收入与税收
            labor_efficiency_mat = cS.ageEffV_new(a_idx) .* le_mat;
            labor_supply_tens = labor_efficiency_mat .* l_choices_reshaped;
            ...
            % 7. 构建选择网格 (k', pps_rate) 并进行向量化计算
            if is_working_age && cS.pps_active && cS.pps_fixed > 0 && cS.n_pps_rate_grid > 1
            ...
            pps_rate_choices_reshaped = reshape(pps_rate_choices_vec, [1, 1, 1, 1, n_pps_rate_grid_eff, 1]);
            
            pps_contrib_hat_tens = labor_income_hat_tens .* pps_rate_choices_reshaped;
            ...
            base_coh_hat_mat = capital_return_hat_mat + M_age.tr_per_hh + pension_income_hat_mat + pps_withdrawal_hat_mat - (capital_tax_hat_mat + pps_tax_hat_mat);
            cash_on_hand_hat_tens = base_coh_hat_mat + labor_income_hat_tens - (labor_tax_hat_tens + payg_contribution_hat_tens + pps_contrib_hat_tens);
            ...
            k_prime_search_grid_reshaped = reshape(k_prime_search_grid, [1, 1, 1, nk_search_grid, 1, 1]);
            ...
            if a_idx < cS.aD_new
                ...
                for ie_next = 1:cS.nw_expanded
                    ...
                    if any(trans_prob_col > 0)
                        ...
                        ev_on_choices_tens = ev_on_choices_tens + v_prime_interp_tens .* reshape(trans_prob_col, [1, 1, cS.nw_expanded, 1, 1, 1]);
                    end
                end
            ...
            total_value_tens = util_c_tens + util_labor_tens + future_value_tens;

            % 9. 寻找联合最优决策 (k', pps_rate, l)
            num_choices = nk_search_grid * n_pps_rate_grid_eff * n_l_choices;
            total_value_reshaped = reshape(total_value_tens, [cS.nk, cS.nkpps, cS.nw_expanded, num_choices]);
            [val_age, best_joint_idx_mat] = max(total_value_reshaped, [], 4, 'omitnan');
            [best_k_prime_idx_mat, best_pps_rate_idx_mat, best_l_idx_mat] = ind2sub([nk_search_grid, n_pps_rate_grid_eff, n_l_choices], best_joint_idx_mat);
            
            % 10. 提取最优策略
            S_full = size(c_choices_tens);
            [I, J, K] = ndgrid(1:S_full(1), 1:S_full(2), 1:S_full(3));
            linear_indices_final = sub2ind(S_full, I, J, K, best_k_prime_idx_mat, best_pps_rate_idx_mat, best_l_idx_mat);
            ...
            l_final_mat = l_choices_vec(best_l_idx_mat);
            
            % --- [!!! 核心修正 !!!] ---
            % 重新计算最优决策下的各项税收和缴费，以构造与基准模型定义一致的 tax_regular
            tax_consumption_mat = c_final_mat .* cS.tau_c;
            optimal_labor_supply = cS.ageEffV_new(a_idx) .* le_mat .* l_final_mat;
            optimal_labor_income = M_age.w_t * optimal_labor_supply;
            optimal_labor_tax = cS.tau_l * optimal_labor_income;
            optimal_payg_contrib = M_age.theta_t * optimal_labor_income;
            
            tax_final_mat = capital_tax_hat_mat + optimal_labor_tax + pps_tax_hat_mat + tax_consumption_mat + optimal_payg_contrib;
            ...
            pol_age = struct(...
                ...
                'pps_contrib_rate', pps_rate_choices_vec(best_pps_rate_idx_mat));
        end        
