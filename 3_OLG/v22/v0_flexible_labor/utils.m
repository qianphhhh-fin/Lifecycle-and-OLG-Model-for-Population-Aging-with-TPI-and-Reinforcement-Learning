classdef utils
    ...
    % =========================================================================
        ...
        % ==               外生路径设置
        % =========================================================================
        function cS=labor_setting(cS,graph_flag)
% 校准数值
% nu_h_values = [ ...
%     1.5;  % 类型1: 城镇职工 - 高弹性
%     0.95;  % 类型2: 城镇职工 - 中等弹性
%     1.5;  % 类型3: 城乡居民 - 较低弹性
%     0.71  % 类型4: 城乡居民 - 低弹性 (更接近生存需求，收入效应强)
% ];
% 测试数值
nu_h_values = [ ...
    inf;  % 类型1: 城镇职工 - 高弹性
    inf;  % 类型2: 城镇职工 - 中等弹性
    inf;  % 类型3: 城乡居民 - 较低弹性
    inf  % 类型4: 城乡居民 - 低弹性 (更接近生存需求，收入效应强)
];
cS.nu_h = nu_h_values;
% cS.lGridV = linspace(0.01, 0.8, cS.nL)'; % 劳动供给选择网格
cS.lGridV = [1.0]; % 测试： 强制唯一的劳动选择是 l=1

% [!!! 核心修改: 构造一个U型的劳动负效用权重剖面 !!!]
chi_scalar_base = 15.0; % 基准负效用权重，作为中年时期的最低点
chi_young_factor = 1.3; % 年轻人相对于最低点的厌恶程度倍数 (例如高50%)
chi_old_factor = 1.5;   % 临近退休者相对于最低点的厌恶程度倍数 (例如高100%)

% 关键年龄点 (以模型期索引 a 表示, 1=20岁, 2=21岁...)
age_start_work = 1; % 开始工作的索引
age_peak_work_desire = 25; % 工作意愿最强(chi最低)的年龄索引 (大约45岁)
age_retire = cS.aR_new; % 退休年龄索引 (这里使用模型设定的退休年龄)

% 构造二次函数的 x 轴 (标准化年龄)
x = 1:cS.aD_new;
x_normalized = (x - age_peak_work_desire) ./ (age_retire - age_peak_work_desire);

% 构造抛物线
chi_age_profile = ones(cS.aD_new, 1);

% 对工作年龄段应用U型剖面
% 年轻段 (从 age_start_work 到 age_peak_work_desire)
...
% 年老段 (从 age_peak_work_desire 到 age_retire)
...
% 对退休后年龄，可以将chi设为极高值以确保劳动供给为0 (或者保持不变，因为VFI中l=0)
chi_age_profile(age_retire+1:end) = chi_old_factor * 1.5; % 确保退休后劳动厌恶程度很高
