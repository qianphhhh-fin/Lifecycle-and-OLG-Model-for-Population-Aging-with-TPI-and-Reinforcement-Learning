...
[Z_path,Z_path_raw, cS] = population.generate_Z_path(cS, true);
T_sim = size(Z_path_raw, 2);

[cS.A_path,g_path_annual_output,~] = utils.generate_tfp_path(cS, 'constant',false); % 'optimistic'  'pessimistic' 'baseline' 'constant'
...
cS = utils.retire_age_path(cS,'mild', Z_path_raw,false); %  'moderate' ''aggressive' 'mild' 'baseline'
...
cS = utils.generate_PPS_path(cS, 'moderate',false); % 'conservative' 'moderate' 'ambitious'
