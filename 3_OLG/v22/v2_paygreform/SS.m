        ...
        function [F_error, ss, Dist, polS, valS] = system_of_equations(x_guess, Z_ss_norm, cS, paramS, params_ext, is_bgp_ss)
            ...
            if ~is_bgp_ss
                ...
                % error_bq = 0;
                        ...
                        % --- [新增] 调试打印模块 ---
            % fprintf('\n--- SS Solver Iteration Diagnosis ---\n');
            % fprintf('%-25s | %15s | %15s | %15s\n', 'Variable/Market', 'Guess/Demand', 'New/Supply', 'Error');
            % fprintf('%s\n', repmat('-', 1, 80));
            % fprintf('%-25s | %15.6f | %15.6f | %15.4e\n', 'Interest Rate (r)', r_guess, r_new, error_r);
            % fprintf('%-25s | %15.6f | %15.6f | %15.4e\n', 'Wage (w_hat)', w_guess, w_new, error_w);
            % fprintf('%-25s | %15.6f | %15.6f | %15.4e\n', 'Transfer (tr_pc)', tr_guess, tr_new, error_tr);
            % fprintf('%-25s | %15.6f | %15.6f | %15.4e\n', 'PAYG Balance', total_benefits_paid_out, PAYG_contributions_total, error_payg);
            % fprintf('%-25s | %15.6f | %15.6f | %15.4e\n', 'Bequest (bq_pc)', bq_guess, bq_new, error_bq);
            % 
            % K_demand_bgp = K_supply_agg_tplus1_raw_total / (1 + n_period);
            % if ~is_bgp_ss, K_demand_bgp = K_supply_agg_t_total; end % For non-bgp, demand equals supply
            % fprintf('%-25s | %15.6f | %15.6f | %15.4e\n', 'BGP K Consistency', K_demand_bgp, K_supply_agg_t_total, error_K);
            % fprintf('%s\n', repmat('-', 1, 80));
            % 
            % temp_F_error = [error_r; error_w; error_tr; error_payg; error_bq];
            % if is_bgp_ss, temp_F_error = [temp_F_error; error_K]; end
            % fprintf('Total Error Norm: %.4e\n', norm(temp_F_error));
            % % --- 结束调试打印 ---
