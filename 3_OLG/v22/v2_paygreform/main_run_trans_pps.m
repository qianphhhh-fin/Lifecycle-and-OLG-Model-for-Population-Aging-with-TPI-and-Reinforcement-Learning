...
% =========================================================================
function main_run_trans_pps(pps_path_mode)
fprintf('=== OLG异质性模型转轨动态求解脚本 (TPI) v9.4 ===\n\n');

    if nargin < 1
        pps_path_mode = 'structural_reform';
    end
    fprintf('=== OLG异质性模型转轨动态求解 (TPI) (模式: %s) ===\n\n', pps_path_mode);


%% --- 1. 加载稳态数据与初始化 ---
fprintf('--- 1. 加载稳态数据与初始化 ---\n');
data_filename = ['SS/data_for_het_transition_pps_',pps_path_mode,'.mat'];
...
iter_results_filename = [ 'TRANS/iter_results_het_pps_',pps_path_mode,'.mat'];
...
for iter = 1:max_iter
    ...
    if errors.total < 5e-5
damp_very_slow = 0.1; % 极度保守
damp_slow = 0.1;     % 保守
damp_fast = 0.1;      % 相对快速
...
if converged
    ...
    if ~exist('TRANS', 'dir'), mkdir('TRANS'); end
    results_filename = ['TRANS/TPI_results_het_pps_',pps_path_mode,'.mat'];


    results.ss_data = data;
    iter_results = struct();
    iter_results.r_path_iter = r_path_iter;
    iter_results.w_hat_path_iter = w_hat_path_iter;
    iter_results.bequest_gen_raw_pc_path_iter = bequest_gen_raw_pc_path_iter;
    iter_results.TR_hat_pc_path_iter = TR_hat_pc_path_iter;
    iter_results.adj_path_iter = adj_path_iter;
    iter_results.K_g_hat_path_iter = K_g_hat_path_iter;
    results.iter_results = iter_results;
