classdef SS
    methods (Static)
        ...
        function [F_error, ss, Dist, polS, valS] = system_of_equations(x_guess, Z_ss_norm, cS, paramS, params_ext, is_bgp_ss)
            ...
            % == 版本: [v5.2 - h_current_type 修正版]
            % ==
            % == 核心修改:
            % ==   - [!!! 关键BUG修复 !!!] 在遍历各类家庭 h 的循环中，向传递给
            % ==     VFI 求解器的 cS_h 结构体中添加了字段 'h_current_type'。
            % ==   - 这一修正确保了 VFI 引擎能够正确地从参数向量 cS.nu_h 中
            % ==     为当前家庭类型 h 选取对应的劳动供给弹性，解决了
            % ==     "无法识别的字段名称" 错误。
            % =========================================================================
            nH = cS.num_hh_types;
            ...
            for h = 1:nH
                ...
                % [!!! 核心修正 !!!] 将当前家庭类型索引h传递给VFI
                cS_h.h_current_type = h;
        ...
        function report_struct = display_national_accounts(ss, cS, paramS, Z_ss_norm, report_filename, print_flag, is_bgp_ss)
            ...
            % ==
            % == 核心修改:
            % ==   - [!!!] 在报告内部用于重新计算和验证宏观流量的核心循环中，
            % ==     更新了总劳动供给(L_hat_total)和PAYG缴费(PAYG_contributions_agg_h)
            % ==     的计算逻辑。
            % ==   - 现在，这些量是基于家庭的内生最优劳动决策 pol_ia_h.l 进行聚合的，
            % ==     而不是依赖外生参数，确保了报告与模型内在逻辑的一致性。
            % =========================================================================
            ...
            if print_flag
                ...
                catch ME, warning('创建报告文件时出错: %s', ME.message); fileID = 1; end
            else, fileID = -1; end
            ...
            for h = 1:nH
                ...
                for ia = 1:cS.aD_new
                    ...
                    if ia <= cS.aR_new
                        % 从策略函数中聚合内生劳动供给
                        L_hat_total = L_hat_total + sum(pol_ia_h.l .* mass_dist_ia_h, 'all');
                        
                        % 基于内生劳动供给计算PAYG缴费
                        labor_income_grid = ss.w_hat .* pol_ia_h.l;
                        payg_contrib_grid = cS_h.theta_path .* labor_income_grid;
                        ...
                        % 退休人员也需计算一般税收 (资本税、消费税等)
                        General_Tax_Revenue_total = General_Tax_Revenue_total + sum(pol_ia_h.tax_regular .* mass_dist_ia_h, 'all');
                    end
        ...
        function polS_by_type = get_final_polS(ss, cS, paramS, Z_ss_norm)
            % =========================================================================
            % == 函数: get_final_polS
            % == 版本: [v2.3 - h_current_type 修正版]
            % ==
            % == 核心修改:
            % ==   - [!!! 关键BUG修复 !!!] 在遍历各类家庭 h 的循环中，向传递给
            % ==     VFI 求解器的 cS_h 结构体中添加了字段 'h_current_type'。
            % ==   - 此修补确保了在生成报告（重新计算策略函数）时，VFI 引擎
            % ==     也能正确获取劳动供给弹性参数，解决了 "无法识别的字段名称" 错误。
            % =========================================================================
            nH = cS.num_hh_types;
