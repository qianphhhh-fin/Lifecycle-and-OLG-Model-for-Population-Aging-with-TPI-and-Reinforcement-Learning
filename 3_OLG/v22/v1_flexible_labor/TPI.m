...
classdef TPI
    methods (Static)
        ...
        function TR_total_hat_path = calculate_endogenous_TR_path(r_path, w_hat_path, K_p_hat_path, L_path, Y_hat_path, Dist_path_h, Pol_path_h, cS, paramS)
            % =========================================================================
            % == 函数: calculate_endogenous_TR_path (v2.2 - 内生劳动税基版)
            % == 核心修改:
            % ==   - [!!!] 在计算从总税收中剥离的PAYG缴费时，不再使用外生参数
            % ==     构建劳动收入网格。
            % ==   - 而是基于家庭的内生最优劳动决策 `pol_t_h(ia).l` 来构建
            % ==     每个状态点的劳动收入 `w_hat * l`，从而得到精确的PAYG缴费基数。
            % ==   - 这确保了TR的计算与家庭的实际劳动决策完全一致。
            % =========================================================================
            T = cS.T_sim;
            nH = cS.nTypes;
            ...
            for t=1:T
                ...
                for h = 1:nH
                    ...
                    for ia = 1:aR_t
                        mass_ia_abs_h = dist_t_abs_h(:,:,:,ia);
                        if sum(mass_ia_abs_h, 'all') < 1e-30, continue; end

                        % [!!! 核心修改: 基于内生劳动计算PAYG缴费 !!!]
                        labor_income_grid_hat = w_hat_path(t) .* pol_t_h(ia).l;
                        payg_contrib_grid_hat = cS_h.theta_path .* labor_income_grid_hat;
...
function check_transition_NIPA(results, cS, ss0, ssF)
            ...
            fprintf('         t=1 K/Y ............................... : %15.6f\n', (K_p_hat_path(1)/total_pop_path(1)) / (Y_hat_path(1)/total_pop_path(1)));

            % 打印诊断结论
            k_diff = (K_p_hat_path(1)/total_pop_path(1)) - ss0.K_private_hat;
            fprintf('      [诊断]\n');
