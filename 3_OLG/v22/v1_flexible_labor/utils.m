classdef utils
    methods (Static)
                function cS = labor_setting_for_test(cS, chi_scalar_base)
            % =========================================================================
            % == 辅助函数: labor_setting_for_test (v1.0)
            % == 目的: 为 household.run_vfi_labor_supply_test 提供一个动态设置
            % ==       劳动负效用参数 chi 的接口。
            % =========================================================================
            if nargin < 2
                chi_scalar_base = 4.5; % 默认值
            end
            
            % --- 这部分逻辑与主代码中的 labor_setting 完全一致 ---
            chi_young_factor = 1.3;
            chi_old_factor = 1.5;   
            age_start_work = 1;
            age_peak_work_desire = 25;
            
            if isfield(cS, 'aR_new_path') && ~isempty(cS.aR_new_path)
                age_retire = cS.aR_new_path(1);
            else
                age_retire = cS.aR_new;
            end

            x = 1:cS.aD_new;
            chi_age_profile = ones(cS.aD_new, 1);
            
            idx_young = age_start_work:age_peak_work_desire;
            parabola_young = linspace(sqrt(chi_young_factor - 1), 0, length(idx_young)).^2 + 1;
            chi_age_profile(idx_young) = parabola_young;
            
            idx_old = age_peak_work_desire:age_retire;
            parabola_old = linspace(0, sqrt(chi_old_factor - 1), length(idx_old)).^2 + 1;
            chi_age_profile(idx_old) = parabola_old;
            
            chi_age_profile(age_retire+1:end) = chi_old_factor * 1.5; 
            
            % --- 核心部分: 使用传入的 chi_scalar_base 生成最终路径 ---
            cS.chi_pathV = chi_scalar_base * chi_age_profile;
        end
        ...
        % =========================================================================
            ...
            nu_h_values = [ ...
                ...
                1.52;  % 类型2: 城镇职工 - 中等弹性
                2;  % 类型3: 城乡居民 - 较低弹性
                2  % 类型4: 城乡居民 - 低弹性 (更接近生存需求，收入效应强)
            ];
            cS.nu_h = nu_h_values;
            cS.lGridV = linspace(0.1, 1, cS.nL)';

            % ===================== [最终平滑版修正] =====================
            % 修正 chi_pathV 的构建逻辑，使其在中年后单调递增至生命终点
            
            % 1. 定义基准参数
            chi_scalar_base = 7;       % 基准负效用权重 (根据您的调试，1.0是一个好的起点)
            chi_young_factor = 1.3;    % 年轻人厌恶程度乘数 (相对于最低点)
            chi_end_life_factor = 2.0; % [新] 生命终点厌恶程度乘数 (相对于最低点)
            
            % 2. 在物理年龄上定义新的剖面
            phys_ages_vec = (cS.age1_orig:cS.ageLast_orig)';
            chi_phys_profile = ones(length(phys_ages_vec), 1);
            
            % 定义关键物理年龄点
            age_peak_work_desire_phys = 45; % 工作意愿最强(chi最低)的物理年龄
            
            % 从物理年龄向量中找到对应的索引
            idx_start_work_phys = 1;
            idx_peak_desire_phys = find(phys_ages_vec == age_peak_work_desire_phys, 1);
            idx_last_age_phys = length(phys_ages_vec);

            % [新逻辑] 构建两段式抛物线
            
            % 第一段：工作初期到中年 (从高到低)
            % y = a*x^2 + b, 我们让它经过 (start, factor_young) 和 (peak, 1)
            idx_young_phys = idx_start_work_phys:idx_peak_desire_phys;
            % 使用 linspace 创建一个从 sqrt(factor-1) 到 0 的向量
            parabola_young = linspace(sqrt(chi_young_factor - 1), 0, length(idx_young_phys)).^2 + 1;
            chi_phys_profile(idx_young_phys) = parabola_young;
            
            % 第二段：中年到生命终点 (从低到高，平滑递增)
            % y = a*x^2 + b, 我们让它经过 (peak, 1) 和 (last, factor_end_life)
            idx_old_phys = idx_peak_desire_phys:idx_last_age_phys;
            % 使用 linspace 创建一个从 0 到 sqrt(factor-1) 的向量
            parabola_old = linspace(0, sqrt(chi_end_life_factor - 1), length(idx_old_phys)).^2 + 1;
            chi_phys_profile(idx_old_phys) = parabola_old;

            % 3. 将物理年龄剖面聚合到模型年龄组 (此部分逻辑不变)
            chi_age_profile_model = zeros(cS.aD_new, 1);
            for a = 1:cS.aD_new
                phys_ages_in_group = cS.physAgeMap{a};
                model_ages_indices_in_phys_vec = phys_ages_in_group - cS.age1_orig + 1;
                valid_indices = model_ages_indices_in_phys_vec(model_ages_indices_in_phys_vec <= length(chi_phys_profile));
                if ~isempty(valid_indices)
                    chi_age_profile_model(a) = mean(chi_phys_profile(valid_indices));
                end
            end

            % 4. 最终赋值
            cS.chi_pathV = chi_scalar_base * chi_age_profile_model;
        ...
        function cS = generate_PPS_path(cS, mode, graph_flag)
            ...
            % ==
            % == 目的:
            % ==   1. 模拟中国个人养老金(PPS)制度从2025年开始的缴费率演进路径。
            % ==   2. 基于国际经验设定长期稳态目标，并提供多种改革情景。
            % ==   3. 使用曲率可控的平滑函数模拟政策的逐步推广过程。
            % =========================================================================

            fprintf('--- 正在构建个人养老金(PPS)缴费率路径 (模式: %s) ---\n', mode);

            % --- 1. 定义所有情景的共性参数 ---
            reform_start_year = 2025; % 缴费率从2025年开始爬升
            initial_rate = 0.0;       % 2023-2024年的初始费率为0

            % --- 2. 根据模式设定情景参数 ---
            switch lower(mode)
                case 'conservative' % 保守情景
                    target_rate_final = 0.03; % 长期目标: 3% ( uptake 有限)
                    year_reach_final = 2055;  % 达到长期目标所需时间较长
                    curvature = 2.0;          % 初始增速较慢

                case 'moderate' % 温和情景 (基准)
                    target_rate_final = 0.06; % 长期目标: 6% (成为补充养老的重要部分)
                    year_reach_final = 2050;  % 中等速度达到目标
                    curvature = 1.8;          % 初始增速温和

                case 'ambitious' % 积极情景
                    target_rate_final = 0.09; % 长期目标: 9% (接近发达国家水平)
                    year_reach_final = 2045;  % 较快达到目标
                    curvature = 1.5;          % 初始增速相对较快

                otherwise
                    error('未知的PPS路径模式: %s. 可选模式: conservative, moderate, ambitious', mode);
            end

            fprintf('      [情景: %s] 初始费率=%.1f%%, 启动年份=%d, 最终目标=%.1f%% (于%d年达到)\n', ...
                mode, initial_rate*100, reform_start_year, target_rate_final*100, year_reach_final);
            ...
            % --- 4. 从年度路径采样得到模型期路径 ---
            model_sim_years = cS.start_year:cS.time_Step:cS.end_year;
            pps_fixed_path_model_period = interp1(annual_years_vector, full_pps_rate_path_annual, model_sim_years, 'cubic');

            % --- 5. 确保路径长度与 T_sim 一致并存入 cS ---
            T_sim = cS.T_sim;
            ...
            if nargin > 2 && graph_flag
                ...
                scenarios = {'conservative', 'moderate', 'ambitious'};
                colors = {'r', 'b', 'g'};
                ...
                for i = 1:length(scenarios)
                    ...
                    plot(plot_years, cS_temp.pps_fixed_path * 100, 'o-', 'Color', colors{i}, ...
