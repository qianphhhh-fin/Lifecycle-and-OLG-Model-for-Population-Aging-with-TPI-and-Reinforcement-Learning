...
classdef main_olg_v8_utils % 定义 main_olg_v8_utils 类
    ...
    methods (Static) % 定义静态方法
        ...
        function cS = ParameterValues_HuggettStyle()
            ...
            if length(cS.d_orig) ~= cS.aD_orig, error('年度死亡率数据 d_orig 长度与年龄跨度不匹配'); end
            ...
            for a = 1:cS.aD_new, cS.physAgeV_new(a) = cS.physAgeV_orig(cS.physAgeMap{a}(1)); end
            cS.s_1yr_transitionV = zeros(cS.aD_new, 1);
            for a = 1:(cS.aD_new - 1)
                lastYearIdxInGroup = cS.physAgeMap{a}(end);
                if lastYearIdxInGroup < cS.aD_orig, cS.s_1yr_transitionV(a) = cS.s_orig(lastYearIdxInGroup);
                else, cS.s_1yr_transitionV(a) = 0; end
            end
            ...
            if length(beta_surv_pop) ~= cS.aD_new - 1, error('年龄组间存活率 beta_surv_pop 长度不正确。'); end
            ...
            power_k = 1.5; % 重命名以避免与函数名冲突
            kGridV_temp_k = cS.kMin + (cS.kMax - cS.kMin) * (linspace(0, 1, cS.nk).^power_k); 
            if cS.nk > 0, kGridV_temp_k(1)=cS.kMin; end
            cS.kGridV = kGridV_temp_k(:);
            ...
            if length(cS.ageEffV_orig) ~= cS.aD_orig, error('ageEffV_orig 年度年龄效率剖面长度不匹配'); end
            cS.ageEffV_new = zeros(cS.aD_new, 1);
            for a = 1:cS.aD_new, cS.ageEffV_new(a) = mean(cS.ageEffV_orig(cS.physAgeMap{a})); end
            ...
            cS.pps_return_rate_premium = 0.08; % 示例调整       
            ...
            cS.pps_max_contrib_frac = 1.0; % 示例调整 (之前是0.12)            
            ...
            if cS.nkpps > 0, cS.kppsMax = max(cS.kppsMax, 1e-3); end
            power_kpps = 1.5;                           
            if cS.nkpps > 1
                kppsGridV_temp = cS.kppsMin + (cS.kppsMax - cS.kppsMin) * (linspace(0, 1, cS.nkpps).^power_kpps); 
                kppsGridV_temp(1) = cS.kppsMin;
            elseif cS.nkpps == 1, kppsGridV_temp = cS.kppsMin;
            else, kppsGridV_temp = []; end
            cS.kppsGridV = kppsGridV_temp(:);

            % --- 一般均衡求解参数 ---
            cS.max_iter_K_tau_l = 200;                 
            cS.tol_K_tau_l = 1e-4;                     
            cS.damp_K_v5 = 0.1;                        
            ...
            % ... (填充 pps_fixed_contrib_schedule_frac 的逻辑与之前相同) ...
            ...
            % --- 神经网络VFI近似参数 ---
            cS.use_NN_for_VFI = false; % 默认为false, 可在主脚本中修改以启用NN

            % % --- 单一网络预测 k_prime 和 c_pps (旧方式, 已注释) ---
            % cS.trained_nn_filename = 'trained_VFI_NN_v8.mat'; 
            % cS.normalization_params_filename = 'normalization_params_VFI_NN_v8.mat';

            % --- 三网络模型文件名 (新的推荐方式) ---
            cS.nn_three_networks_filename = 'trained_VFI_NN_v8_three_networks.mat';
            
            % --- 分阶段模型文件名 (旧方式，保留向后兼容) ---
            cS.nn_model_k_prime_filename = 'trained_VFI_NN_v8_k_prime.mat';
            cS.nn_model_c_pps_classifier_filename = 'trained_VFI_NN_v8_c_pps_classifier.mat';
            cS.nn_model_c_pps_regressor_filename = 'trained_VFI_NN_v8_c_pps_regressor.mat';
            
            % 归一化参数文件名 (假设与模型文件同名或从主模型文件加载)
            % HHSolution_NN_Huggett_two_stage 将从这些文件中加载norm params
            cS.nn_norm_params_k_prime_filename = 'trained_VFI_NN_v8_k_prime.mat'; 
            cS.nn_norm_params_c_pps_classifier_filename = 'trained_VFI_NN_v8_c_pps_classifier.mat';
            cS.nn_norm_params_c_pps_regressor_filename = 'trained_VFI_NN_v8_c_pps_regressor.mat';
            % 注意: 训练脚本 train_NN_VFI_...m 中保存的 .mat 文件需要包含相应的归一化参数结构体。

            fprintf('V8: 完整参数已设置完毕。\n');
            % ... (其他打印信息) ...
            fprintf('    PPS可遗赠 (cS.pps_bequeathable): %s\n', mat2str(cS.pps_bequeathable)); 
            fprintf('    是否使用NN进行VFI近似 (cS.use_NN_for_VFI): %s\n', mat2str(cS.use_NN_for_VFI));
        end
        ...
        function [K_sol_out, tau_l_sol_out, gbc_res_final_out, converged_and_feasible_out, solution_details_out] = solve_K_tau_l_for_rho_prime(...
                ...
                trainedNet_optional, norm_params_optional) % <<<< 新增可选参数: trainedNet_optional, norm_params_optional
            % solve_K_tau_l_for_rho_prime - V8模型的一般均衡求解器
            % (MODIFIED FOR PPS BEQUESTS and NN VFI OPTION)
            
            if nargin < 6, trainedNet_optional = []; end % Default if not provided
            if nargin < 7, norm_params_optional = []; end % Default if not provided
            ...
            if theta_payg_required_calc > cS_global.theta_payg_max + 1e-5 % Check against hard cap
            ...
            for iter_ktl_idx = 1:maxIter_ktl_loop
                ...
                theta_payg_actual_iter_val = theta_payg_required_calc; % Start with theoretically required
                % Apply total labor tax constraint
                ...
                theta_payg_actual_iter_val = max(0, theta_payg_actual_iter_val); % Ensure non-negative
                ...
                paramS_for_vfi_sim_iter = paramS_global_in; % paramS_global_in contains leGridV
                ...
                tol_vfi_tr_sub_iter = 1e-3 * (MPL_gross_iter_val + 1e-9); 
                ...
                for i_vfi_tr_sub_loop = 1:max_vfi_tr_sub_iter
                    % --- MODIFIED SECTION FOR NN VFI ---
                    if cS_global.use_NN_for_VFI && ...
                       isfield(nn_models_input, 'trainedNet_proportion') && ~isempty(nn_models_input.trainedNet_proportion) && ...
                       isfield(nn_models_input, 'total_input_dim') && isfield(nn_models_input, 'use_b_payg_as_input')
                        
                        [kPolM_vfi_temp, cPpsPolM_vfi_temp_choice, cPolM_vfi_temp] = ... 
                            main_olg_v8_utils.HHSolution_NN_Huggett(R_k_net_hh_factor_iter_val, MPL_gross_iter_val, ...
                            TR_total_for_vfi_guess_val, bV_payg_vec_iter_val, paramS_for_vfi_sim_iter, cS_global, ...
                            nn_models_input.trainedNet_proportion, nn_models_input.total_input_dim, nn_models_input.use_b_payg_as_input);
                    else
                        % Use full VFI
                        if cS_global.use_NN_for_VFI % Warning if NN was intended but not available
                             warning('solve_K_tau_l (inner loop): cS.use_NN_for_VFI is true, but NN or norm_params not provided. Falling back to full VFI for bequest iteration.');
                        end
                ...
                % If labor supply is endogenous and depends on tau_l, this GBC update for tau_l needs adjustment
                denominator_tau_l_update = MPL_gross_iter_val * L_per_capita_global;
                % Crude adjustment if PPS deductions affect tax base and are large (not fully modeled here)
                % if cS_global.pps_active && paramS_for_vfi_sim_iter.pps_tax_deferral_active
                %    % This would require aggregate PPS contributions, which we don't directly have here
                %    % For simplicity, we ignore this refinement in the GBC update step for tau_l
                % end
                if abs(denominator_tau_l_update) < 1e-9, denominator_tau_l_update = 1e-9 * sign(denominator_tau_l_update + 1e-12); end
                tau_l_dev_raw_for_update = -gbc_residual_iter_val / denominator_tau_l_update;
                ...
                elseif current_devNorm_val < tol_norm_ktl_loop && ...
                    ...
                    converged_and_feasible_out = false; % Not feasible if PAYG not funded as per target due to caps
            ...
            if ~converged_and_feasible_out && iter_ktl_idx == maxIter_ktl_loop
                ...
                tau_l_sol_out = tau_l_current_guess;
                gbc_res_final_out = gbc_residual_iter_val;
                if exist('MPL_gross_iter_val', 'var') && isfinite(MPL_gross_iter_val) % Populate details if loop ran
                    solution_details_out.R_mkt_gross_factor = R_mkt_gross_factor_iter_val;
                    solution_details_out.MPL_gross = MPL_gross_iter_val;
                    solution_details_out.theta_payg = theta_payg_actual_iter_val;
                    solution_details_out.b_payg = b_payg_iter_val;
                    ...
                    solution_details_out.K_model_pps = K_model_pps_sim_iter_val;
                    solution_details_out.K_model_non_pps = K_model_nonpps_sim_iter_val;
                else % Fallback if loop didn't run or critical vars are NaN
            ...
            if ~isfield(solution_details_out, 'theta_payg_required_before_cap') || ...
             ...
             if ~isfield(solution_details_out, 'K_model_pps') 
                if exist('K_model_pps_sim_iter_val','var') && isfinite(K_model_pps_sim_iter_val)
                    solution_details_out.K_model_pps = K_model_pps_sim_iter_val;
                else, solution_details_out.K_model_pps = NaN; end
            end
             if ~isfield(solution_details_out, 'K_model_non_pps') 
                 if exist('K_model_nonpps_sim_iter_val','var') && isfinite(K_model_nonpps_sim_iter_val)
                     solution_details_out.K_model_non_pps = K_model_nonpps_sim_iter_val;
                 else, solution_details_out.K_model_non_pps = NaN; end
            end
            if converged_and_feasible_out && isfield(solution_details_out, 'MPL_gross') && isfinite(solution_details_out.MPL_gross) && isfinite(K_sol_out)
                 solution_details_out.Y_model = cS_global.A * (K_sol_out^cS_global.alpha) * (L_per_capita_global^(1-cS_global.alpha));
            elseif ~isfield(solution_details_out,'Y_model') || isnan(solution_details_out.Y_model)
                 solution_details_out.Y_model = NaN; % Ensure Y_model exists
            end
        end
...
% end
   ...
   % ... (在 main_olg_v8_utils 类的方法中添加新函数)
    ...
    % --- V8 家庭问题核心函数：基于分阶段NN的策略预测 ---
    function [kPolM_nn, cPpsPolM_choice_nn, cPolM_q_nn] = HHSolution_NN_Huggett(...
            R_k_net_factor_current, w_gross_current, TR_total_current, bV_payg_current_age_vec, ...
            paramS_nn, cS_nn, trainedNet_pps_classifier, trainedNet_pps_regressor, trainedNet_k_prime_regressor, total_input_dim_nn, use_b_payg_as_input_nn)
        % HHSolution_NN_Huggett - 使用三个神经网络求解家庭问题
        % 
        % 输入：
        %   trainedNet_pps_classifier - PPS二分类器
        %   trainedNet_pps_regressor - PPS比例回归器 (可以为空)
        %   trainedNet_k_prime_regressor - k_prime比例回归器
        %   其他参数与VFI版本相同
        % 
        % 输出：
        %   kPolM_nn, cPpsPolM_choice_nn, cPolM_q_nn - 策略函数矩阵
        ...
        for a_idx = 1:cS_nn.aD_new
            ...
            % 1. PPS分类预测
            Y_pred_pps_binary_prob = predict(trainedNet_pps_classifier, X_batch_age);
            Y_pred_pps_binary_logical = (Y_pred_pps_binary_prob(:,2) > 0.5); % 第二列是'true'的概率
            
            % 2. PPS比例预测 (仅对分类器预测为正的样本)
            Y_pred_pps_proportion = zeros(num_states_current_age, 1);
            if ~isempty(trainedNet_pps_regressor) && sum(Y_pred_pps_binary_logical) > 0
                idx_pred_positive = find(Y_pred_pps_binary_logical);
                X_pred_positive = X_batch_age(idx_pred_positive, :);
                Y_pred_pps_subset = predict(trainedNet_pps_regressor, X_pred_positive);
                Y_pred_pps_subset = max(0, min(1, Y_pred_pps_subset)); % 确保在[0,1]范围内
                Y_pred_pps_proportion(idx_pred_positive) = Y_pred_pps_subset;
            end
            
            % 3. k_prime比例预测
            Y_pred_k_prime_proportion = predict(trainedNet_k_prime_regressor, X_batch_age);
            Y_pred_k_prime_proportion = max(0, min(1, Y_pred_k_prime_proportion));
            ...
            for ik = 1:cS_nn.nk
                for ikpps = 1:cS_nn.nkpps
                    for ie = 1:cS_nn.nw
                        ...
                        pred_pps_proportion = Y_pred_pps_proportion(count_fill);
                        pred_k_prime_proportion = Y_pred_k_prime_proportion(count_fill);
                        ...
                        if a_idx <= cS_nn.aR_new && cS_nn.physAgeMap{a_idx}(1) <= cS_nn.pps_contribution_age_max_idx && cS_nn.pps_active
                            ...
                            if current_gross_labor_income_nn > 1e-6
                                ...
                                actual_c_pps_final = pred_pps_proportion * max_permissible_cpps_nn;
                            end
                        end
                        cPpsPolM_choice_nn(ik, ikpps, ie, a_idx) = actual_c_pps_final;
                        
                        % 计算可用资源
                        current_k = cS_nn.kGridV(ik);
                        epsilon_val = paramS_nn.leGridV(ie);
                        [resources_for_c_k_prime, ~, ~] = main_olg_v8_utils.HHIncome_Huggett(...
                            current_k, R_k_net_factor_current, w_gross_current, TR_total_current, ...
                            current_b_payg_for_age, actual_c_pps_final, ...
                            a_idx, paramS_nn, cS_nn, epsilon_val);
                        
                        % 计算实际的k_prime
                        consumption_floor_spending = cS_nn.cFloor * (1 + cS_nn.tau_c);
                        ...
                        if resources_for_kprime_and_c_above_floor > 1e-6
                            actual_k_prime = cS_nn.kMin + pred_k_prime_proportion * resources_for_kprime_and_c_above_floor;
                        else
                            actual_k_prime = cS_nn.kMin;
                        end
                        kPolM_nn(ik, ikpps, ie, a_idx) = max(cS_nn.kMin, min(cS_nn.kMax, actual_k_prime));
                        
                        % 计算消费
                        consumption_expenditure_nn = resources_for_c_k_prime - kPolM_nn(ik, ikpps, ie, a_idx);
            ...
            % 在 main_olg_v8_utils.m 中
function [K_sol_out, tau_l_sol_out, gbc_res_final_out, converged_and_feasible_out, solution_details_out] = solve_K_tau_l_for_rho_prime_nn_twostage(...
        rho_prime_payg_target_input, K_init_guess_input, cS_global, paramS_global_in, eIdxM_global_sim_paths, ...
        nn_models_input) 
    ...
    if mass_workers_global > 1e-9
        ...
        if rho_prime_payg_target_input > 1e-9, theta_payg_required_calc = Inf; else, theta_payg_required_calc = 0; end
    end
    theta_payg_required_calc = max(0, theta_payg_required_calc);
    solution_details_out.theta_payg_required_before_cap = theta_payg_required_calc;
    ...
    if theta_payg_required_calc > cS_global.theta_payg_max + 1e-5 
        if ~isfield(paramS_global_in, 'suppress_initial_theta_print') || ~paramS_global_in.suppress_initial_theta_print
            fprintf('  solve_K_tau_l (V8-NN2stage): rho_prime_target=%.4f 导致理论theta_req=%.4f > theta_max=%.3f. 直接标记为不可行。\n', ...
                rho_prime_payg_target_input, theta_payg_required_calc, cS_global.theta_payg_max);
        end
        converged_and_feasible_out = false; K_sol_out = K_init_guess_input; tau_l_sol_out = tau_l_current_guess; gbc_res_final_out = Inf;
        solution_details_out.theta_payg = min(theta_payg_required_calc, cS_global.theta_payg_max);
        solution_details_out.MPL_gross = NaN; solution_details_out.R_mkt_gross_factor = NaN; solution_details_out.b_payg = NaN;
        solution_details_out.T_bequest_Model = NaN; solution_details_out.C_model = NaN; solution_details_out.Y_model = NaN;
        solution_details_out.K_model_pps = NaN; solution_details_out.K_model_non_pps = NaN;
    ...
    stagnation_counter_ktl = 0; prev_devNorm_ktl = Inf; tau_l_boundary_strike_count_ktl = 0;

    if ~isfield(paramS_global_in, 'suppress_inner_print_header') || ~paramS_global_in.suppress_inner_print_header
        fprintf('  solve_K_tau_l_for_rho_prime_nn_twostage (V8): rho_prime_target=%.4f (理论theta_req=%.4f), K_init=%.2f, tau_l_init=%.3f\n', ...
                rho_prime_payg_target_input, theta_payg_required_calc, K_current_guess, tau_l_current_guess);
        fprintf('  IterKTL | K_guess  | tau_l_gs | MPL_g    | theta_act| K_tot_mod| K_pps_mod| GBC_res  | K_dev    | tau_l_dev| Norm     | Improv   | Strikes  | Time (s) |\n');
        fprintf('  -----------------------------------------------------------------------------------------------------------------------------------------------------------\n');
    end
    ...
    for iter_ktl_idx = 1:maxIter_ktl_loop
        ...
        TR_total_for_vfi_guess_val = 0.01 * MPL_gross_iter_val; 
        if iter_ktl_idx > 1 && isfinite(T_bequest_model_iter_val), TR_total_for_vfi_guess_val = T_bequest_model_iter_val; end
        
        % <<<< 新增: 打印传递给家庭问题求解器的宏观参数 >>>>
        if cS_global.use_NN_for_VFI % 只在用NN时打印，避免干扰VFI的输出
            fprintf('  Iter %3d Macro Inputs for HH Problem: R_k_net_fac=%.4f, MPL=%.4f, tau_l=%.4f, theta_act=%.4f, TR_total=%.4f, b_payg (avg for retired)=%.4f\n', ...
                iter_ktl_idx, R_k_net_hh_factor_iter_val, MPL_gross_iter_val, tau_l_current_guess, theta_payg_actual_iter_val, TR_total_for_vfi_guess_val, b_payg_iter_val);
        end
        % <<<< 结束新增打印 >>>>

        max_vfi_tr_sub_iter = 5; tol_vfi_tr_sub_iter = 1e-3 * (MPL_gross_iter_val + 1e-9); 
        cPolM_4D_from_vfi_final = []; kPolM_4D_from_vfi_final = []; cPpsPolM_4D_choice_from_vfi_final = []; 
        TR_total_for_vfi_final_iter = TR_total_for_vfi_guess_val;

        for i_vfi_tr_sub_loop = 1:max_vfi_tr_sub_iter
            if cS_global.use_NN_for_VFI && ...
               isfield(nn_models_input, 'trainedNet_pps_classifier') && ~isempty(nn_models_input.trainedNet_pps_classifier) && ...
               isfield(nn_models_input, 'trainedNet_k_prime_regressor') && ~isempty(nn_models_input.trainedNet_k_prime_regressor) && ...
               isfield(nn_models_input, 'total_input_dim') && isfield(nn_models_input, 'use_b_payg_as_input')
                
                % 获取PPS回归器（可能为空）
                trainedNet_pps_regressor_local = [];
                if isfield(nn_models_input, 'trainedNet_pps_regressor')
                    trainedNet_pps_regressor_local = nn_models_input.trainedNet_pps_regressor;
                end
                
                [kPolM_vfi_temp, cPpsPolM_vfi_temp_choice, cPolM_vfi_temp] = ... 
                    main_olg_v8_utils.HHSolution_NN_Huggett(R_k_net_hh_factor_iter_val, MPL_gross_iter_val, ...
                    TR_total_for_vfi_guess_val, bV_payg_vec_iter_val, paramS_for_vfi_sim_iter, cS_global, ...
                    nn_models_input.trainedNet_pps_classifier, trainedNet_pps_regressor_local, ...
                    nn_models_input.trainedNet_k_prime_regressor, nn_models_input.total_input_dim, nn_models_input.use_b_payg_as_input);
            else
                if cS_global.use_NN_for_VFI
                     fprintf('    警告: solve_K_tau_l (迭代 %d, 子迭代 %d): NN fallback to full VFI.\n', iter_ktl_idx, i_vfi_tr_sub_loop);
                end
                [cPolM_vfi_temp, kPolM_vfi_temp, cPpsPolM_vfi_temp_choice, ~] = ... 
                    main_olg_v8_utils.HHSolution_VFI_Huggett(R_k_net_hh_factor_iter_val, MPL_gross_iter_val, ...
                    TR_total_for_vfi_guess_val, bV_payg_vec_iter_val, paramS_for_vfi_sim_iter, cS_global);
            ...
            TotalNonPPSBequests_pc_iter_val = sum(mean_non_pps_bequest_wealth(:) .* ageDeathMass_annual_iter_val(:));
            TotalPPSBequests_pc_iter_val = 0;
            if cS_global.pps_active && cS_global.pps_bequeathable && ~isempty(kPpsHistM_sim_for_bequest)
                mean_pps_bequest_wealth = mean(kPpsHistM_sim_for_bequest, 1);
                TotalPPSBequests_pc_iter_val = sum(mean_pps_bequest_wealth(:) .* ageDeathMass_annual_iter_val(:));
            end
            TotalBequests_pc_iter_val = TotalNonPPSBequests_pc_iter_val + TotalPPSBequests_pc_iter_val;
            T_bequest_model_new_iter_val = max(0, TotalBequests_pc_iter_val / (1 + paramS_global_in.popGrowthForDebt));
            ...
            TR_total_for_vfi_final_iter = T_bequest_model_new_iter_val; 
            if abs(T_bequest_model_new_iter_val - TR_total_for_vfi_guess_val) < tol_vfi_tr_sub_iter || ...
               i_vfi_tr_sub_loop == max_vfi_tr_sub_iter, break; end
            TR_total_for_vfi_guess_val = 0.5 * TR_total_for_vfi_guess_val + 0.5 * T_bequest_model_new_iter_val; 
        end
        T_bequest_model_iter_val = TR_total_for_vfi_final_iter; 
        ...
        K_model_nonpps_sim_iter_val = mean(kHistM_non_pps_sim_iter_val, 1) * paramS_global_in.Z_ss_norm_annual;
        K_model_pps_sim_iter_val = 0;
        if cS_global.pps_active && cS_global.pps_in_K && ~isempty(kPpsHistM_sim_iter_val)
            K_model_pps_sim_iter_val = max(0, mean(kPpsHistM_sim_iter_val, 1) * paramS_global_in.Z_ss_norm_annual);
        end
        K_model_from_sim_iter_val = max(1e-6, K_model_nonpps_sim_iter_val + K_model_pps_sim_iter_val);
        C_model_iter_val = mean(cHistM_sim_iter_val,1) * paramS_global_in.Z_ss_norm_annual;
        Y_for_gbc_iter_val = cS_global.A * (K_current_guess^cS_global.alpha) * (L_per_capita_global^(1-cS_global.alpha));
        G_val_target_iter_val = cS_global.gov_exp_frac_Y * Y_for_gbc_iter_val;
        B_val_target_iter_val = cS_global.gov_debt_frac_Y * Y_for_gbc_iter_val;
        ...
        if current_devNorm_val < tol_norm_ktl_loop && abs(gbc_residual_iter_val) < cS_global.gbc_tol_for_internal_loop && payg_fully_funded_by_actual_theta_check
            converged_and_feasible_out = true; K_sol_out = K_model_from_sim_iter_val; tau_l_sol_out = tau_l_current_guess; gbc_res_final_out = gbc_residual_iter_val;
            solution_details_out.R_mkt_gross_factor = R_mkt_gross_factor_iter_val; solution_details_out.MPL_gross = MPL_gross_iter_val;
            solution_details_out.theta_payg = theta_payg_actual_iter_val; solution_details_out.b_payg = b_payg_iter_val;
            solution_details_out.T_bequest_Model = T_bequest_model_iter_val; solution_details_out.C_model = C_model_iter_val;
            solution_details_out.Y_model = cS_global.A * (K_sol_out^cS_global.alpha) * (L_per_capita_global^(1-cS_global.alpha));
            solution_details_out.K_model_pps = K_model_pps_sim_iter_val; solution_details_out.K_model_non_pps = K_model_nonpps_sim_iter_val;
            fprintf('  solve_K_tau_l_nn_twostage (V8): K和tau_l成功收敛.\n'); break;
        elseif current_devNorm_val < tol_norm_ktl_loop && abs(gbc_residual_iter_val) < cS_global.gbc_tol_for_internal_loop && ~payg_fully_funded_by_actual_theta_check
             fprintf('  solve_K_tau_l_nn_twostage (V8): K, tau_l, GBC收敛, 但PAYG未完全覆盖. 标记为不可行.\n');
            converged_and_feasible_out = false; K_sol_out = K_model_from_sim_iter_val; tau_l_sol_out = tau_l_current_guess; gbc_res_final_out = gbc_residual_iter_val;
            solution_details_out.R_mkt_gross_factor = R_mkt_gross_factor_iter_val; solution_details_out.MPL_gross = MPL_gross_iter_val;
            solution_details_out.theta_payg = theta_payg_actual_iter_val; solution_details_out.b_payg = b_payg_iter_val;
            solution_details_out.T_bequest_Model = T_bequest_model_iter_val; solution_details_out.C_model = C_model_iter_val;
            solution_details_out.Y_model = Y_for_gbc_iter_val; solution_details_out.K_model_pps = K_model_pps_sim_iter_val;
            solution_details_out.K_model_non_pps = K_model_nonpps_sim_iter_val;
            break;
        end
        ...
        new_tau_l_unconstrained_val = tau_l_current_guess + damp_tau_l_ktl_loop * tau_l_dev_raw_for_update;
        tau_l_next_iter_constrained_val = max(cS_global.tau_l_min, min(cS_global.tau_l_max, new_tau_l_unconstrained_val));
        is_tau_l_at_boundary_now = (abs(tau_l_next_iter_constrained_val - cS_global.tau_l_max) < 1e-7 && new_tau_l_unconstrained_val >= cS_global.tau_l_max - 1e-7) || ...
                               (abs(tau_l_next_iter_constrained_val - cS_global.tau_l_min) < 1e-7 && new_tau_l_unconstrained_val <= cS_global.tau_l_min + 1e-7);
        if is_tau_l_at_boundary_now && abs(gbc_residual_iter_val) > cS_global.gbc_tol_for_internal_loop, tau_l_boundary_strike_count_ktl = tau_l_boundary_strike_count_ktl + 1;
        else, tau_l_boundary_strike_count_ktl = 0; end
        tau_l_current_guess = tau_l_next_iter_constrained_val;
        if tau_l_boundary_strike_count_ktl >= cS_global.max_tau_l_boundary_strikes && abs(gbc_residual_iter_val) > cS_global.gbc_tol_for_internal_loop
            fprintf('  警告 (V8-NN2stage): tau_l 在边界持续且GBC未平衡. 中止.\n'); converged_and_feasible_out = false; 
            solution_details_out.R_mkt_gross_factor = R_mkt_gross_factor_iter_val; solution_details_out.MPL_gross = MPL_gross_iter_val; % etc.
            break; 
        end
        if iter_ktl_idx > 1
            if norm_improvement_val < (cS_global.min_norm_improvement_frac * prev_devNorm_ktl) && current_devNorm_val > tol_norm_ktl_loop, stagnation_counter_ktl = stagnation_counter_ktl + 1;
            else, stagnation_counter_ktl = 0; end
        end
        prev_devNorm_ktl = current_devNorm_val;
        if stagnation_counter_ktl >= cS_global.max_stagnation_iters && current_devNorm_val > tol_norm_ktl_loop
            fprintf('  警告 (V8-NN2stage): 范数停滞. 中止.\n'); converged_and_feasible_out = false; 
            solution_details_out.R_mkt_gross_factor = R_mkt_gross_factor_iter_val; solution_details_out.MPL_gross = MPL_gross_iter_val; % etc.
            break; 
        end
    end 
    ...
    if ~converged_and_feasible_out && iter_ktl_idx == maxIter_ktl_loop
        fprintf('  警告 (V8-NN2stage): K和tau_l迭代达到最大次数或未达可行解.\n');
        K_sol_out = K_model_from_sim_iter_val; tau_l_sol_out = tau_l_current_guess; gbc_res_final_out = gbc_residual_iter_val;
    ...
    if ~isfield(solution_details_out, 'theta_payg_required_before_cap') || isnan(solution_details_out.theta_payg_required_before_cap)
         recalc_mass_retirees_final_sd = sum(paramS_global_in.ageMassV(cS_global.aR_new + 1 : cS_global.aD_new));
         recalc_theta_req_final_sd = 0;
         if mass_workers_global > 1e-9, recalc_theta_req_final_sd = rho_prime_payg_target_input * (recalc_mass_retirees_final_sd / mass_workers_global); end
         solution_details_out.theta_payg_required_before_cap = max(0, recalc_theta_req_final_sd);
    end
    if ~isfield(solution_details_out, 'K_model_pps'), solution_details_out.K_model_pps = K_model_pps_sim_iter_val; end
    if ~isfield(solution_details_out, 'K_model_non_pps'), solution_details_out.K_model_non_pps = K_model_nonpps_sim_iter_val; end
    if converged_and_feasible_out && isfield(solution_details_out, 'MPL_gross') && isfinite(solution_details_out.MPL_gross) && isfinite(K_sol_out)
         solution_details_out.Y_model = cS_global.A * (K_sol_out^cS_global.alpha) * (L_per_capita_global^(1-cS_global.alpha));
    elseif ~isfield(solution_details_out,'Y_model') || isnan(solution_details_out.Y_model)
         solution_details_out.Y_model = NaN;
    end
end
