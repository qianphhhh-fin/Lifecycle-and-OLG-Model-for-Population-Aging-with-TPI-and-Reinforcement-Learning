...
cS.ss0_year = 2023;
cS.start_year = 2023;
cS.pop_data_last_year = 2040; % 人口数据在2050年后进入稳态
...
cSF = cS;
cSF.pps_active = true; % 在这里决定终期稳态是否激活PPS
cSF.nkpps = 30; cSF.n_pps_rate_grid = 20;
...
if ~isempty(fieldnames(ssF))
    ...
    save_filename = 'SS/data_for_het_transition.mat';
    ...
    if cS.pps_active
        save(save_filename, 'data_for_transition_pps', '-v7.3');
    else
            save(save_filename, 'data_for_transition_nopps', '-v7.3');
    end
