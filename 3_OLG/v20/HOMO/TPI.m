...
% --- TPI.m ---
% --- TPI.m ---
function [target_paths, errors, Pol_path, Dist_path, aggr_supply] = calculate_paths_and_errors(r_path, w_hat_path, bequest_gen_raw_pc_path_guess, TR_hat_pc_path, b_hat_path, K_g_hat_path_pe_guess, ss0, ssF, cS, paramSF, valF, polF, dist0)
    % =========================================================================
    % ==          TPI核心计算引擎 (v8.9 - PPS制度开启衔接修正版)
    % ==
    % == 核心修正:
    % ==   - [!!! 关键BUG修复 !!!] 解决了当初始稳态(ss0)无PPS而过渡期
    % ==     有PPS时，初始分布(dist0)与路径分布(Dist_path)的维度不匹配问题。
    % ==   - 新增逻辑: 在TPI开始时，检查是否需要进行维度扩展。
    % ==   - 经济学假设: 当PPS制度在t=1开启时，所有家庭的初始PPS资产为零。
    % ==     这通过将无PPS的dist0人口完全放置在扩展后分布的第一个kpps切片
    % ==     (ikpps=1)上来实现。
    % =========================================================================
    T = cS.T_sim;
    ...
    pathS.g_A_path = g_A_path_period(1:T);
    ...
    % --- 3. [核心修正] 处理初始分布的维度衔接问题 ---
    dist0_for_sim = dist0;
    if cS.pps_active && size(dist0, 2) == 1 && cS.nkpps > 1
        % fprintf('   [TPI信息] 检测到从无PPS到有PPS的转轨，正在扩展初始分布dist0的维度...\n');
        
        % 检查 kpps 网格的第一个点是否为零
        if abs(cS.kppsGridV(1)) > 1e-9
            warning('TPI:PPS_Initialization', 'PPS制度开启时，kpps网格的第一个点不为零。模型假设所有初始PPS资产都集中在该点。');
        end
        
        dist0_expanded = zeros(cS.nk, cS.nkpps, cS.nw_expanded, cS.aD_new);
        % 将所有人口放置在第一个kpps切片上 (ikpps=1)，代表初始PPS资产为零
        dist0_expanded(:, 1, :, :) = dist0;
        dist0_for_sim = dist0_expanded;
    end
    
    % --- 4. 向前模拟分布 ---
    total_pop_t0 = sum(dist0, 'all');
    bequest_gen_total_t0 = ss0.Bequest_gen_hat_raw_ss; 

    bequest_gen_total_path_guess = bequest_gen_raw_pc_path_guess .* sum(cS.Z_path_raw, 1);
    
    bequest_gen_total_t_minus_1 = [bequest_gen_total_t0*sum(cS.Z_path_raw(:,1), 1), bequest_gen_total_path_guess(1:T-1)];
    % bequest_gen_total_t_minus_1 = bequest_gen_total_path_guess;
    ...
    bequest_per_newborn_hat_path(non_zero_newborns_mask) = ...
        bequest_gen_total_t_minus_1(non_zero_newborns_mask) ./ mass_newborns_path(non_zero_newborns_mask);

    Dist_path = distribution.simulate_dist_forward(dist0_for_sim, Pol_path, cS, paramSF, bequest_per_newborn_hat_path);
    ...
    % --- 6. 计算引致的新宏观价格和流量路径 (Target Paths) ---
    target_paths = struct();
    ...
    for t = 1:(T-1)
        current_Kg_total_hat = target_paths_K_g_hat_total(t); 
        investment_g_hat_t = I_g_hat_total_path_calc(t);
        bgp_growth_factor_t = (1 + g_A_path_period(t));
        
        target_paths_K_g_hat_total(t+1) = ((1 - cS.ddk_g) * current_Kg_total_hat + investment_g_hat_t) / bgp_growth_factor_t;
    end
    ...
    errors.b_hat = max(abs(target_paths.b_hat_path(update_range_flow) - b_hat_path(update_range_flow)));
...
function F_error_vec = tpi_error_wrapper(X_guess_flat, T, iter_vars_map, const_paths, ss0, ssF, cS, paramSF, valF, polF, dist0)
            % =========================================================================
            % == 函数: tpi_error_wrapper (非线性求解器专用包装器)
            % == 版本: [v3.0 - 最终定稿版]
            % ==
            % == 目的:
            % ==   1. 作为所有高级求解器(Newton, Broyden, fsolve)的目标函数。
            % ==   2. 接收一个扁平化的猜测向量 X_guess_flat。
            % ==   3. 精确地将 X_guess_flat 解包，并与边界值拼接成长度为 T 的完整路径。
            % ==   4. 调用核心引擎 TPI.calculate_paths_and_errors。
            % ==   5. 计算并返回一个与 X_guess_flat 维度完全一致的扁平化误差列向量。
            % =========================================================================
            
            % --- 1. 解包扁平化的猜测向量，并立即转置为行向量 ---
            r_path_iter                       = X_guess_flat(iter_vars_map.r)';
            w_hat_path_iter                   = X_guess_flat(iter_vars_map.w_hat)';
            bequest_gen_raw_pc_path_iter      = X_guess_flat(iter_vars_map.beq)';
            TR_hat_pc_path_iter               = X_guess_flat(iter_vars_map.tr)';
            b_hat_path_iter                   = X_guess_flat(iter_vars_map.b_hat)';
            K_g_hat_path_iter_segment         = X_guess_flat(iter_vars_map.k_g)'; % 这是 t=2 到 t=T-1 的路径

            % --- 2. 精确重组所有路径，确保其输入给核心引擎的长度为 T ---
            r_path_full         = [r_path_iter, ssF.r_mkt];
            w_hat_path_full     = [w_hat_path_iter, ssF.w_hat];
            b_hat_path_full     = [b_hat_path_iter, ssF.b_hat];
            
            % K_g路径由 t=1(固定), t=2...T-1(求解), t=T(固定) 拼接而成
            K_g_hat_pe_path_full = [const_paths.K_g_hat_path_t1, K_g_hat_path_iter_segment, ssF.K_public_hat];
            
            total_pop_ssF = sum(cS.Z_path_raw(:, end));
            bequest_gen_raw_pc_path_full = [bequest_gen_raw_pc_path_iter, ssF.Bequest_gen_hat_raw_ss / total_pop_ssF];
            TR_hat_pc_path_full       = [TR_hat_pc_path_iter, ssF.TR_distributed_agg / total_pop_ssF];
            
            % --- 3. 调用核心计算引擎 ---
            [target_paths, ~] = TPI.calculate_paths_and_errors(...
                r_path_full, w_hat_path_full, bequest_gen_raw_pc_path_full, ...
                TR_hat_pc_path_full, b_hat_path_full, K_g_hat_pe_path_full, ...
                ss0, ssF, cS, paramSF, valF, polF, dist0);

            % --- 4. 计算与求解变量完全对应的误差向量 ---
            % 所有流量变量的误差在 t=1...T-1 (长度 T-1)
            error_r     = target_paths.r_path(1:T-1)                - r_path_iter;
            error_w     = target_paths.w_hat_path(1:T-1)            - w_hat_path_iter;
            error_beq   = target_paths.bequest_gen_raw_pc_path(1:T-1) - bequest_gen_raw_pc_path_iter;
            error_tr    = target_paths.TR_hat_pc_path(1:T-1)        - TR_hat_pc_path_iter;
            error_b_hat = target_paths.b_hat_path(1:T-1)            - b_hat_path_iter;
            
            % K_g 的误差在 t=2...T-1 (长度 T-2)
            error_k_g   = target_paths.K_g_hat_path(2:T-1)            - K_g_hat_path_iter_segment;

            % --- 5. 组合成一个扁平化的误差列向量并返回 ---
            F_error_vec = [error_r'; error_w'; error_beq'; error_tr'; error_b_hat'; error_k_g'];
        end       

        function J = calculate_jacobian_numerical(error_func_handle, X_k)
            % =========================================================================
            % == 函数: calculate_jacobian_numerical (数值雅可比矩阵)
            % == 版本: [v2.2 - 进度条增强版]
            % ==
            % == 核心修改:
            % ==   - [!!! 新增功能 !!!] 在核心的 for 循环中加入了一个高效的
            % ==     命令行文本进度条，用于实时监控这个最耗时步骤的进展。
            % ==   - 对循环结构和变量名进行了微调，以提高可读性。
            % =========================================================================
            
            n = length(X_k);
            h = 1e-6; % 有限差分的步长
            
            fprintf('   [Broyden] 正在计算数值雅可比矩阵 (维度: %d x %d, 高效矩阵化方法)... \n', n, n);
            
            % --- 1. 构建所有正向和负向的扰动点矩阵 ---
            X_k_mat = repmat(X_k, 1, n);
            h_mat = diag(repelem(h, n));
            X_plus_h_mat  = X_k_mat + h_mat;
            X_minus_h_mat = X_k_mat - h_mat;
            
            % --- 2. 评估所有扰动点处的误差函数 ---
            F_plus_mat  = zeros(n, n);
            F_minus_mat = zeros(n, n);
            
            % [!!! 进度条初始化 !!!]
            fprintf('      Progress: [');
            progress_bar_width = 50; % 进度条的显示宽度
            last_progress_tick = -1;
            
            % 核心循环：这是不可避免的，但我们可以监控它的进度
            for j = 1:n
                % a. 对第j个变量的扰动，评估模型响应
                F_plus_mat(:, j)  = error_func_handle(X_plus_h_mat(:, j));
                F_minus_mat(:, j) = error_func_handle(X_minus_h_mat(:, j));
                
                % b. [!!! 进度条更新 !!!]
                current_progress_tick = floor((j / n) * progress_bar_width);
                if current_progress_tick > last_progress_tick
                    progress_chars = repmat('=', 1, current_progress_tick);
                    space_chars = repmat(' ', 1, progress_bar_width - current_progress_tick);
                    fprintf('%s%s] %6.2f%%', progress_chars, space_chars, (j/n)*100);
                    % 将光标移回进度条的开头，为下次更新做准备
                    if j < n
                        fprintf(repmat('\b', 1, progress_bar_width + 9)); % 9 = length('] 100.00%')
                    end
                    last_progress_tick = current_progress_tick;
                end
            end
