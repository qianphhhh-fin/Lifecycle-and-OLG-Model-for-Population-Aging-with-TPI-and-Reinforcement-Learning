...
classdef household
 ...
 function [polS_by_type, valS_by_type] = VFI_solver(M_vfi, paramS_vfi, cS_vfi)
            % =========================================================================
            % == 函数: VFI_solver
            % == 版本: [v3.3 - 显式类型参数版]
            % ==
            % == 核心修改:
            % ==   - [!!! 关键修正 !!!] 移除了隐式的 is_urban 判断逻辑。
            % ==   - 在循环开始前，显式地构建一个 theta_ss_by_type 向量，
            % ==     其中包含了每个类型的稳态养老金缴费率。
            % ==   - 在循环内部，直接从该向量中为 M_vfi_for_type.theta_t 赋值。
            % ==   - 这使得同质化测试更加可靠，并消除了任何可能由隐式逻辑
            % ==     引起的、难以察觉的行为差异。
            % =========================================================================
            
            polS_by_type = cell(cS_vfi.nTypes, 1);
            valS_by_type = cell(cS_vfi.nTypes, 1);
            
            % [!!! 核心修正 !!!] 在循环外显式构建分类型的参数
            theta_ss_by_type = zeros(cS_vfi.nTypes, 1);
            if cS_vfi.nTypes > 1
                for i_type = 1:cS_vfi.nTypes
                    is_urban = (i_type <= 2);
                    theta_ss_by_type(i_type) = is_urban * cS_vfi.theta_path_urban(end) + (1-is_urban) * cS_vfi.theta_path_resident(end);
                end
            else
                theta_ss_by_type(1) = cS_vfi.theta_path(end);
            end
            ...
            for i_type = 1:cS_vfi.nTypes
                ...
                M_vfi_for_type = M_vfi;
                % [!!! 核心修正 !!!] 直接从预计算的向量中赋值
                M_vfi_for_type.theta_t = theta_ss_by_type(i_type);
                
                valS = -Inf(cS_type.nk, cS_type.nkpps, cS_type.nw_expanded, cS_type.aD_new);
                polS_cell = cell(cS_type.aD_new, 1);

                b_payg_hat_age_vec = zeros(1, cS_type.aD_new);
                if isfield(M_vfi, 'b_hat_by_type')
                     b_hat_for_type = M_vfi.b_hat_by_type(i_type);
                     b_payg_hat_age_vec((cS_type.aR_new+1):end) = b_hat_for_type;
                end
                
                for a_idx = cS_type.aD_new : -1 : 1
                    vPrime_kkppse_next = [];
                    if a_idx < cS_type.aD_new
                        vPrime_kkppse_next = valS(:,:,:,a_idx+1);
                    end
                    b_age_val = b_payg_hat_age_vec(a_idx);
                    
                    [val_age, pol_age] = household.VFI_by_age_PPS(a_idx, vPrime_kkppse_next, M_vfi_for_type, b_age_val, paramS_vfi, cS_type);
                    
                    valS(:,:,:,a_idx) = val_age;
                    polS_cell{a_idx} = pol_age;
                end
                
                polS_by_type{i_type} = [polS_cell{:}];
                valS_by_type{i_type} = valS;
            end
        end        
