...
classdef distribution
    methods (Static)
        ...
        function Dist = get_ss_dist(polS, paramS, cS, Z_dist_for_type, bequest_per_newborn)
            % =========================================================================
            % == 函数: get_ss_dist
            % == 版本: [v4.0 - 接口统一版]
            % ==
            % == 核心修改:
            % ==   - 统一了函数签名，明确接收 Z_dist_for_type 和 bequest_per_newborn。
            % ==     Z_dist_for_type 是特定类型的、已经按权重缩放的人口分布。
            % =========================================================================

            if isfield(cS, 'pps_active') && cS.pps_active
                Dist = distribution.get_ss_dist_pps(polS, paramS, cS, Z_dist_for_type, bequest_per_newborn);
            else
                Dist = distribution.get_ss_dist_no_pps(polS, paramS, cS, Z_dist_for_type, bequest_per_newborn);
            end
        end

       function Dist = get_ss_dist_pps(polS, paramS, cS, Z_dist_for_type, bequest_per_newborn)
            % =========================================================================
            % == 函数: get_ss_dist_pps (v2.1 - 新生儿财富上限修正版)
            % == 核心修正:
            % ==   - [!!! 关键稳健性修正 !!!] 在分配新生儿财富前，检查
            % ==     bequest_per_newborn 是否超过资产网格的最大值。
            % ==   - 如果超过，则将其强制设为网格最大值 (kGridV(end))，
            % ==     并将所有质量直接分配到最后一个网格点上。
            % ==   - 这可以防止因人均遗赠过大导致的插值错误和数值不稳定问题。
            % =========================================================================
            nk = cS.nk;
            nkpps = cS.nkpps;
            nw = cS.nw_expanded;
            ...
            beq_per_newborn = bequest_per_newborn;
            k_grid_max = cS.kGridV(end);
            ...
            % [!!! 核心修正 !!!] 新生儿财富上限机制
            if beq_per_newborn >= k_grid_max
                % 如果人均遗赠超过网格上限，将所有质量直接放到最后一个点
                dist_newborn_cond_shape(nk, ikpps_newborn, 1:cS.nw) = prob_mass_to_distribute;
            else
                % 否则，正常进行网格插值
            ...
            Dist = Dist_cond .* reshape(Z_dist_for_type, [1, 1, 1, aD]);
        end
        
        function Dist = get_ss_dist_no_pps(polS, paramS, cS, Z_dist_for_type, bequest_per_newborn)
            % =========================================================================
            % == 函数: get_ss_dist_no_pps (v2.1 - 新生儿财富上限修正版)
            % == 核心修正:
            % ==   - [!!! 关键稳健性修正 !!!] 增加了与 get_ss_dist_pps 中
            % ==     完全相同的财富上限 (Capping) 机制。
            % =========================================================================
            nk = cS.nk;
            nw = cS.nw_expanded;
            aD = cS.aD_new;
            ...
            % [!!! 核心修正 !!!] 新生儿财富上限机制
            if beq_per_newborn >= k_grid_max
                dist_newborn_cond_shape(nk, 1, 1:cS.nw) = prob_mass_to_distribute;
            else
                [ik_lower, ~, w_upper] = utils.find_grid_and_weights(beq_per_newborn, cS.kGridV);
                w_lower = 1.0 - w_upper;
                ik_upper = min(nk, ik_lower + 1);
