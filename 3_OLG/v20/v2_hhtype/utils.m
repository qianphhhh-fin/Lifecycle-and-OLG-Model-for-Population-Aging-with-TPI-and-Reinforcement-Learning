    ...
    methods (Static)
        ...
        function cS = ParameterValues()
          ...
          % --- [新增] 异质性家庭类型定义 ---
            ...
            % --- 行为参数 (家庭偏好) ---
            % [修改] 将标量参数扩展为向量，允许按类型赋值
            cS.beta =  repmat(0.98, cS.nTypes, 1);       % 主观折现因子 (年化)。
            cS.sigma = 4;                              % 跨期替代弹性的倒数 (所有类型相同)。
            cS.phi_bequest = repmat(0, cS.nTypes, 1); % 遗赠效用权重。
            cS.cFloor = 0.005;                         % 最低消费水平。

            % --- 生产技术参数 ---
            cS.alpha = 0.35;                    % 产出中私人资本的份额 (资本产出弹性)。
            cS.gamma = 0.15;                    % 产出中公共资本的份额 (公共资本产出弹性)。
            ...
            cS.tau_c = 0.03;                    % 消费税率。
            cS.G_c_to_Y_ratio_ss = 0.03;        % [新增] 稳态下，政府消费占GDP的目标比率。
            cS.I_g_to_Y_ratio_ss = 0.1;
            ...
            % --- [核心修改] 调用函数以生成特定于类型的路径 ---
            cS = population.generate_mortality_path(cS); % 生成死亡率数据 (未来可修改为类型特定)
        ...
        function cS = age_efficiency(cS)
            ...
            cS.ageEff_by_type = zeros(cS.nTypes, cS.aD_new);
            for i_type = 1:cS.nTypes
                ...
                cS.ageEff_by_type(i_type, :) = ageEff_type_i_new';
            end
        ...
        function cS = calcaulte_theta_payg_path(cS, graph_flag)
            ...
            % --- 1. 定义关键参数的【起点】与【终点】 ---
            theta_urban_initial = 0.06;   % 初始值: 基于2023年 16%+8% 的法定费率
            theta_urban_final   = 0.06;  % 长期目标: 参考澳大利亚等国的成熟DC体系费率

            theta_resident_initial = 0.06; % 初始值: 对当前名义缴费的合理有效费率估算
            theta_resident_final   = 0.06; % 长期目标: 假设保基本的作用略有增强
            ...
            cS.theta_path_urban = theta_path_urban;
            cS.theta_path_resident = theta_path_resident;
        ...
        function report = check_k_grid_limit(Dist, kGridV, ss_or_t_str, grid_name, graph_flag, quantiles_to_plot)
            % =========================================================================
            % == 函数: check_k_grid_limit (版本 v2.0 - 分年龄分位数可视化版)
            % == 目的: 诊断资产分布，可视化每个年龄组的资产中位数和其他分位数。
            % == 核心修改:
            % ==   - [!!! 功能重构 !!!] 函数的核心功能从检查网格上限，转变为计算并
            % ==     可视化每个年龄组(age cohort)的资产分位数。
            % ==   - 新增第6个参数 `quantiles_to_plot` (默认 [0.25, 0.5, 0.75, 0.9])，
            % ==     允许用户指定要计算和绘制的分位数。
            % ==   - 当 `graph_flag` 为 `true` 时，函数会生成一个图表，其x轴为模型年龄，
            % ==     y轴为资本水平，图中绘制多条线，每条线代表一个分位数（如中位数、90分位等）
            % ==     的生命周期路径。
            % ==   - 此图表对于理解生命周期储蓄行为、财富不平等以及网格是否足够大
            % ==     提供了极为深刻的洞察。
            % ==   - 原有的网格上限检查功能被简化并保留在文本报告中。
            % =========================================================================
            
            % --- 1. 参数初始化与默认值设定 ---
            if nargin < 6
                quantiles_to_plot = [0.25, 0.50, 0.75, 0.90];
            end
            if nargin < 5
                graph_flag = false;
            end
            ...
            mass_on_last_point = sum(Dist(end, :, :, :, :), 'all'); % Works for 4D and 5D
            percentage_on_last_point = (mass_on_last_point / total_mass) * 100;
            
            fprintf('\n   --- [诊断报告] %s 网格检查 (对象: %s) ---\n', grid_name, ss_or_t_str);
            fprintf('      网格上限 (k_max): %.2f\n', kGridV(end));
            fprintf('      挤在最后一个网格点上的总人口占比: %.4f %%\n', percentage_on_last_point);
            if percentage_on_last_point > 0.5
                fprintf('      >>> [警告] 网格上限可能不足！\n');
            else
                fprintf('      >>> [✅ 通过] 网格上限设置在合理范围内。\n');
            end
            report = sprintf('Percentage on last point for %s: %.4f%%\n', grid_name, percentage_on_last_point);

            % --- 3. [核心功能] 计算并绘制分位数生命周期剖面 ---
            if ~graph_flag
                return; % 如果不要求绘图，到此结束
            end
            
            dims = ndims(Dist);
            if dims == 5 % 暂不支持对转轨路径绘制分位数图
                fprintf('      >>> [注意] 暂不支持为5维转轨路径数据绘制分年龄分位数图。跳过绘图。\n');
                return;
            end

            aD = size(Dist, 4);
            num_quantiles = length(quantiles_to_plot);
            quantile_values_by_age = zeros(aD, num_quantiles);
            
            % --- 逐个年龄组计算分位数 ---
            for ia = 1:aD
                % 提取当前年龄组的分布 (nk x nkpps x nw)
                dist_age_cohort = squeeze(Dist(:,:,:,ia));
                
                % 将其在 k 维度上展开
                dist_k_by_age = squeeze(sum(dist_age_cohort, [2, 3]));
                
                % 计算累积分布函数 (CDF)
                total_mass_age = sum(dist_k_by_age);
                if total_mass_age < 1e-9, continue; end
                
                cdf = cumsum(dist_k_by_age) / total_mass_age;
                
                % 为每个所需的分位数找到对应的资本水平
                for iq = 1:num_quantiles
                    quantile_level = quantiles_to_plot(iq);
                    % 找到CDF中第一个超过分位数的点的索引
                    idx = find(cdf >= quantile_level, 1, 'first');
                    if isempty(idx)
                        quantile_values_by_age(ia, iq) = kGridV(end);
                    else
                        % (可选) 可以在两个网格点之间进行线性插值以获得更精确的值
                        if idx > 1 && cdf(idx) > quantile_level
                            w = (quantile_level - cdf(idx-1)) / (cdf(idx) - cdf(idx-1));
                            quantile_values_by_age(ia, iq) = kGridV(idx-1) + w * (kGridV(idx) - kGridV(idx-1));
                        else
                            quantile_values_by_age(ia, iq) = kGridV(idx);
                        end
                    end
                end
            end
            ...
            % --- 生成可视化图表 ---
            figure('Name', sprintf('Age-profiles of %s Quantiles (%s)', grid_name, ss_or_t_str), 'Position', [100, 100, 900, 600]);
            
            plot(1:aD, quantile_values_by_age, 'LineWidth', 2, '-o', 'MarkerSize', 4);
            
            legend_str = cell(1, num_quantiles);
            for iq = 1:num_quantiles
                legend_str{iq} = sprintf('%.0fth Quantile', quantiles_to_plot(iq) * 100);
            end
            
            title(sprintf('Life-cycle Profiles of %s Quantiles (%s)', grid_name, ss_or_t_str), 'FontSize', 14);
            xlabel('Model Age');
            ylabel(sprintf('%s Level', grid_name));
            legend(legend_str, 'Location', 'best');
            grid on;
            
            fprintf('   >>> [可视化] 已生成 %s 按年龄划分的资产分位数图。\n', grid_name);
