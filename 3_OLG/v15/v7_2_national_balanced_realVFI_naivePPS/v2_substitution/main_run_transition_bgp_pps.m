...
% --- [新增核心修改] 设定养老金模式 ---
cS.endogenous_theta_mode = false; % <--- 新增开关，true表示DB模式，false表示DC模式
if cS.endogenous_theta_mode
    cS.endogenous_theta_mode = true;
    cS.payg_replacement_rate = 0.15;
...
% --- 步骤 2.1: 为理论稳态设定外生环境 ---
cS.n_ss = -0.015;     % [新增] 设定长期人口年增长率 (0.0 表示ZPG)
...
if cS.endogenous_theta_mode
    ...
    % 在DC模式下，我们使用一个预设的合理值
    theta_steady_state = 0.2; % 使用参数文件中的值
...
[ss, Dist, polS, valS] = ...
    main_steady_state_utils_bgp.solve_steady_state_unified(cS, paramS, params_for_ss, true, [], 'fsolve_transform');
toc;
