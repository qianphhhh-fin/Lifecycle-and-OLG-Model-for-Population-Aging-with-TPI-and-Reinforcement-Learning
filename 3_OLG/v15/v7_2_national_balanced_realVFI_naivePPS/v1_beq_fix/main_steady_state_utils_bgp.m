classdef main_steady_state_utils_bgp
    ...
    methods (Static)
        ...
        function [ss, Dist, polS, valS] = calculate_aggregates_unified(K_private_total_guess, K_g_guess, L_guess, Bequest_Total_guess, Z_ss_norm, cS, paramS)
            ...
            % ==
            % == 核心修改:
            % ==   - [!!! 根本性修正 !!!] 从私人部门可支配总收入和储蓄的计算中
            % ==     彻底移除了遗赠转移项 (Bequest_Total_guess)。
            % ==   - 遗赠作为纯粹的财富再分配，在私人部门内部的宏观加总中相互抵消。
            % ==     它影响微观决策，但不应出现在宏观储蓄的定义中。
            % ==   - 此修改将确保国民收入核算(Y=C+I+G)最终闭合。
            % =========================================================================
        ...
        function [val_age, pol_age] = HHSolutionByAge_VFI_PPS(a_idx, vPrime_kkppse_next, M_age, b_age_val, beq_transfer_val, paramS_age, cS)
            ...
            if a_idx == cS.aD_new % 终期决策不变
                ...
                if isfield(cS,'phi_bequest') && cS.phi_bequest > 0
                    phi_effective = cS.beta * cS.phi_bequest;
                    c_over_k_ratio = phi_effective^(-1/cS.sigma);
                    k_prime_final_mat = total_wealth_mat ./ (1 + (1+cS.tau_c)*c_over_k_ratio);
                    c_expend_final_mat = total_wealth_mat - k_prime_final_mat;
                end
            ...
            for ie = 1:cS.nw_expanded
                for ikpps = 1:cS.nkpps
                    ...
                    val_age(:, ikpps, ie) = best_val_vec;
                    pol_age.c(:, ikpps, ie) = c_final_vec;
                    pol_age.k_prime(:, ikpps, ie) = k_prime_final_optimal_vec;
                    pol_age.kpps_prime(:, ikpps, ie) = kpps_prime_final;
                    pol_age.tax_regular(:, ikpps, ie) = capital_tax_vec + labor_tax + c_final_vec * cS.tau_c;
                    pol_age.tax_payg(:, ikpps, ie) = payg_tax;
                    pol_age.shock_exp(:, ikpps, ie) = shock_exp_vec;
                    pol_age.pension_out(:, ikpps, ie) = pension_out;
                    pol_age.pps_contrib(:, ikpps, ie) = pps_contrib;
                    pol_age.pps_withdrawal(:, ikpps, ie) = pps_withdrawal;
                    pol_age.pps_tax(:, ikpps, ie) = pps_tax;
                    pol_age.beq_received(:, ikpps, ie) = beq_transfer_val;
                end
            end
        end
        ...
        function display_national_accounts_unified(ss, cS, Dist_to_verify, polS, paramS)
            ...
            % ==
            % == 核心修改:
            % ==   - [整合] 将 `verify_FULL_steady_state` 的功能完全整合进本报告，
            % ==     作为【诊断 3】，自动进行最严格的稳态分布自洽性检验。
            % ==   - [新增] 增加【诊断 4】，对家庭和政府两大部门的预算进行检验，
            % ==     确保“总流入 = 总流出”在会计上严格成立。
            % ==   - 函数签名已更新，需要从调用处传入 Dist, polS, paramS。
            % =========================================================================
                ...
                for ie = 1:cS.nw_expanded
                    for ikpps = 1:cS.nkpps
                        for ik = 1:cS.nk
                            ...
                            if mass < 1e-30, continue; end
                            k_p_cont = polS(ia).k_prime(ik, ikpps, ie);
                            if cS.pps_active && cS.nkpps > 1, kpps_p_cont = polS(ia).kpps_prime(ik, ikpps, ie);
                            else, kpps_p_cont = cS.kppsGridV(1); end
                            ...
                            % 分裂概率质量
                            mass_chunk = mass * w_k_lower * w_kpps_lower;
                            if mass_chunk > 1e-30, dist_ia_plus_1_unscaled(ik_lower, ikpps_lower, :) = dist_ia_plus_1_unscaled(ik_lower, ikpps_lower, :) + mass_chunk * trans_probs_vec; end
                            if ik_lower ~= ik_upper, mass_chunk = mass * w_k_upper * w_kpps_lower; if mass_chunk > 1e-30, dist_ia_plus_1_unscaled(ik_upper, ikpps_lower, :) = dist_ia_plus_1_unscaled(ik_upper, ikpps_lower, :) + mass_chunk * trans_probs_vec; end, end
                            if ikpps_lower ~= ikpps_upper, mass_chunk = mass * w_k_lower * w_kpps_upper; if mass_chunk > 1e-30, dist_ia_plus_1_unscaled(ik_lower, ikpps_upper, :) = dist_ia_plus_1_unscaled(ik_lower, ikpps_upper, :) + mass_chunk * trans_probs_vec; end, if ik_lower ~= ik_upper, mass_chunk = mass * w_k_upper * w_kpps_upper; if mass_chunk > 1e-30, dist_ia_plus_1_unscaled(ik_upper, ikpps_upper, :) = dist_ia_plus_1_unscaled(ik_upper, ikpps_upper, :) + mass_chunk * trans_probs_vec; end, end, end
                        end
                    end
                end
                target_mass = Z_theory(ia+1); current_mass_generated = sum(dist_ia_plus_1_unscaled(:));
                if current_mass_generated > 1e-30, Dist_recalculated(:, :, :, ia+1) = dist_ia_plus_1_unscaled * (target_mass / current_mass_generated); end
            end
            if sum(Dist_recalculated(:)) > 1e-9, Dist_recalculated = Dist_recalculated / sum(Dist_recalculated(:)); end

            % 步骤 C: 比较并报告结果
            max_diff_dist = max(abs(Dist_to_verify - Dist_recalculated), [], 'all');
            fprintf('   重新计算的分布与求解器分布的最大差异: %.4e\n', max_diff_dist);
            if max_diff_dist < 1e-9
                fprintf('   ✅ [结论] 分布自洽性通过！VFI策略与分布演化逻辑完全一致。\n');
            else
                fprintf('   ⚠️ [结论] 分布自洽性失败！请检查VFI与分布演化代码的一致性。\n');
            end

            % --- IV. [新增] 部门预算检验 ---
            fprintf('\n--- [诊断 4] 部门预算闭合检验 (总流入 = 总流出) ---\n');

            % a. 家庭部门 (Household Sector)
            % 注: 遗赠是家庭部门内部的转移，不计入与其他部门的净流量
            ...
            fprintf('   部门1: 家庭 [总流入] ................ %12.8f\n', HH_Inflows);
            fprintf('   部门1: 家庭 [总流出] ................ %12.8f\n', HH_Outlays);
            fprintf('   => 家庭部门预算缺口 .................. %12.4e\n', HH_Gap);
            if abs(HH_Gap) < 1e-7, fprintf('      ✅ 家庭预算闭合。\n'); else, fprintf('      ⚠️ 家庭预算不闭合！\n'); end

            % b. 政府部门 (Government Sector)
            ...
            fprintf('   部门2: 政府 [总流入] ................ %12.8f\n', Gov_Inflows);
            fprintf('   部门2: 政府 [总流出] ................ %12.8f\n', Gov_Outlays);
            fprintf('   => 政府部门预算缺口 .................. %12.4e\n', Gov_Gap);
...
%             % == 版本: [v3.1 - 【OG-Core理论对齐版】]
%             % == 核心修改:
%             % ==   - [!!!] 将收到的遗赠转移支付 `beq_transfer_val` 重新加入到
%             % ==     `cash_on_hand` 和 `total_wealth` 的计算中。
%             % ==   - 这使得家庭决策能够反映遗赠收入带来的财富效应，与OG-Core
%             % ==     理论(pdf, Eq.22)保持一致。
%             % ==   - 同时，在策略结构体 polS 中正确记录收到的遗赠。
%             % =========================================================================
%
%             % --- 1. 初始化 ---
%             nk_search_grid = 150;
...
%                 'beq_received', zeros(cS.nk, 1, cS.nw_expanded)); % 注意：这里名字拼错了，应该是 pension_in, 但为了兼容性暂不修改
%
%             % --- 2. 预计算和插值器设置 ---
%             g_A_period = (1 + cS.g_A_ss)^cS.time_Step - 1;
%             market_return_factor = 1 + M_age.r_mkt_t;
...
%                         % [核心修正] 正确记录收到的遗赠
%                         pol_age_slice.beq_received(ik, 1, ie) = beq_transfer_val;
%                     end
%                 end
%             end
%             % --- 5. 扩展维度以匹配主循环 ---
...
%             % == 版本: [v3.1 - 【OG-Core理论对齐版】]
%             % == 核心修改:
%             % ==   - [!!!] 与 noPPS 版本完全对齐，将收到的遗赠转移支付
%             % ==     `beq_transfer_val` 重新加入到 `cash_on_hand` 和 `total_wealth`
%             % ==     的计算中，以确保理论一致性。
%             % =========================================================================
%
%             % --- 1. 初始化 ---
...
%                             % [核心修正] 正确记录收到的遗赠
%                             pol_age.beq_received(ik, ikpps, ie) = beq_transfer_val;
%                         end
%                     end
%                 end
%             end
