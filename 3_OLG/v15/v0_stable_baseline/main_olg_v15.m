...
% 将时间轴参数和绘图开关传递给新函数
cS = main_olg_v15_utils.calcaulte_theta_payg_path(cS, true);
fprintf('✅ 复合有效缴费率路径构建完成。\n');

%% 2. [v14核心] 加载外生数据路径
[Z_path, A_path, T] = main_olg_v15_utils.load_exogenous_paths(cS);
...
for iter = 1:max_iter
    ...
    [~, L_1_main] = main_olg_v15_utils.LaborSupply_Huggett(eIdxM, cS, paramS, Z_1);
    [~, ~, Y_annual_new] = main_olg_v15_utils.HHPrices_Huggett(K_physical_target, L_1_main, A_1, cS);
...
% 获取最终的价格和宏观变量
M_1 = main_olg_v15_utils.get_prices_and_policy_at_t(1, paths.K_pvt(1), paths.B_p(1), paths.K_pps(1), paths.B_g(1), Z_1, A_1, cS, paramS, eIdxM);
paths.L(1) = M_1.L_t;
paths.w(1) = M_1.w_t;
...
for t = 1:(T-1)
    ...
    M_t = main_olg_v15_utils.get_prices_and_policy_at_t(t, paths.K_pvt(t), paths.B_p(t), paths.K_pps(t), paths.B_g(t), Z_path(:,t), A_path(t), cS, paramS, eIdxM);
    paths.Y(t) = M_t.Y_t;
    paths.w(t) = M_t.w_t;
    ...
    [K_pvt_next, K_pps_next, C_t, Total_Cpps_t, Total_PpsTax_t, accidental_bequest_t] = ...
        main_olg_v15_utils.simulate_private_capital_forward(M_t, Z_path(:,t), cS, paramS, eIdxM, TR_total_t);
    ...
    [paths.B_p(t+1), PensionSurplus_t] = main_olg_v15_utils.update_pension_fund(paths.B_p(t), M_t, Z_path(:,t), cS);
    paths.B_g(t+1) = main_olg_v15_utils.update_gov_debt(paths.B_g(t), paths.C(t), M_t, Total_Cpps_t, paths.K_pps(t), Total_PpsTax_t, cS, PensionSurplus_t);
