% =========================================================================
% == 在 main_olg_v15_utils.m 文件中，请使用以下代码块 ==
% == 替换或添加相应的函数。                            ==
% =========================================================================
...
classdef main_olg_v15_utils
    ...
    methods (Static)
        ...
        function cS = ParameterValues_HuggettStyle()
            ...
            % --- [核心修正] 增加确定性重大支出冲击参数 ---
            cS.shock_active = true; % 是否激活此冲击
            cS.shock_frac = 0.9;   % 冲击支出占当期税后净现金流的比例
            cS.shock_age_young_start_orig = 27; % 青年期冲击开始年龄 (买房、生育)
            cS.shock_age_young_end_orig = 32;   % 青年期冲击结束年龄
            cS.shock_age_old_start_orig = 70;   % 老年期冲击开始年龄 (健康支出)
            cS.shock_age_old_end_orig = 74;   % 老年期冲击开始年龄 (健康支出)

            % --- [核心修正] 将冲击的物理年龄映射到模型年龄组索引 ---
            if cS.shock_active
                young_shock_ages = cS.shock_age_young_start_orig:cS.shock_age_young_end_orig;
                old_shock_ages = cS.shock_age_old_start_orig:cS.shock_age_old_end_orig;

                cS.shock_age_young_idx = [];
                cS.shock_age_old_idx = [];

                for a = 1:cS.aD_new
                    model_age_group_phys_ages = cS.physAgeV_orig(cS.physAgeMap{a});
                    if any(ismember(model_age_group_phys_ages, young_shock_ages))
                        cS.shock_age_young_idx = [cS.shock_age_young_idx, a];
                    end
                    if any(ismember(model_age_group_phys_ages, old_shock_ages))
                        cS.shock_age_old_idx = [cS.shock_age_old_idx, a];
                    end
                end
                cS.shock_age_young_idx = cS.shock_age_young_idx(end);
                cS.shock_age_old_idx = cS.shock_age_old_idx(end); % 选择多个匹配组的最后一个
                fprintf('重大支出冲击已激活。青年期冲击年龄组: %s; 老年期冲击年龄组: %s\n', num2str(unique(cS.shock_age_young_idx)), num2str(unique(cS.shock_age_old_idx)));
            end
        ...
        function [ss, eq_found, initial_dist_k, initial_dist_kpps] = solve_steady_state_with_fund(Z_ss_norm, B_p_Y_ratio_target, cS, paramS, eIdxM)
            ...
            [~, L_ss] = main_olg_v15_utils.LaborSupply_Huggett(eIdxM, cS, paramS_ss_for_L, Z_ss_norm);
            ...
            for iter = 1:max_iter
                ...
                M_ss_prices = main_olg_v15_utils.get_prices_at_t(K_guess, L_ss, A_ss, cS);
                ...
                if ~cS_ss.pps_active
                    cS_ss.nkpps = 1; cS_ss.npps = 1; cS_ss = main_olg_v15_utils.generateGrids(cS_ss);
                end

                % 3. [核心修改] 调用新函数，在给定价格下聚合储蓄
                [K_pvt_model, K_pps_model, ~] = main_olg_v15_utils.aggregate_household_savings(M_ss, Z_ss_norm, cS_ss, paramS, eIdxM);
            ...
            M_final_prices = main_olg_v15_utils.get_prices_at_t(K_final, L_ss, A_ss, cS);
            ...
            if ~cS_final.pps_active
                cS_final.nkpps = 1; cS_final.npps = 1; cS_final = main_olg_v15_utils.generateGrids(cS_final);
            end

            % 获取最终的策略和分布
            tr_per_survivor_eq = 0;
            [cPolM, kPolM, cPpsPolM, ~] = main_olg_v15_utils.HHSolution_VFI_Huggett(M_final, tr_per_survivor_eq, paramS, cS_final);
            [kHistM, kPpsHistM, cHistM, ~] = main_olg_v15_utils.HHSimulation_olgm(kPolM, cPpsPolM, cPolM, eIdxM, M_final, paramS, cS_final, tr_per_survivor_eq);
            initial_dist_k = kHistM;
            initial_dist_kpps = kPpsHistM;

            % 使用最终分布聚合
            [K_pvt_final, K_pps_final, C_final] = main_olg_v15_utils.aggregate_household_savings(M_final, Z_ss_norm, cS_final, paramS, eIdxM);
        ...
        function [K_pvt_agg, K_pps_agg, C_agg] = aggregate_household_savings(M_prices_given, Z_dist, cS_agg, paramS, eIdxM)
            ...
            %    假设意外遗赠为0，这是稳态下的标准做法。
            tr_per_survivor_eq = 0;
            [cPolM, kPolM, cPpsPolM, ~] = main_olg_v15_utils.HHSolution_VFI_Huggett(M_prices_given, tr_per_survivor_eq, paramS, cS_agg);

            % 2. 模拟家庭生命周期决策
            %    在稳态下，家庭的初始资产分布是内生的，所以我们从0开始模拟。
            %    注意：HHSimulation_olgm 返回的是一个历史矩阵，其中 kHistM(:,a) 是年龄a期初的资产。
            %    我们需要的是他们选择的下一期资产 k'，即 kHistM(:, a+1)。
            [kHistM, kPpsHistM, cHistM, ~] = main_olg_v15_utils.HHSimulation_olgm(kPolM, cPpsPolM, cPolM, eIdxM, M_prices_given, paramS, cS_agg, tr_per_survivor_eq);
        ...
        function [cPolM, kPolM, cPpsPolM, valM] = HHSolution_VFI_Huggett(M_vfi, tr_per_capita_vfi, paramS_vfi, cS_vfi)
            ...
            for a_idx = cS_vfi.aD_new : -1 : 1
                ...
                [cPolM(:,:,:,a_idx), kPolM(:,:,:,a_idx), cPpsPolM(:,:,:,a_idx), valM(:,:,:,a_idx)] = ...
                    main_olg_v15_utils.HHSolutionByAge_VFI_Vectorized(a_idx, vPrime_kkppse_next, M_vfi, tr_per_capita_vfi, bV_payg_vfi(a_idx), paramS_vfi, cS_vfi);
                % [cPolM(:,:,:,a_idx), kPolM(:,:,:,a_idx), cPpsPolM(:,:,:,a_idx), valM(:,:,:,a_idx)] = ...
                %     main_olg_v15_utils.HHSolutionByAge_NAIVE(a_idx,  M_vfi, tr_per_capita_vfi, bV_payg_vfi(a_idx), paramS_vfi, cS_vfi);
            end
        end
        ...
        function [cPol_age_q, kPol_age, cPpsPol_age_choice, val_age] = HHSolutionByAge_VFI_GridSearch(a_idx, vPrime_kkppse_next, M_age, tr_per_capita_age, b_age_val, paramS_age, cS)
            ...
            if a_idx == cS.aD_new
                ...
                [pretax_non_capital_income, ~] = main_olg_v15_utils.HHIncome_Huggett(0, tr_per_capita_age, b_age_val, 0, a_idx, paramS_age, cS, 0);
                ...
                if isfield(cS, 'phi_bequest') && cS.phi_bequest > 1e-9
                    ...
                    [~, util_b] = main_olg_v15_utils.CES_utility(final_bequest, cS.sigma_bequest, cS);
                else
        ...
        % =========================================================================
        % == 在 main_olg_v15_utils.m 中，请【替换】此函数
        % =========================================================================
        % =========================================================================
        % == 在 main_olg_v15_utils.m 中，请【替换】此函数
        % =========================================================================
        ...
        function PolicyFunctions = solve_transition_path_policies(w_path, r_mkt_path, b_path, T_sim, cS, paramS)
            ...
            for t = T_sim:-1:1
                ...
                if ~cS_vfi.pps_active && cS_vfi.nkpps > 1
                    ...
                    cS_vfi = main_olg_v15_utils.generateGrids(cS_vfi);
                end
                ...
                for a = cS.aD_new:-1:1
                    ...
                    % --- 3. 求解当前(t,a)的策略和价值 ---
                    [~, kPol, cPpsPol, val] = main_olg_v15_utils.HHSolutionByAge_VFI_Vectorized(a, vPrime_kkppse_next, M_t, 0, M_t.b_t, paramS, cS_vfi);
        ...
        function [K_pvt_next, K_pps_next, C_t, Total_Cpps_t, Total_PpsTax_t, total_accidental_bequest, M_t_complete] = simulate_private_capital_forward(t, K_pvt_t, B_p_t, K_pps_t, B_g_t, Z_t, A_t, cS, paramS, eIdxM)
            ...
            [~, L_t] = main_olg_v15_utils.LaborSupply_Huggett(eIdxM, cS, paramS_t, Z_t);
            M_prices = main_olg_v15_utils.get_prices_at_t(K_physical_t, L_t, A_t, cS);
            ...
            if ~cS_vfi.pps_active, cS_vfi.nkpps = 1; cS_vfi.npps = 1; cS_vfi = main_olg_v15_utils.generateGrids(cS_vfi); end
            if ~isfield(cS_vfi, 'theta_t'), cS_vfi.theta_t = cS.theta_path(t); end

            % --- 2. [核心简化] 单次求解和模拟 ---
            tr_per_survivor_eq = 0;
            [cPolM_final, kPolM_final, cPpsPolM_final, ~] = main_olg_v15_utils.HHSolution_VFI_Huggett(M_t, tr_per_survivor_eq, paramS, cS_vfi);
            [kHistM, kPpsHistM, cHistM, cppsHistM] = main_olg_v15_utils.HHSimulation_olgm(kPolM_final, cPpsPolM_final, cPolM_final, eIdxM, M_t, paramS, cS_vfi, tr_per_survivor_eq);
        ...
        function Results = simulate_forward_with_policies(initial_dist_k, initial_dist_kpps, PolicyFunctions, T_sim, Z_path, A_path, w_path, r_mkt_path, b_path, L_path, cS, paramS, eIdxM)
            ...
            for t = 1:T_sim
                ...
                [kHist_t, kppsHist_t, c_panel, ~] = main_olg_v15_utils.HHSimulation_olgm_transition(k_panel, kpps_panel, Policies_t, eIdxM, M_t, paramS, cS_sim);
        ...
        function [leLogGridV, leTrProbM, leProb1V] = EarningProcess_olgm(cS)
            ...
            Tauchen_q = 2.0;
            [leLogGridV_raw, leTrProbM] = main_olg_v15_utils.tauchen(cS.nw, lePersistence, leShockStd, 0, Tauchen_q);
        ...
        function eIdxM_group = LaborEndowSimulation_olgm_AgeGroup(cS, paramS)
            eIdxM_group = main_olg_v15_utils.MarkovChainSimulation_AgeGroup(cS.nSim, cS, paramS.leProb1V, paramS.leTrProbM);
        end
    ...
    methods (Static, Access = private) % <<<<<< 新增一个私有静态方法块
        ...
        function neg_v = objective_for_k_prime_private(k_prime_choice, resources, k_pps_state, c_pps_choice, R_k_net_factor_age, a_idx, ev_interpolant, cS)
            ...
            ev = main_olg_v15_utils.CallInterpolator(ev_interpolant, k_prime_clamped, k_pps_prime_clamped, cS);
