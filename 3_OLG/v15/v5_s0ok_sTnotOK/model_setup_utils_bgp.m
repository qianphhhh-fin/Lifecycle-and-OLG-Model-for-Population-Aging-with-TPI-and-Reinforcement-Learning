classdef model_setup_utils_bgp
    methods (Static)
        ...
        function [leGridV_expanded, TrProbM_by_age, leProb1V, nw_expanded] = EarningProcess_AgeDependent(cS)
            ...
            % [BGP不变] 此函数描述的是个体相对于同龄人均值的偏离以及冲击的转移概率
            % 经济体整体的技术进步会等比例地提升所有人的工资水平，但不影响他们之间的相对生产率分布
        ...
        function [Z_path, A_path] = load_exogenous_paths(cS, plot_flag)
            % [BGP不变] 此函数已经正确地基于历史数据和长期增长率预测，构建了一条完整的TFP水平路径
            % 这条路径的终点 A_path(end) 将成为终期稳态的"技术水平基准值"
            % 用于将终期稳态的标准化结果"复原趋势"
            if nargin < 2, plot_flag = false; end
            ...
            extended_birth_rate = [birth_rate_path_annual(1), birth_rate_path_annual];
            
            Pop_dist_annual = zeros(cS.aD_new, length(extended_years_vec));
            Pop_dist_annual(:, 1) = initial_pop_dist;
