...
classdef SS
    methods (Static)
        ...
        function [F_error, ss, Dist, polS, valS] = system_of_equations(x_guess, Z_ss_norm, cS, paramS, params_ext)
            ...
            % ==
            % == 核心修改:
            % ==   - 接收 b_hat_guess 作为第5个均衡变量。
            % ==   - 基于 PAYG 预算平衡，从 b_hat_guess 反解出厂商需要雇佣的
            % ==     劳动量 L_required。
            % ==   - 将劳动市场出清条件修改为：error_L = L_required - L_supply_agg。
            % =========================================================================
            r_guess = x_guess(1);
            w_guess = x_guess(2);
            BQ_guess = x_guess(3);
            TR_guess = x_guess(4);
            b_hat_guess = x_guess(5); % 现在直接求解 b_hat

            % --- 流程 1: 微观聚合 (现在接收 b_hat_guess) ---
            ...
            % --- 流程 2: 宏观计算 ---
            % [!!! 核心修正: 从 b_hat 反解 L !!!]
            % 根据 PAYG 预算平衡: b_hat * mass_retirees = theta * w * L
            % 因此，为支持这个养老金水平，经济体需要提供的总劳动是:
            ...
            error_L = L_required_for_PAYG - L_supply_agg; % [!!! 劳动市场出清条件 !!!]
            ...
            if nargout > 1
                ...
                ss.b_hat = b_hat_guess; % 存储均衡的养老金福利
            end
        end
