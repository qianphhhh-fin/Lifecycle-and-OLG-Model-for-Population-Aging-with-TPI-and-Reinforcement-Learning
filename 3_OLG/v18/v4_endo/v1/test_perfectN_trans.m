...
for iter = 1:max_iter
    ...
    % [新增] 策略函数一致性检验
    % 在完美BGP下，倒数第二期(T-1)的策略应与最终稳态策略polF完全一致
    policy_error = 0;
    pol_tm1 = Pol_path{T-1};
    for ia = 1:cS.aD_new
        err_c = max(abs(pol_tm1(ia).c - polF(ia).c), [], 'all');
        err_k_prime = max(abs(pol_tm1(ia).k_prime - polF(ia).k_prime), [], 'all');
        policy_error = max([policy_error, err_c, err_k_prime]);
        
        if isfield(polF, 'kpps_prime')
             err_kpps_prime = max(abs(pol_tm1(ia).kpps_prime - polF(ia).kpps_prime), [], 'all');
             policy_error = max(policy_error, err_kpps_prime);
        end
    end

    % [报告迭代结果] 加入策略函数误差
    fprintf('Iter [%2d/%2d] | Error: %.2e (r:%.1e, w:%.1e, bq:%.1e, tr:%.1e, b:%.1e) | Pol_Err: %.2e\n', ...
        iter, max_iter, errors.total, errors.r, errors.w, errors.beq, errors.tr, errors.b_hat, policy_error);

    % [检查收敛]
    if errors.total < tolerance && policy_error < tolerance
        fprintf('\n✅ 诊断成功！宏观路径与策略函数均收敛。\n'); 
        break; 
    end
