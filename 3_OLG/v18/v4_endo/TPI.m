...
classdef TPI
    methods (Static)
        ...
        function [target_paths, errors, Pol_path, Dist_path, aggr_supply] = calculate_paths_and_errors(r_path, w_hat_path, Bequest_per_newborn_path, TR_per_hh_path, b_hat_path, ss0, ssF, cS, paramSF, valF, polF, dist0)
            % =========================================================================
            % ==          TPI核心计算引擎 (v4.2 - 逻辑统一最终版)
            % ==
            % == 核心修正:
            % ==  - [!!!] 删除了对输入猜测路径 `Bequest_per_newborn_path` 的
            % ==    任何修改。猜测路径被直接、完整地传递给 VFI 和分布模拟器。
            % ==  - 聚合函数 aggregates.m 现在返回纯粹的当期发生量。
            % ==  - 所有跨期核算逻辑被集中在此函数内，确保清晰和正确。
            % =========================================================================
            T = cS.T_sim;
            ...
            pathS.tr_per_hh_hat_path = TR_per_hh_path;
            ...
            % --- 3. 向前模拟人口分布的演进 ---
            % [!!! 核心修正 !!!] 直接将猜测的人均遗赠路径传递给模拟器
            Dist_path = distribution.simulate_dist_forward(dist0, Pol_path, cS, paramSF, Bequest_per_newborn_path);

            % --- 4. 聚合宏观供给量 ---
            aggr_supply = aggregates.get_path_aggregates(Dist_path, Pol_path, cS, ss0, paramSF);
            L_path_supply = aggr_supply.L_path;
            K_p_hat_path_supply = aggr_supply.K_p_hat_path;
            ...
            K_g_hat_path = ssF.K_public_hat * (total_pop_path / total_pop_path(1));

            for t = 1:T
                prices_t = firm.get_prices_at_t(K_p_hat_path_supply(t), K_g_hat_path(t), L_path_supply(t), cS);
                target_paths.r_path(t) = prices_t.r_mkt_t;
                target_paths.w_hat_path(t) = prices_t.w_hat_t;
                Y_hat_path_calc(t) = prices_t.Y_hat_t;
            end
            
            % 5a. 计算 b_hat, TR_per_hh 目标路径 (逻辑正确)
            ...
            target_paths.b_hat_path = total_pension_pot_hat_path ./ max(1e-9, mass_retirees_path);
            
            TR_total_hat_target_path = TPI.calculate_endogenous_TR_path(target_paths.r_path, target_paths.w_hat_path, K_p_hat_path_supply, L_path_supply, Y_hat_path_calc, Dist_path, Pol_path, cS, paramSF);
            target_paths.TR_per_hh_path = TR_total_hat_target_path ./ total_pop_path;
            
            % 5c. [!!! 核心修正: 重新计算目标遗赠路径 !!!]
            target_paths.Bequest_per_newborn_path = zeros(1, T);
            
            % t=1...T 每一期产生的总遗赠 (来自纯粹的聚合)
            total_bequest_generated_hat_path = aggr_supply.Bequest_gen_hat_path;
            
            % t=1...T 每一期的新生儿数量
            mass_newborns_path = cS.Z_path_raw(1, :);
            
            % 目标路径的 t=1 期由 ss0 决定
            target_paths.Bequest_per_newborn_path(1) = ss0.Bequest_generated_agg / mass_newborns_path(1);
            
            % 目标路径的 t=2...T 期，由 t-1...T-1 期产生的遗赠决定
            for t = 2:T
                target_paths.Bequest_per_newborn_path(t) = total_bequest_generated_hat_path(t-1) / mass_newborns_path(t);
            end

            % --- 6. 计算误差 ---
            errors.r = max(abs(target_paths.r_path(2:T) - r_path(2:T)));
            errors.w = max(abs(target_paths.w_hat_path(2:T) - w_hat_path(2:T)));
            errors.b_hat = max(abs(target_paths.b_hat_path(2:T) - b_hat_path(2:T)));
            errors.beq = max(abs(target_paths.Bequest_per_newborn_path(2:T) - Bequest_per_newborn_path(2:T)));
            errors.tr = max(abs(target_paths.TR_per_hh_path(2:T) - TR_per_hh_path(2:T)));
