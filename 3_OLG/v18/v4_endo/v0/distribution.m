...
function Dist = get_ss_dist_pps(polS, paramS, cS, Z_ss_norm, Bequest_Total_guess)
        ...
        function Dist_path = simulate_dist_forward(dist0, Pol_path, cS, paramS, Bequest_received_by_newborns_path)
            % =========================================================================
            % == 函数: simulate_dist_forward (v3.7 - 遗赠接收路径接口版)
            % == 核心修正:
            % ==   - [!!!] 输入变量名修改为 Bequest_received_by_newborns_path，
            % ==     以明确其含义：向量的第 t 个元素是 t 期新生儿收到的人均遗赠。
            % ==   - 在循环中，使用 Bequest_received_by_newborns_path(t+1) 来初始化
            % ==     t+1 期的新生儿。
            % =========================================================================
            T = cS.T_sim;
            nk = cS.nk;
            ...
            for t = 1:(T-1)
                ...
                if mass_newborns_tp1 > 1e-9
                    % [!!! 核心修正 !!!] 
                    % t+1期的新生儿，接收 t+1期的人均遗赠
                    beq_per_newborn_hat = Bequest_received_by_newborns_path(t+1);
