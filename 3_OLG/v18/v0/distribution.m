...
classdef distribution
 ...
 % =========================================================================
        % ==           [修正版] 转轨路径分布前向模拟器 v3.3 (净化逻辑版)
        % == 函数: simulate_dist_forward
        % == 核心修改:
        % ==   - 彻底分离幸存者和新生儿的计算，以增强稳健性。
        % ==   - 不再使用 += 为新生儿添加质量，而是直接赋值。
        % =========================================================================
        function Dist_path = simulate_dist_forward(dist0, Pol_path, cS, paramS, Bequest_hat_path)
            ...
            for t = 1:(T-1)
                ...
                if mass_newborns_tp1 > 1e-9
                    % 注意：Bequest_hat_path是【总量】，这里需要转换为per-capita
                    % (在你的调用逻辑中，Bequest_hat_path已经是per-capita了，保持原样)
                    beq_per_newborn_hat = Bequest_hat_path(t+1);
