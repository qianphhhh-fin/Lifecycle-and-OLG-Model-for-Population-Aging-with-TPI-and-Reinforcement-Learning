...
classdef distribution
 ...
 % =========================================================================
        % ==           [修正版] 转轨路径分布前向模拟器 v3.4 (遗赠分配修正)
        % == 函数: simulate_dist_forward
        % == 核心修改:
        % ==   - [!!!] 修正了遗赠分配逻辑。现在，它接收总遗赠路径 (Bequest_hat_path)，
        % ==     并根据当期新生儿的数量 (mass_newborns_tp1) 计算正确的人均遗赠。
        % =========================================================================
        function Dist_path = simulate_dist_forward(dist0, Pol_path, cS, paramS, Bequest_hat_path)
            T = cS.T_sim;
            nk = cS.nk;
            nkpps = cS.nkpps;
            nw = cS.nw_expanded;
            ...
            for t = 1:(T-1)
                ...
                % 初始化 t+1 期的幸存者分布 (年龄从 2 到 aD)
                dist_survivors_tp1 = zeros(nk, nkpps, nw, aD);

                % --- 步骤 a: 计算 t+1 期的幸存者 (直接计算绝对数量) ---
                for ia = 1:(aD - 1)
                ...
                % --- 步骤 b: 计算 t+1 期的新生儿分布 (年龄为 1) ---
                dist_newborns_tp1 = zeros(nk, nkpps, nw, aD);
                mass_newborns_tp1 = Z_path_abs(1, t+1);

                if mass_newborns_tp1 > 1e-9
                    % [!!! 核心修正 !!!]
                    % Bequest_hat_path 是 t+1 期的【总】遗赠。
                    % mass_newborns_tp1 是 t+1 期的【总】新生儿数量。
                    % 我们需要计算人均遗赠。
                    total_bequest_pool_tp1 = Bequest_hat_path(t+1);
                    beq_per_newborn_hat = total_bequest_pool_tp1 / mass_newborns_tp1;
                    
                    [ik_lower, ~, w_upper] = utils.find_grid_and_weights(beq_per_newborn_hat, cS.kGridV);
                    w_lower = 1.0 - w_upper;
                    ik_upper = min(nk, ik_lower + 1);
                    ikpps_newborn = 1;
                    ...
                    % 直接赋值给 ia=1 的切片
                    dist_newborns_tp1(ik_lower, ikpps_newborn, 1:cS.nw, 1) = w_lower * prob_mass_le * mass_newborns_tp1;
                    dist_newborns_tp1(ik_upper, ikpps_newborn, 1:cS.nw, 1) = dist_newborns_tp1(ik_upper, ikpps_newborn, 1:cS.nw, 1) + w_upper * prob_mass_le * mass_newborns_tp1;
                end

                % --- 步骤 c: 合并幸存者和新生儿，得到最终分布 ---
