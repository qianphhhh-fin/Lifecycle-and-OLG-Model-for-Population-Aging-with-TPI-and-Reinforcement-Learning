...
classdef TPI
    methods (Static)
        ...
        function [target_paths, errors, Pol_path, Dist_path, aggr_supply] = calculate_paths_and_errors(r_path, w_hat_path, Bequest_hat_path, TR_hat_path, ss0, ssF, cS, paramSF, valF, polF, dist0)
            % =========================================================================
            % ==          TPI核心计算引擎 (v3.9 - 公共资本路径修正版)
            % ==
            % == 核心修正:
            % ==  - 之前版本中，公共资本 K_g_hat 在整个转轨路径上被固定为稳态值。
            % ==  - 当 n > 0 时，这导致 K_g/L 和 K_g/K_p 的比率随时间下降，
            % ==    从而使厂商计算出的要素价格偏离BGP。
            % ==  - 本次修正构建了一个随人口增长的公共资本路径 K_g_hat_path，
            % ==    确保在BGP上所有要素投入的比率保持不变，从而稳定价格。
            % =========================================================================
            T = cS.T_sim;
            ...
            pathS.tr_per_hh_hat_path = TR_hat_path ./ total_pop_path;
            ...
            % [!!! 核心修正 !!!]
            % 构建一个随人口增长的公共资本路径 (hat value)
            % 保证在BGP上，K_g_hat/L_hat 和 K_g_hat/K_p_hat 比率恒定。
            K_g_hat_path = ssF.K_public_hat * (total_pop_path / total_pop_path(1));

            for t = 1:T
                % 使用随时间变化的 K_g_hat_path(t)
                prices_t = firm.get_prices_at_t(K_p_hat_path(t), K_g_hat_path(t), L_path(t), cS);
