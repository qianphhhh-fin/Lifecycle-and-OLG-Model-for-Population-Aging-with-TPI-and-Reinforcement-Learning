% --- 分析SQP算法VFI策略函数平滑性和稳定性脚本 ---

clc; clear; close all;

fprintf('=== SQP算法VFI策略函数平滑性和稳定性分析 ===\n');
...
% try
    ...
    cS.use_continuous_optimization = true; % 强制使用fmincon进行SQP分析
...
fprintf('强制使用fmincon SQP算法进行VFI...\n');
vfi_start_time = tic;

% 调用VFI主函数（确保使用SQP版本）
[cPolM_sqp, kPolM_sqp, cPpsPolM_sqp, valM_sqp] = main_olg_v8_utils.HHSolution_VFI_Huggett(...
    R_k_net_factor_test, w_gross_test, test_params.TR_total_test, bV_test, paramS_test, cS);

vfi_time = toc(vfi_start_time);
fprintf('SQP-VFI完成，耗时: %.2f秒\n', vfi_time);
...
for age_idx = 1:length(analysis_config.test_ages)
    ...
    % 提取该年龄组的策略函数
    c_policy_slice = squeeze(cPolM_sqp(:, :, ie_test, a_test));
    k_policy_slice = squeeze(kPolM_sqp(:, :, ie_test, a_test));
    cpps_policy_slice = squeeze(cPpsPolM_sqp(:, :, ie_test, a_test));
    val_slice = squeeze(valM_sqp(:, :, ie_test, a_test));
...
for run_idx = 1:analysis_config.n_stability_runs
    ...
    % 计算差异
    c_diff = abs(cPolM_sqp - cPolM_pert);
    k_diff = abs(kPolM_sqp - kPolM_pert);
    cpps_diff = abs(cPpsPolM_sqp - cPpsPolM_pert);
    val_diff = abs(valM_sqp - valM_pert);
...
create_policy_plots(cPolM_sqp, kPolM_sqp, cPpsPolM_sqp, valM_sqp, cS, analysis_config.test_ages);

%% 8. 边界行为分析
fprintf('\n--- 8. 边界行为分析 ---\n');

boundary_analysis = analyze_boundary_behavior(cPolM_sqp, kPolM_sqp, cPpsPolM_sqp, cS);
...
function create_policy_plots(cPolM, kPolM, cPpsPolM, valM, cS, test_ages)
    ...
    sgtitle('SQP算法VFI策略函数 (效率状态2, 中等PPS资产水平)');
end
...
function provide_convergence_diagnostics(smoothness_results, stability_results, boundary_analysis, c_stability_ratio, k_stability_ratio)
    ...
    if c_stability_ratio > 10 || k_stability_ratio > 10
        ...
        fprintf('   - 建议: 减少fmincon精度 (TolFun, TolX → 5e-6)\n\n');
    else
        fprintf('2. 数值稳定性良好:\n');
    ...
    if c_stability_ratio > 10
        fprintf('   a) 立即措施: 暂时改用网格搜索 (use_continuous_optimization = false)\n');
        fprintf('   b) 参数调整: 放宽外层收敛条件\n');
        fprintf('   c) 算法改进: 考虑混合策略 (粗网格 + 局部SQP)\n');
    else
        fprintf('   - SQP算法总体表现良好，可通过微调参数进一步优化\n');
        fprintf('   - 建议适度放宽外层容忍度以匹配SQP的高精度特性\n');
    end
end 