...
% };
configurations_to_test = { ...
    struct('label', 'VFI_vectorized (20x20)', 'nk', 20, 'nkpps', 20, 'nkprime', 20, 'npps', 20, 'solver', 'vectorized_grid'), ...
    struct('label', 'VFI_vectorized (30x30)', 'nk', 30, 'nkpps', 30, 'nkprime', 30, 'npps', 30, 'solver', 'vectorized_grid'), ...
...
if exist(shock_path_filename, 'file')
    ...
    error('错误: 找不到效率冲击路径文件 %s。请先运行Python脚本生成它。', shock_path_filename);
end
...
for i_config = 1:length(configurations_to_test)
    ...
    % b. 求解VFI (使用已修复的函数)
    tic;
    [cPolM, kPolM, cPpsPolM, ~] = main_olg_v8_utils.HHSolution_VFI_Huggett(...
    ...
    % c. 模拟生命周期路径
    tic;
    [k_path, ~, c_path, ~] = main_olg_v8_utils.HHSimulation_olgm(...
...
function plot_focused_comparison(results_cell)
    ...
    sgtitle('聚焦对比: Hybrid (20x20) vs Vectorized (80x80)', 'FontSize', 16, 'FontWeight', 'bold');
end