classdef SS
    methods (Static)
        ...
        function [F_error, ss, Dist, polS, valS] = system_of_equations(x_guess, Z_ss_norm, L_supply_exog, cS, paramS, params_ext)
            ...
            % ==
            % == 核心修正:
            % ==   - 在函数末尾打包最终ss结构体时，确保所有宏观变量（特别是价格和流量）
            % ==     都使用本次迭代中最新计算出的 `_new` 值，而不是求解器的`_guess`值。
            % ==     这保证了最终报告的绝对内部一致性。
            % =========================================================================
            r_guess = x_guess(1);
            w_guess = x_guess(2);
            ...
            Total_Gov_Revenue_calc = macro_state.Total_Tax_Revenue + Public_Capital_Return_calc;
            ...
            TR_new = Total_Gov_Revenue_calc - (G_consumption_spending + G_investment_spending);
            ...
            if nargout > 1
                ...
                % ======================== 核心修正区域 ========================
                ss.r_mkt = r_new; % 使用新计算的 r_new
                ss.w_hat = w_new; % 使用新计算的 w_new
                % =============================================================
                ...
                % ======================== 核心修正区域 ========================
                % 使用新计算出的 BQ_new 和 TR_new 来确保报告的一致性
        ...
        function [macro_state, Dist, polS, valS] = run_micro_to_macro(r_mkt, w_hat, BQ, TR, Z_ss_norm, L_supply_exog, cS, paramS, params_ext)
            ...
            % ==
            % == 核心修改:
            % ==   - 此函数不再计算最终的Y, K_g等宏观变量。
            % ==   - 它的唯一职责是：给定价格，求解家庭问题，并聚合出宏观状态变量
            % ==     (K_p_agg, C_agg, BQ_agg, Tax_agg)。
            % ==   - 返回一个包含这些聚合状态的结构体 `macro_state`。
            % =========================================================================
        ...
        function display_national_accounts(ss, cS, paramS, Z_ss_norm, report_filename)
            ...
            % ==
            % == 核心修改:
            % ==   - 在"国民支出"部分，明确展示 Y - C - I_p - I_g - G_c 的计算过程和结果。
            % ==   - 调整了措辞，使其更清晰地对应国民收入核算恒等式。
            % =========================================================================
            ...
            for ia = 1:cS.aD_new
                ...
                if pps_active
                    ...
                    if ia <= cS.aR_new
                        labor_income_slice = ss.w_hat * cS.ageEffV_new(ia) .* paramS.leGridV(1:cS.nw_expanded)';
                        pps_contrib_slice = cS.pps_contrib_rate * labor_income_slice;
                        pps_contrib_full = repmat(reshape(pps_contrib_slice, [1,1,cS.nw_expanded]), [cS.nk, cS.nkpps, 1]);
                        PPS_contrib_agg = PPS_contrib_agg + sum(pps_contrib_full .* mass_dist_ia, 'all');
                    else
            ...
            Tax_non_pps = ss.Total_Tax_Revenue - PPS_tax_agg;
            ...
            Total_Gov_Revenue = ss.Total_Tax_Revenue + Public_Capital_Return_display;
            G_c_display = cS.G_c_to_Y_ratio_ss * Y;
            Total_Gov_Expenditure = G_c_display + ss.I_g + ss.TR_distributed_agg;
            budget_balance_error = Total_Gov_Revenue - Total_Gov_Expenditure;

            Total_Expenditure = ss.C_agg + I_p_demand_bgp + ss.I_g + G_c_display;
            resource_constraint_error = Y - Total_Expenditure;

            % --- 报告开始 ---
            fprintfAndLog(fileID, '\n\n=========================================================================================================\n');
            if pps_active
                fprintfAndLog(fileID, '###                 稳 态 国 民 经 济 核 算 报 告 (v7.5 - 带个人养老金账户 PPS)               ###\n');
            else
                fprintfAndLog(fileID, '###                       稳 态 国 民 经 济 核 算 报 告 (v7.5 - 标准版)                       ###\n');
            end
            fprintfAndLog(fileID, '=========================================================================================================\n');
            ...
            fprintfAndLog(fileID, '      私人总投资 (I_p) .......................... : %15.6f\n', I_p_demand_bgp);
            fprintfAndLog(fileID, '      私人资本折旧 (d*K_p) ...................... : %15.6f\n', Depreciation_p);
            fprintfAndLog(fileID, '      私人净投资 (I_p - d*K_p) .................. : %15.6f\n', I_p_demand_bgp - Depreciation_p);
            ...
            Total_Household_Income = Total_Labor_Income + Net_Capital_Income + PPS_withdrawal_agg + ss.Bequest_distributed_agg + ss.TR_distributed_agg;
            fprintfAndLog(fileID, '      --------------------------------------------------------------------------------------------------\n');
            fprintfAndLog(fileID, '      >>> 家庭总收入 ............................ : %15.6f\n', Total_Household_Income);
            fprintfAndLog(fileID, '   B. 收入用途 (流量):\n');
            fprintfAndLog(fileID, '      消费支出 (C) .............................. : %15.6f\n', ss.C_agg);
            fprintfAndLog(fileID, '      支付的税收 (非PPS) ........................ : %15.6f\n', Tax_non_pps);
            if pps_active
                fprintfAndLog(fileID, '      PPS账户缴费 (储蓄) ...................... : %15.6f\n', PPS_contrib_agg);
                fprintfAndLog(fileID, '      PPS提取税 ................................. : %15.6f\n', PPS_tax_agg);
            end
            Household_Savings = Total_Household_Income - ss.C_agg - ss.Total_Tax_Revenue;
            fprintfAndLog(fileID, '      净储蓄 (收入 - C - T_total) ............... : %15.6f\n', Household_Savings);
            fprintfAndLog(fileID, '      --------------------------------------------------------------------------------------------------\n');
            ...
            fprintfAndLog(fileID, '\n\n--- [ IV. 政府部门 ] ---\n');
            fprintfAndLog(fileID, '   A. 收入来源 (流量):\n');
            fprintfAndLog(fileID, '      家庭常规税收 .............................. : %15.6f\n', Tax_non_pps);
            if pps_active
                fprintfAndLog(fileID, '      PPS提取税 ................................. : %15.6f\n', PPS_tax_agg);
            end
