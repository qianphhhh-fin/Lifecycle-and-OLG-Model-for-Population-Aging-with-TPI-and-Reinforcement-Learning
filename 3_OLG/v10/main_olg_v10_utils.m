classdef main_olg_v10_utils
    ...
    methods (Static)
        ...
        function M_prices = get_prices_at_t(K_total_t, Z_t_norm, cS, paramS_t, eIdxM)
            % [V10.3辅助] 仅根据总资本和人口，计算当期的要素价格和宏观总量。

            paramS_t.ageMassV = Z_t_norm;
            [~, L_t] = main_olg_v10_utils.LaborSupply_Huggett(eIdxM, cS, paramS_t, Z_t_norm);

            [R_mkt_factor, w_t] = main_olg_v10_utils.HHPrices_Huggett(K_total_t, L_t, cS);
        ...
        % --- 在 main_olg_v10_utils.m 中，替换 check_gbc_residual 函数 ---
        ...
        % --- 在 main_olg_v10_utils.m 的 methods (Static) 块中新增此函数 ---
        ...
        function [ss, eq_found] = solve_steady_state_with_fund(Z_ss_norm, B_p_Y_ratio_target, cS, paramS, eIdxM)
            % [V10.3核心, 已修正K_pps聚合] 求解包含目标养老金基金的稳态均衡。
            % - 迭代变量: 总资本 K_total
            ...
            for iter = 1:max_iter
                ...
                M_ss_prices = main_olg_v10_utils.get_prices_at_t(K_guess, Z_ss_norm, cS, paramS_ss, eIdxM);
                ...
                [K_pvt_model, K_pps_model, C_ss_model, ~] = main_olg_v10_utils.simulate_private_capital_forward(M_ss, Z_ss_norm, cS, paramS_ss, eIdxM);
        ...
        function [resources_for_c_and_k_prime, actual_pps_contribution_expenditure, non_capital_income] = HHIncome_Huggett(...
            ...
            if a_idx <= cS.aR_new
                ...
                % (1) 计算并扣除 PAYG 税 (θ)
                %     PAYG税的税基是全部的税前劳动收入。
                payg_tax = cS.theta_payg * labor_income_gross;
            ...
            for a_idx = cS.aD_new : -1 : 1
                ...
                if strcmpi(solverMethod, 'hybrid')
                    [cPolM_q(:,:,:,a_idx), kPolM(:,:,:,a_idx), cPpsPolM_choice(:,:,:,a_idx), valM(:,:,:,a_idx)] = ...
                        main_olg_v10_utils.HHSolutionByAge_VFI_Huggett_HybridOptimizer(a_idx, vPrime_kkppse_next, ...
                ...
                elseif strcmpi(solverMethod, 'vectorized_grid') % <<<<< 新增CASE
                    [cPolM_q(:,:,:,a_idx), kPolM(:,:,:,a_idx), cPpsPolM_choice(:,:,:,a_idx), valM(:,:,:,a_idx)] = ...
                        main_olg_v10_utils.HHSolutionByAge_VFI_Huggett_VectorizedGrid(a_idx, vPrime_kkppse_next, ...
                        R_k_net_factor_vfi, w_gross_vfi, TR_total_vfi, bV_payg_vfi(a_idx), paramS_vfi, cS);
                else % 默认或指定 'grid'
                    [cPolM_q(:,:,:,a_idx), kPolM(:,:,:,a_idx), cPpsPolM_choice(:,:,:,a_idx), valM(:,:,:,a_idx)] = ...
                        main_olg_v10_utils.HHSolutionByAge_VFI_Huggett_v9_GridSearch(a_idx, vPrime_kkppse_next, ...
                        R_k_net_factor_vfi, w_gross_vfi, TR_total_vfi, bV_payg_vfi(a_idx), paramS_vfi, cS);
                end
        ...
        function [cPol_age_q, kPol_age, cPpsPol_age_choice, val_age] = HHSolutionByAge_VFI_Huggett_v9_GridSearch(...
            ...
            if a_idx == cS.aD_new
                % 1. 计算当期的非财富收入 (只有养老金)
                [pension_income, ~, ~] = main_olg_v10_utils.HHIncome_Huggett(0, 0, 0, TR_total_age, b_age_val, 0, a_idx, paramS_age, cS, 0);
                ...
                if cS.phi_bequest > 1e-9 % 使用一个小的容差来判断是否有遗赠动机
                    ...
                    % 计算最终的价值函数
                    [~, util_c] = main_olg_v10_utils.CES_utility(final_c, cS.sigma, cS);
                    [~, util_b] = main_olg_v10_utils.CES_utility(final_bequest, cS.sigma_bequest, cS);
                    ...
                    % 最终价值只来源于消费
                    [~, final_v] = main_olg_v10_utils.CES_utility(final_c, cS.sigma, cS);
                end
                % =================================================================
                ...
                for ikpps = 1:cS.nkpps
                    for ie = 1:cS.nw
                        ...
                        for c_pps_choice = cpps_grid
                            [resources, ~, ~] = main_olg_v10_utils.HHIncome_Huggett(k_state, R_k_net_factor_age, w_gross_age, TR_total_age, b_age_val, c_pps_choice, a_idx, paramS_age, cS, epsilon_state);
                            ...
                            for k_prime_choice = k_prime_grid
                                ...
                                [~, util] = main_olg_v10_utils.CES_utility(c_choice, cS.sigma, cS);
                ...
                for ikpps = 1:cS.nkpps
                    for ie = 1:cS.nw
                        ...
                        for c_pps_choice = cpps_grid
                            ...
                            % 2. 定义目标函数
                            % [重要修改] 现在调用一个独立的静态方法
                            objective_func = @(k_prime) main_olg_v10_utils.objective_for_k_prime_private(...
        ...
        function [cPol_age_q, kPol_age, cPpsPol_age_choice, val_age] = HHSolutionByAge_VFI_Huggett_VectorizedGrid(...
            ...
            if a_idx == cS.aD_new
                ...
                [resources_from_k, ~, ~] = main_olg_v10_utils.HHIncome_Huggett(K_grid, R_k_net_factor_age, 0, TR_total_age, b_age_val, 0, a_idx, paramS_age, cS, 0);
            ...
            for ie = 1:cS.nw
                ...
                % 计算当期资源
        [resources_for_c_and_k_prime, ~, ~] = main_olg_v10_utils.HHIncome_Huggett(k_state_4D, R_k_net_factor_age, w_gross_age, TR_total_age, b_age_val, actual_cpps_4D, a_idx, paramS_age, cS, epsilon_state);
        ...
        resources_4D = resources_for_c_and_k_prime + pps_withdrawal_val;
                ...
                % 计算价值
                [~, util_4D] = main_olg_v10_utils.CES_utility(c_choice_4D, cS.sigma, cS);
                ...
                [resources_final_pre_pps, best_c_pps, ~] = main_olg_v10_utils.HHIncome_Huggett(k_state_2D, R_k_net_factor_age, w_gross_age, TR_total_age, b_age_val, best_prop_cpps, a_idx, paramS_age, cS, epsilon_state);
        ...
        % --- 在 main_olg_v10_utils.m 中，修改此函数 ---

        % --- 在 main_olg_v10_utils.m 中，修改此函数 ---

        function M_t = get_prices_and_policy_at_t(K_pvt_t, B_p_t, K_pps_t, Z_t_norm, cS, paramS, eIdxM)
            % [V10.3, 已修正K_pps聚合] 调用价格计算函数，并计算当期政策变量。

            % a. 计算总资本并获取价格
            K_total_t = K_pvt_t + B_p_t + K_pps_t;
            M_t = main_olg_v10_utils.get_prices_at_t(K_total_t, Z_t_norm, cS, paramS, eIdxM);

            % b. 计算养老金福利 (基于固定的替代率)
            mass_workers_t = sum(Z_t_norm(1:cS.aR_new));
            avg_wage_t = M_t.w_t * M_t.L_t / mass_workers_t;
            M_t.b_t = cS.rho_prime_payg * avg_wage_t;
        end


        % --- 在 main_olg_v10_utils.m 的 methods (Static) 块中 ---
        % --- 替换掉旧的 simulate_private_capital_forward 函数 ---
        ...
        function [K_pvt_next, K_pps_next, C_t, Total_Cpps_t, Total_PpsWithdrawalTax_t] = simulate_private_capital_forward(M_t, Z_t_norm, cS, paramS, eIdxM)
            ...
            [kHistM, kPpsHistM, cHistM, cppsHistM] = main_olg_v10_utils.HHSimulation_olgm(...
        ...
        function B_p_next = update_pension_fund(B_p_t, M_t, Z_t_norm, cS)
            ...
            % a. 计算当期养老金收支
            PensionRevenue_t = cS.theta_payg * M_t.w_t * M_t.L_t;
            mass_retirees_t = sum(Z_t_norm(cS.aR_new+1:end));
            PensionOutlay_t = M_t.b_t * mass_retirees_t;

            % b. 应用基金演化方程
            % [修正] 基金获得市场回报率，折旧已在MPK计算中扣除
            B_p_next = (1 + M_t.r_mkt_t) * B_p_t + (PensionRevenue_t - PensionOutlay_t);

            % c. 确保基金不会无限负债 (一个简单的下限约束)
            B_p_next = max(0, B_p_next);
        end


        % --- 在 main_olg_v10_utils.m 中，替换整个 update_gov_debt 函数 ---

        % --- 在 main_olg_v10_utils.m 中，替换 update_gov_debt 函数 ---

        % --- 在 main_olg_v10_utils.m 中，替换 update_gov_debt 函数 ---

        function [B_g_next, G_t] = update_gov_debt(B_g_t, C_t, M_t, Total_Cpps_t, K_pps_t, Total_PpsWithdrawalTax_t, cS, paramS)
            % [V10核心 - 最终修正版] 更新政府债务，精确核算税收

            % a. 应用财政规则，决定当期政府支出 G_t
            debt_ratio = B_g_t / M_t.Y_t;
            gov_spending_frac = cS.gov_exp_frac_Y;
            if debt_ratio > cS.debt_ratio_target
                reduction_factor = max(0, 1 - cS.debt_feedback_strength * (debt_ratio - cS.debt_ratio_target));
                gov_spending_frac = gov_spending_frac * reduction_factor;
            end
            G_t = gov_spending_frac * M_t.Y_t;

            % b. 计算当期一般预算赤字
            % (1) 劳动税收入 (税基扣除PPS缴费)
            GrossLaborIncome_t = M_t.w_t * M_t.L_t;
            TaxableLaborIncome_t = GrossLaborIncome_t - Total_Cpps_t;
            LaborTaxRevenue_t = cS.tau_l * max(0, TaxableLaborIncome_t);

            % (2) [核心修正] 资本税收入的税基是经济中的总资本回报
            %    PPS账户的税收优惠体现在“递延”，而不是“免除”。
            %    在宏观核算层面，所有资本都产生了应税收入。
            TotalCapitalIncome_t = M_t.r_mkt_t * M_t.K_total_t;
            CapitalTaxRevenue_t = cS.tau_k * TotalCapitalIncome_t;

            % (3) 消费税收入
            ConsumptionTaxRevenue_t = cS.tau_c * C_t;

            % (4) [核心修正] 总税收 = 各项税收 + PPS提取税
            TaxRevenue_t = LaborTaxRevenue_t + CapitalTaxRevenue_t + ConsumptionTaxRevenue_t + Total_PpsWithdrawalTax_t;

            % (5) 计算政府基本赤字 (Primary Deficit)
            PrimaryDeficit_t = G_t - TaxRevenue_t;

            % c. 应用债务演化方程
            B_g_next = (1 + M_t.r_mkt_t) * B_g_t + PrimaryDeficit_t;
        end
        ...
        function [leLogGridV, leTrProbM, leProb1V] = EarningProcess_olgm(cS)
            ...
            Tauchen_q = 2.0;
            [leLogGridV_raw, leTrProbM] = main_olg_v10_utils.tauchen(cS.nw, lePersistence, leShockStd, 0, Tauchen_q);
        ...
        function eIdxM_group = LaborEndowSimulation_olgm_AgeGroup(cS, paramS)
            eIdxM_group = main_olg_v10_utils.MarkovChainSimulation_AgeGroup(cS.nSim, cS, paramS.leProb1V, paramS.leTrProbM);
        end
    ...
    methods (Static, Access = private) % <<<<<< 新增一个私有静态方法块
        ...
        function neg_v = objective_for_k_prime_private(k_prime_choice, resources, k_pps_state, c_pps_choice, R_k_net_factor_age, a_idx, ev_interpolant, cS)
            ...
            ev = main_olg_v10_utils.CallInterpolator(ev_interpolant, k_prime_clamped, k_pps_prime_clamped, cS);
