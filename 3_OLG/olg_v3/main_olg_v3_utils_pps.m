% --- START OF FILE main_olg_v3_utils_pps.m ---

classdef main_olg_v3_utils_pps % CLASS NAME UPDATED
    % OLG 模型 v3 的工具函数
    ...
    methods (Static)
        ...
        function [Z_ss, dependency_ratio_ss, bgp_reached, bgp_period] = detectSteadyStatePopulation(popS, cS)
            ...
            % 绘制初始与稳态年龄组分布的比较图
             ...
             % --- 绘制初始与稳态年龄组分布图 ---
             figure('Name', 'V3: 初始 vs 稳态 年龄组人口分布'); % 更改图名以反映V3版本
             hold on;
        ...
        % =====================================================================
        % == 经济函数 (修改版：混合时间单位 & PPS & 内生PAYG税率 & Heer GBC) =======
        % =====================================================================

        function cS = ParameterValues_HuggettStyle()
            % 参数设定: 外生替代率, 内生PAYG税率, PPS, Heer GBC
            
            % --- 人口统计与分组参数 (与之前v2_pps版本一致) ---
            ...
            cS.max_payg_payroll_tax_rate = 1.0; 

            %% NEW: Heer (2020) Style Government Finance Parameters (年度)
            cS.tau_k = 0.20;       % 资本所得税率 (例如，对 r_gross - ddk 征收20%)
            cS.tau_c = 0.10;       % 消费税率 (例如, 10%)
            cS.tau_l = 0.05;       % 额外的劳动所得税率 (在PAYG税之上再征收5%)
            cS.gov_exp_frac_Y = 0.15; % 政府消费占GDP的比例 (例如, 15%)
            cS.gov_debt_frac_Y = 0.60; % 政府债务占GDP的比例 (例如, 60%)
            % cS.interest_rate_on_debt_premium = 0.00; % 可选：政府债务相对于资本净回报率的溢价
        ...
        function [R_market_gross_factor, MPL_gross] = HHPrices_Huggett(K_productive, L_total_eff, cS)
            ...
            % R_market_gross_factor = 1 + r_k_gross - ddk
            % r_k_gross = MPK_gross (边际产出未扣除折旧)
            % 所以，R_market_gross_factor = 1 + (MPK_gross - cS.ddk)
        ...
        function [logGridV, trProbM, prob1V] = EarningProcess_olgm(cS)
            % 为年度 AR(1) 过程生成网格和转移矩阵。
             [logGridV_raw, trProbM_calc] = main_olg_v3_utils_pps.tauchen(cS.nw, cS.lePersistence, cS.leShockStd, 0, cS.leWidth);
             ...
             prob1V_calc = []; % 初始化初始概率向量
              try
                  [prob1V_calc, ~, ~] = main_olg_v3_utils_pps.norm_grid(logGridV_raw, normGridMin, normGridMax, 0, cS.leSigma1);
        ...
        function [HHlaborM_group, L] = LaborSupply_Huggett(eIdxM, cS, paramS, Z_ss_norm_group) % (未改变)
            nSim = size(eIdxM, 1); 
        ...
        function eIdxM = LaborEndowSimulation_olgm(cS, paramS) % (未改变)
            rng(433); 
            rvInM=rand([cS.nSim,cS.aD_orig]); 
            eIdxM = main_olg_v3_utils_pps.MarkovChainSimulation(cS.nSim, cS.aD_orig, paramS.leProb1V, paramS.leTrProbM, rvInM);
        end

        function [kHistM, kPpsHistM, cHistM] = HHSimulation_olgm(kPolM, cPpsPolM, cPolM_consump, eIdxM, R_k_net_factor_hh, w_net_hh, TR_total, bV_payg, paramS, cS) % Inputs are HH net
            ...
            cPolConsumpInterp = cell(cS.nw, cS.aD_new); 
             for a_new=1:cS.aD_new 
                 for ie=1:cS.nw 
                      kPolInterp{ie, a_new}    = griddedInterpolant(kGridV_col, kPolM(:, ie, a_new), 'linear', 'nearest'); 
            ...
            % R_k_net_factor_hh already includes (1-tau_k)
            % For PPS, return rate is R_k_net_factor_hh - 1 (net rate) + premium
            % If R_k_net_factor_hh = 1 + r_gross*(1-tau_k) - ddk*(1-tau_k) (if ddk tax deductible)
            % or R_k_net_factor_hh = 1 + (r_gross - ddk)*(1-tau_k) (if ddk not tax deductible)
            % Assuming the latter from main script logic.
            % pps_return_net = (R_k_net_factor_hh - 1) + cS.pps_return_rate_premium;
            % More robustly, the premium should be on top of some base market rate.
            % Let's assume PPS enjoys the gross market return before capital tax, minus ddk, plus premium.
            % This makes PPS potentially more attractive if tau_k > 0.
            % R_market_gross_factor was used in main for HHPrices.
            % Let's assume pps_return_net is based on the same net return as non-PPS capital for simplicity, plus premium
            pps_return_net = (R_k_net_factor_hh - 1) + cS.pps_return_rate_premium;
            ...
            for a_orig = 1 : cS.aD_orig 
                ...
                cConsumpValV = zeros(nSim, 1); 
                kPpsNextV    = zeros(nSim, 1); 

                is_working = (a_orig < cS.aR_idx_orig); 
                
                pps_withdrawal_for_sim_taxed = zeros(nSim,1);  
                pps_withdrawal_for_sim_pretax = zeros(nSim,1); 

                if ~is_working && cS.pps_active && a_orig >= cS.pps_withdrawal_age_min 
                ...
                for ie = 1 : cS.nw 
                    ...
                    if ~isempty(simIdxV)
                         ...
                         kNow_ie_clamped = max(kGridV_col(1), min(kGridV_col(end), kNow_ie)); 
                         ...
                         if is_working && cS.pps_active && a_orig <= cS.pps_contribution_age_max 
                             % VFI already handles budget and max_contrib_frac (based on w_net_hh)
                         else 
                             cPpsContribV(simIdxV) = 0; 
                         end
                         ...
                         if cS.pps_active
                             ...
                             kPpsNextV(simIdxV) = kPpsNowV(simIdxV); 
                         end
                    end
                end 
                 ...
                 cHistM(:, a_orig)        = max(cS.cFloor, cConsumpValV); 
        ...
        function [muM, utilM] = CES_utility(cM, sig, cS) % (Unchanged)
             ...
             utilM(~vld) = -1e10 - (min_c - cM(~vld))*1e10; 
             muM(~vld) = c_c(~vld).^(-sig) + 1e10; 
        end

         function incomeM_resources = HHIncome_Huggett(k_non_pps_grid, R_k_net_factor_hh_price, w_net_hh_price, TR_total_price, b_payg_price_group, a_new_idx_income, paramS_income, cS)
            % R_k_net_factor_hh_price is the net return factor on non-PPS capital (already includes 1-tau_k)
            % w_net_hh_price is net wage (already includes 1-theta_payg-tau_l)
            % TR_total_price is total lump-sum transfers (gov + bequests)
        ...
        function [cPolM, kPolM, cPpsPolM, valueM] = HHSolution_VFI_Huggett(R_k_net_factor_vfi, w_net_hh_vfi, TR_total_vfi, bV_payg_vfi, paramS_vfi, cS)
            % Inputs R_k_net_factor_vfi, w_net_hh_vfi, TR_total_vfi are household net
            % cS contains tau_c needed for HHSolutionByOneState_OptK
            ...
            for a_vfi_loop_main = cS.aD_new : -1 : 1
               ...
               [cPolM(:,:,a_vfi_loop_main), kPolM(:,:,a_vfi_loop_main), cPpsPolM_temp, valueM(:,:,a_vfi_loop_main)] = ...
                  main_olg_v3_utils_pps.HHSolutionByAge_VFI_Huggett(a_vfi_loop_main, vPrimeM_vfi, ...
                  R_k_net_factor_vfi, w_net_hh_vfi, TR_total_vfi, bV_payg_vfi(a_vfi_loop_main), paramS_vfi, cS); % Pass cS
               
               if cS.pps_active && cS.pps_max_contrib_frac > 1e-9
                   cPpsPolM(:,:,a_vfi_loop_main) = cPpsPolM_temp;
               end
            end
            validateattributes(cPolM, {'double'}, {'finite', 'nonnan', 'real','>=', (cS.cFloor/(1+cS.tau_c))-1e-6,'size',[cS.nk,cS.nw,cS.aD_new]}); % cPolM is quantity
        ...
        function [cPolM_age, kPolM_age, cPpsPolM_age_actual, valueM_age_out] = HHSolutionByAge_VFI_Huggett(...
            a_idx, vPrime_keM_age, R_k_net_factor_age, w_net_hh_age, TR_total_age, b_age_group, paramS_age, cS) % Added cS input
            % R_k_net_factor_age, w_net_hh_age, TR_total_age are net for HH
            % cS is needed for tau_c in HHSolutionByOneState_OptK
            ...
            budgetM_before_pps_age = main_olg_v3_utils_pps.HHIncome_Huggett(cS.kGridV, R_k_net_factor_age, w_net_hh_age, TR_total_age, b_age_group, a_idx, paramS_age, cS);
            ...
            if a_idx == cS.aD_new % Last period
               % Budget is used for consumption, tax is paid
               cPolM_age = max(cS.cFloor, budgetM_before_pps_age / (1 + cS.tau_c) ); % Quantity of consumption
               kPolM_age = cS.kMin*ones(cS.nk,cS.nw); % No savings
               cPpsPolM_age_actual = zeros(cS.nk, cS.nw); % No PPS contribution
               [~, val_temp_age] = main_olg_v3_utils_pps.CES_utility(cPolM_age, cS.sigma, cS); % Utility from quantity
               ...
               parfor ik_age_loop = 1:cS.nk
                   ...
                   for ie_age_loop_inner = 1 : cS.nw
                        ...
                        if is_pps_contrib_age_val && cS.pps_active && cS.pps_max_contrib_frac > 1e-9
                           % PPS contribution is from pre-tax-on-consumption resources
                           % Max contrib is based on wage income (w_net_hh_age already has PAYG and tau_l out)
                           wage_income_for_pps_limit_age = w_net_hh_age * cS.ageEffV_new(a_idx) * paramS_age.leGridV(ie_age_loop_inner);
                           max_contrib_pps_age = wage_income_for_pps_limit_age * cS.pps_max_contrib_frac;
                           
                           % Budget available for (c*(1+tau_c) + k_prime_non_pps) must be >= cS.cFloor*(1+tau_c) + cS.kMin
                           % So cPps cannot exceed budget_ike - (cS.cFloor + cS.kMin/(1+tau_c)) effectively if budget is tight.
                           % This is tricky. Let's assume PPS contribution reduces cash available for c and k'.
                           % Minimum expenditure on c (quantity) is cS.cFloor, costing cS.cFloor*(1+cS.tau_c).
                           % Minimum k' is cS.kMin.
                           % So, max cPps is budget_ike_before_pps_age - (cS.cFloor * (1 + cS.tau_c) + cS.kMin)
                           max_permissible_cPps_from_budget = budget_ike_before_pps_age - (cS.cFloor * (1 + cS.tau_c) + cS.kMin);
                           
                           cPps_chosen_this_state_val = min(max_contrib_pps_age, max(0, max_permissible_cPps_from_budget));
                        ...
                        [c_row_age(ie_age_loop_inner), k_row_age(ie_age_loop_inner), v_row_age(ie_age_loop_inner)] = ...
                              main_olg_v3_utils_pps.HHSolutionByOneState_OptK(a_idx, budget_remaining_for_k_prime_and_c_expenditure, vPofK_interpolant_age, fminOpt_age, cS, paramS_age);
                   end
        ...
        function [c_quantity, kPrime, ValueFunc] = HHSolutionByOneState_OptK(a_new_osa_k, budget_remaining_expenditure_osa_k, vPofK_int_osa_k, fminOpt_osa_k, cS, paramS_osa_k)
            % budget_remaining_expenditure_osa_k is cash-on-hand to be spent on c*(1+tau_c) and kPrime
            kPMin_osa_k = cS.kMin;
            % Max kPrime leaves enough for cS.cFloor quantity, which costs cS.cFloor * (1+cS.tau_c)
            kPMax_osa_k = budget_remaining_expenditure_osa_k - (cS.cFloor * (1 + cS.tau_c)); 
            ValueFunc = -Inf;

            % Nested functions will have access to cS via the scope of HHSolutionByOneState_OptK
            ...
            function [negVal_nested, Val_nested] = BellmanInner_nested(kP_inner_nested, ag_inner_nested, budget_exp_inner_nested, evInt_inner_nested)
                % budget_exp_inner_nested is for expenditure: c*(1+tau_c) + kP
                % So, c_quantity = (budget_exp_inner_nested - kP_inner_nested) / (1 + cS.tau_c)
                cons_quantity_nested = max(cS.cFloor, (budget_exp_inner_nested - kP_inner_nested) / (1 + cS.tau_c) ); % CRITICAL CHANGE FOR V3
                
                [~, util_val_nested] = main_olg_v3_utils_pps.CES_utility(cons_quantity_nested, cS.sigma, cS); % Utility from quantity
                if ~isfinite(util_val_nested), util_val_nested = -1e12; end; Val_nested = util_val_nested;
                if ag_inner_nested < cS.aD_new % If not the last period
                    evF_val_nested = -Inf;
                    try 
            ...
            if kPMax_osa_k <= kPMin_osa_k % Corner solution for kPrime (usually kPMin if budget too tight)
                kPrime = kPMin_osa_k; 
                c_quantity = max(cS.cFloor, (budget_remaining_expenditure_osa_k - kPrime) / (1 + cS.tau_c) );
                 [~, u_c_osa_k] = main_olg_v3_utils_pps.CES_utility(c_quantity, cS.sigma, cS); 
            ...
            else % Interior solution for kPrime (or bounded interior)
                kPMin_opt_osa_k = max(cS.kMin, kPMin_osa_k); 
                kPMax_opt_osa_k = max(kPMin_opt_osa_k + 1e-9, min(cS.kMax, kPMax_osa_k)); % Ensure Max > Min for fminbnd

                if kPMin_opt_osa_k >= kPMax_opt_osa_k % If bounds are still problematic
                    ...
                    % warning('OptK bounds invalid after adjustment, age %d',a_new_osa_k); % Less verbose
                else
                    ...
                    % Check if solution is at bounds, fminbnd might not be perfect
                    if eflag_osa_k <= 0 || abs(kPrime_opt_osa_k-kPMin_opt_osa_k)<1e-7 || abs(kPrime_opt_osa_k-kPMax_opt_osa_k)<1e-7
                        ...
                        if nV_min_osa_k <= nV_max_osa_k + 1e-9 % If min bound gives better or equal value
                             kPrime_opt_osa_k=kPMin_opt_osa_k; negV_osa_k=nV_min_osa_k; 
                        else % Max bound is better
                             kPrime_opt_osa_k=kPMax_opt_osa_k; negV_osa_k=nV_max_osa_k; 
                        end
                        if eflag_osa_k <= 0, warning('fminbnd issue OptK age %d, eflag %d, k_bounds [%.2e, %.2e]',a_new_osa_k,eflag_osa_k, kPMin_opt_osa_k, kPMax_opt_osa_k); end
                    end
                    ValueFunc = -negV_osa_k;
                end
            ...
            % Final checks and re-evaluation of ValueFunc based on chosen c_quantity and kPrime
            kPrime = max(cS.kMin, min(cS.kMax, kPrime)); 
            c_quantity = max(cS.cFloor, (budget_remaining_expenditure_osa_k - kPrime) / (1 + cS.tau_c) );

            [~, u_final_c_osa_k] = main_olg_v3_utils_pps.CES_utility(c_quantity, cS.sigma, cS);
            if a_new_osa_k < cS.aD_new
                ...
                catch %ME_final_interp
                    % warning('OptK final interp error age %d: %s', a_new_osa_k, ME_final_interp.message);
        ...
        % The static helper functions negBellmanObjective_k_pps and BellmanInner_k_pps
        % are kept for potential backward compatibility or alternative VFI structures,
        % but the primary VFI now uses the nested functions within HHSolutionByOneState_OptK.
        % If they were to be used, they would also need modification for cS.tau_c.
        function negV = negBellmanObjective_k_pps(kP, ag_obj, budget_rem_obj, evInt_obj, cS_in_obj, pS_in_obj) % Unused by current V3 VFI path
            % THIS FUNCTION WOULD NEED MODIFICATION FOR cS_in_obj.tau_c if used directly
            [negV, ~] = main_olg_v3_utils_pps.BellmanInner_k_pps(kP, ag_obj, budget_rem_obj, evInt_obj, cS_in_obj, pS_in_obj);
        end

        function [negVal, Val] = BellmanInner_k_pps(kP_inner, ag_inner, budget_rem_inner, evInt_inner, cS_in, pS_in) % Unused by current V3 VFI path
            % THIS FUNCTION WOULD NEED MODIFICATION FOR cS_in.tau_c if used directly
            % cons_quantity = max(cS_in.cFloor, (budget_rem_inner - kP_inner) / (1 + cS_in.tau_c) ); % Example change
            cons = max(cS_in.cFloor, budget_rem_inner - kP_inner); % Original logic
            [util_val, ~] = main_olg_v3_utils_pps.CES_utility(cons, cS_in.sigma, cS_in); 
...
% --- END OF FILE main_olg_v3_utils_pps.m ---