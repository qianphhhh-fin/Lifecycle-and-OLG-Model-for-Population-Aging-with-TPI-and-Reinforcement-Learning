...
cS = utils.ParameterValues_HuggettStyle();
...
popS = utils.initPopulation(cS);
popS = utils.populationDynamics(popS, cS);
[Z_ss, dep_ratio_ss, bgp_reached, bgp_period] = utils.detectSteadyStatePopulation(popS, cS); % Z_ss 是稳态人口数量
...
[paramS.leLogGridV, paramS.leTrProbM, paramS.leProb1V] = utils.EarningProcess_olgm(cS);
...
eIdxM = utils.LaborEndowSimulation_olgm(cS, paramS);
[~, L] = utils.LaborSupply_Huggett(eIdxM, cS, paramS, Z_ss_norm_group); % L的计算使用归一化质量
...
for iter = 1:maxIter
    ...
    [~, R_iter, w_iter, b_iter] = utils.HHPrices_Huggett(KGuess, L, cS, paramS); % <--- 修改调用
...
[Y_eq, R_eq, w_eq, b_eq_final, MPL_eq_final] = utils.HHPrices_Huggett(K_eq, L, cS, paramS); % <--- 修改调用并获取MPL_eq_final
