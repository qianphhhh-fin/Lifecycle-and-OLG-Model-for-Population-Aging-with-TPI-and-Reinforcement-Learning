...
% end
    ...
    % =========================================================================
    % == 函数: backward_hh (v1.1 - 异质性参数传递修正版)
    % == 核心修改:
    % ==   - [!!! 关键BUG修复 !!!] 修正了参数 cS 的传递逻辑，以适配异质性家庭。
    % ==   - 此函数现在接收的 cS 结构体，被假定已经是【针对某一特定类型】
    % ==     配置好的 (例如，cS.ageEffV_new 已经是该类型的效率剖面)。
    % ==   - 删除了内部冗余且错误的 cS_t = cS; 赋值。
    % ==   - 直接将接收到的 cS 传递给 VFI_transition_engine_by_age，
    % ==     并在循环中为每个时期 t 更新时变的生存率 s_pathV。
    % =========================================================================
    T = cS.T_sim;
    Pol_path = cell(1, T);
    ...
    for t = (T-1) : -1 : 1
        ...
        % [!!! 核心修正 !!!] 
        % 直接在传入的 cS 上修改时变参数，而不是创建一个新的 cS_t。
        % cS 本身已经是类型 h 特定的了。
        if size(s_pathV_full_path, 2) > 1
            cS.s_pathV = s_pathV_full_path(:, t);
        end
        
        Vprime_t_plus_1 = Val_path(:,:,:,:,t+1);

        % 将修改了生存率的 cS 传递下去
        [valS_t, polS_t] = household.VFI_transition_engine_by_age(M_vfi_t, pathS.b_hat_path(t), paramSF, cS, Vprime_t_plus_1);
