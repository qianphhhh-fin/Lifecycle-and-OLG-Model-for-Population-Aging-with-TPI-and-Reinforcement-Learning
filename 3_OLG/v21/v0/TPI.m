...
classdef TPI
    ...
    % =========================================================================
    % ==          TPI核心计算引擎 (v9.14 - 遗赠会计核算修正版)
    % ==
    % == 核心修改:
    % ==   - [!!! 关键BUG修复 !!!] 修正了遗赠在代际间转移的会计逻辑。
    % ==   - 旧逻辑: bequest_adj_total_for_t2_to_T 错误地对转轨路径上的
    % ==     遗赠流应用了BGP增长调整 (`./ pop_growth_factors`)。
    % ==   - 新逻辑: 在转轨路径中，t-1期产生的【原始】遗赠总量，就是
    % ==     t期新生儿可用的总量，无需任何调整。
    % ==     - 对于t=1的新生儿, 他们收到的遗赠是ss0期产生的原始总量。
    % ==     - 对于t>1的新生...儿, 他们收到的遗赠是t-1期产生的原始总量。
    % ==   - 此修改确保了SS和TPI之间对遗赠处理的一致性，是解决
    % ==     “完美实验”失败的关键。
    % =========================================================================
    T = cS.T_sim;
    nH = cS.nTypes;
    ...
    % 正确构造每一期新生儿可用的遗赠总量路径
    % ---------------------------------------------------------------------
    % 1. t=1 的新生儿收到的遗赠，是 t=0 (即初始稳态 ss0) 期死亡者产生的【原始】总量。
    bequest_gen_total_for_t1_newborns = ss0.Bequest_gen_hat_raw_ss;

    % 2. 猜测的t=1到t=T-1各期产生的【原始】遗赠总量。
    ...
    for t = 1:T
        ...
        for h = 1:nH
            L_path_supply_h_t = TPI.get_labor_supply_path_for_type(h, {Dist_path_h{h}(:,:,:,:,t)}, cS, paramSF);
            
            total_contrib_t = total_contrib_t + cS.theta_path_h(h,t) * target_paths.w_hat_path(t) * L_path_supply_h_t;
            mass_retirees_path_h_t = sum(cS.Z_path_raw((cS.aR_new+1):end, t)) * cS.type_weights(h);
    ...
    % --- 4. 强制施加终点条件 (此部分可根据收敛策略调整，当前保持开放) ---
    % 在一个收敛良好的系统中，路径末端应自然趋向终期稳态
...
function L_path_h = get_labor_supply_path_for_type(h, Dist_path_h, cS, paramS)
            ...
            dist_path_for_type_h = Dist_path_h{1}; % 总是使用第一个cell元素
