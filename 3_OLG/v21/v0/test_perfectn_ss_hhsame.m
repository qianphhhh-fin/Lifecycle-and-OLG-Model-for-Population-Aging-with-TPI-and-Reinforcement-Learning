...
% a. 设定四类家庭人口占比相同
cSF_test.pop_share_h = repmat(1/nH, [nH, 1]);
fprintf('   [设定] 模型包含 %d 类家庭，每类占比 %.2f%%。\n', nH, (1/nH)*100);

% b. 设定四类家庭的经济参数完全相同
%    (对于大部分参数，由于它们存储在cS根部，已天然相同)
%    我们需要显式复制那些可能因类型而异的参数。
...
Z_theory_h = Z_theory_total * cSF_test.pop_share_h';
...
% if ~isempty(fieldnames(ssF_hhsame))
%     fprintf('\n\n--- 5. 保存转轨分析所需数据 ---\n');
%     if ~exist('SS', 'dir'), mkdir('SS'); end
%     save_filename = 'SS/test_perfectNfor_het_transition.mat';
% 
%     % =========================================================================
%     % ==            [!!! 核心会计修正 !!!]
...
%         pop_h_by_age_t1 = sum(Z_theory_total) * cSF_test.type_weights(h); % [aD x 1]
% 
%         % 2. 从ss0返回的分布 dist0_h 中，计算出【条件分布】
%         %    dist0_h{h} 是基于 Z0_norm(:,h) 计算的，Z0_norm(:,h) 是归一化人口乘以权重
%         %    所以 dist0_h{h}(:,:,:,ia) 的加总之和等于 Z0_norm(ia, h)
%         dist_cond_h = zeros(size(distF_hhsame{h}));
%         for ia = 1:cSF_test.aD_new
%             pop_mass_norm_hia = sum(distF_hhsame{h}(:,:,:,ia), 'all');
%             if pop_mass_norm_hia > 1e-12
%                 dist_cond_h(:,:,:,ia) = distF_hhsame{h}(:,:,:,ia) / pop_mass_norm_hia;
%             end
%         end
% 
...
% 
%     % [!!! 确保转轨参数是完整的路径 !!!]
%     cS_for_trans = cSF_test; 
%     cS_for_trans.theta_path_h = repmat(cSF_test.theta_path_h,1,50);
%     cS_for_trans.s_pathV = repmat(cSF_test.s_pathV,1,50);
%     cS_for_trans.Z_path_raw = repmat(sum(Z_theory_total),1,50);
%     cS_for_trans.type_weights = cSF_test.type_weights;
%     cS_for_trans.ageEffV_new_h = cSF_test.ageEffV_new_h; % 确保效率剖面也传递
% 
%     % [!!! 核心结构修正: 创建一个1x1的struct，其字段为元胞数组 !!!]
%     data_for_transition = struct(); % 初始化一个1x1的struct
%     data_for_transition.ss0 = ssF_hhsame;
%     data_for_transition.ssF = ssF_hhsame;
%     data_for_transition.dist0_h = dist0_abs_h; % 这是一个 cell array, 包含【绝对】人口分布
%     data_for_transition.distF_h = distF_hhsame;     % 这是一个 cell array, 包含【归一化】人口分布
%     data_for_transition.polF_h = polF_hhsame;       % 这是一个 cell array
%     data_for_transition.valF_h = valF_hhsame;       % 这是一个 cell array
%     data_for_transition.cS = cS_for_trans;
%     data_for_transition.paramSF =  paramSF_test;
% 
