classdef main_steady_state_utils_bgp
    ...
    methods (Static)
        ...
        % 文件: main_steady_state_utils_bgp.m
% 文件: main_steady_state_utils_bgp.m
function [ss, Dist, polS, valS] = calculate_aggregates_unified(K_private_total_guess, K_g_guess, L_guess, Bequest_Total_guess, Z_ss_norm, cS, paramS)
    ...
    % ==
    % == 核心修正:
    % ==   - [!!! BUG修复 !!!] 修正了微观预算检验中的双重计算问题。
    % ==     原检验将消费税同时计入了“消费支出”和“常规税收”中，导致
    % ==     总支出被高估。
    % ==   - [正确逻辑] 现在，检验中的总支出被正确定义为：
    % ==     c(净消费) + k'(储蓄) + tax(所有税收) + shock_exp。
    % ==     其中，总税收 `taxes_paid` 已包含消费税，因此不再需要将
    % ==     消费与(1+tau_c)相乘。
    % =========================================================================
    ...
    if isfield(cS, 'endogenous_theta_mode') && cS.endogenous_theta_mode
        ...
        if total_wage_base > 1e-12, theta_theoretical = total_pension_outlay_target / total_wage_base;
    ...
    micro_budget_balance_error_agg = 0;
    g_A_period_ss = (1 + cS.g_A_ss)^cS.time_Step - 1;
    ...
    for ia = 1:cS.aD_new
        ...
        c_ia = polS(ia).c;
        shock_exp_ia = polS(ia).shock_exp;
        tax_regular_ia = polS(ia).tax_regular;
        tax_payg_ia = polS(ia).tax_payg;
        pension_out_ia = polS(ia).pension_out;

        % --- [核心检验逻辑] ---
        k_grid_mat = repmat(cS.kGridV, [1, 1, cS.nw_expanded]);

        % 来源 (Sources of fund)
        capital_return = k_grid_mat * (1 + M_for_hh.r_mkt_t);
        labor_income = zeros(size(k_grid_mat));
        if ia <= cS.aR_new
            labor_income = M_for_hh.w_t * cS.ageEffV_new(ia) * reshape(paramS.leGridV, [1, 1, cS.nw_expanded]);
        end
        pension_received = pension_out_ia; 
        bequest_received = M_for_hh.beq_transfer_pers;
        
        % VFI中的 'cash on hand' (不含税) = k(1+r) + w*eff*e + b + beq
        % 这个定义在聚合与VFI之间是一致的。
        cash_on_hand_replicated = capital_return + labor_income + pension_received + bequest_received;

        % 去向 (Uses of fund)
        % [!!! 修正 !!!] total_uses = c_net + k_prime_adj + all_taxes + shock_exp
        net_consumption = c_ia; % 这是不含税的净消费
        next_period_capital = k_prime_ia * (1 + g_A_period_ss); 
        taxes_paid = tax_regular_ia + tax_payg_ia; % tax_regular_ia 已经包含了消费税
        shock_expenditure = shock_exp_ia;

        % 注意：这里不再是 c_ia * (1+tau_c)，因为消费税已经包含在 taxes_paid 中
        total_uses_replicated = net_consumption + next_period_capital + taxes_paid + shock_expenditure;
        
        budget_error_ia = cash_on_hand_replicated - total_uses_replicated;
        micro_budget_balance_error_agg = micro_budget_balance_error_agg + sum(budget_error_ia .* mass_dist_ia, 'all');
        % --- [检验结束] ---

        % --- [继续进行原有的宏观聚合] ---
        K_private_agg = K_private_agg + sum(cS.kGridV(:) .* squeeze(sum(mass_dist_ia, [2, 3])));
        K_private_hat_agg_vfi = K_private_hat_agg_vfi + sum(k_prime_ia .* mass_dist_ia, 'all');
        
        kpps_prime_dist = 0;
        if cS.pps_active, kpps_prime_dist = polS(ia).kpps_prime;
            K_pps_hat_agg_vfi = K_pps_hat_agg_vfi + sum(kpps_prime_dist .* mass_dist_ia, 'all');
            PPS_contrib_agg = PPS_contrib_agg + sum(polS(ia).pps_contrib .* mass_dist_ia, 'all');
            PPS_withdrawal_agg = PPS_withdrawal_agg + sum(polS(ia).pps_withdrawal .* mass_dist_ia, 'all');
            PPS_tax_agg = PPS_tax_agg + sum(polS(ia).pps_tax .* mass_dist_ia, 'all'); end
        
        Bequest_generated_agg = Bequest_generated_agg + sum((k_prime_ia + kpps_prime_dist) .* mass_dist_ia, 'all') * (1 - cS.s_pathV(ia));
        
        if ia <= cS.aR_new
            mass_by_epsilon = squeeze(sum(mass_dist_ia, [1,2])); 
            L_agg = L_agg + sum( (cS.ageEffV_new(ia) * paramS.leGridV') .* mass_by_epsilon' , 'all'); 
        end
        C_agg = C_agg + sum(c_ia .* mass_dist_ia, 'all'); % C_agg 本身定义为净消费，这是正确的
        Shock_exp_agg = Shock_exp_agg + sum(shock_exp_ia .* mass_dist_ia, 'all');
        Regular_tax_agg = Regular_tax_agg + sum(tax_regular_ia .* mass_dist_ia, 'all');
        Pension_in_agg = Pension_in_agg + sum(tax_payg_ia .* mass_dist_ia, 'all');
        Pension_out_agg = Pension_out_agg + sum(pension_out_ia .* mass_dist_ia, 'all');
    end

    % --- 5. 宏观经济流量核算 --- (此部分不受影响，因为C_agg定义正确)
    ...
    % --- 7. 填充ss结构体 ---
    ss = struct();
    ss.K_private_begin_hat = K_private_total_guess; ss.K_private_implied = K_private_agg;
    ...
    ss.theta = theta_ss;
    ss.micro_budget_balance_error = micro_budget_balance_error_agg;
end
...
%...
        ...
        function [val_age, pol_age] = HHSolutionByAge_VFI_noPPS(a_idx, vPrime_kkppse_next, M_age, b_age_val, beq_transfer_val, paramS_age, cS)
            ...
            % ==
            % == 核心修正:
            % ==   - [!!! 根本性修正 !!!] 统一了所有年龄段（包括最后一个年龄段）
            % ==     的预算约束处理逻辑。现在最后一个年龄段的遗赠 k' 也被
            % ==     乘以了增长因子 (1+g_A_period)。这确保了所有 k' 在经济上
            % ==     具有相同的相对价格，使得宏观聚合有意义，并从根源上解决
            % ==     存量不平衡问题。
            % =========================================================================
            ...
            if a_idx == cS.aD_new % 终期决策
                ...
                if isfield(cS,'phi_bequest') && cS.phi_bequest > 0
                    ...
                    % [!!! 根本性修正 !!!]
                    % 保持与工作年龄段完全一致的逻辑：遗赠k'的“成本”是相对于当期消费的
                    % (1+g_A_period)。这确保了所有k'都可以在宏观层面被直接加总。
                    k_prime_final_vec = total_wealth_vec ./ ((1+g_A_period) + (1+cS.tau_c)*c_over_k_ratio);
                    c_expend_final_vec = total_wealth_vec - k_prime_final_vec * (1 + g_A_period);
                end
                ...
                util_final_vec = util_c + cS.beta * util_bequest; % 注意：遗赠效用函数 u_b(k')，而非u_b(k'*(1+g))
...
% 文件: main_steady_state_utils_bgp.m
        ...
        function Dist = solve_steady_state_distribution_unified(polS, paramS, cS, Z_ss_norm)
            ...
            % ==
            % == 核心修正:
            % ==   - [!!! 根本性修正 !!!] 彻底移除了对各代分布进行强制重新缩放的
            % ==     `final_rescale_factor`。该因子是破坏资本在代际间守恒传递、
            % ==     导致宏观存量不平衡的根本原因。
            % ==   - [财富守恒逻辑] 分布的迭代现在严格遵循物理和经济过程：
            % ==     Dist(a+1) = Operator(Dist(a))。这意味着从 ia 岁人口群体
            % ==     储蓄的财富，会不多不少地、精确地由存活下来的 ia+1 岁
            % ==     人口群体继承。
            % ==   - [自洽性] 这样构建出的分布，其内部的资本流动是完全自洽的，
            % ==     从而保证了宏观聚合的资本存量是正确的。
            % =========================================================================
            nk = cS.nk; 
            nkpps = cS.nkpps; 
            ...
            % 步骤 1: 设置新生儿(a=1)的初始分布
            dist_newborn_shape = zeros(nk, nkpps, nw);
            % 新生儿在 k=0, kpps=0 的位置，根据长期劳动冲击分布 leProb1V 进入模型
            dist_newborn_shape(1, 1, 1:cS.nw) = reshape(paramS.leProb1V, [1, 1, cS.nw]); 
            % 分布的初始总质量由理论人口分布 Z_ss_norm 的第一项决定
            Dist(:, :, :, 1) = dist_newborn_shape * Z_ss_norm(1);

            % 步骤 2: 向前迭代，构建整个年龄的联合分布
            for ia = 1:(aD - 1)
                dist_ia_slice = Dist(:, :, :, ia);
                
                if sum(dist_ia_slice(:)) < 1e-30
                    continue; 
                end

                dist_ia_plus_1_next = zeros(nk, nkpps, nw, 'double');
                trans_mat_next_age = paramS.TrProbM_by_age{ia + 1};

                % --- 核心迭代: 将 ia 岁的每个人分配到 ia+1 岁的相应位置 ---
                % 这个过程必须保证质量(mass)和财富(mass*k)的守恒
                for ik = 1:nk
                    for ikpps = 1:nkpps % 即使 nkpps=1, 循环也只跑一次
                        for ie = 1:nw
                            mass_start = dist_ia_slice(ik, ikpps, ie);
                            if mass_start < 1e-30
                                continue; 
                            end
                            ...
                            % b. 获取他们的最优决策 k' 和 kpps'
                            k_prime = polS(ia).k_prime(ik, ikpps, ie);
                            kpps_prime = polS(ia).kpps_prime(ik, ikpps, ie);
                            
                            % c. 找到新位置的网格点和权重用于线性插值
                            [ik_lower, ik_upper, w_k_upper] = main_steady_state_utils_bgp.find_grid_and_weights(k_prime, cS.kGridV);
                            w_k_lower = 1.0 - w_k_upper;

                            [ikpps_lower, ikpps_upper, w_kpps_upper] = main_steady_state_utils_bgp.find_grid_and_weights(kpps_prime, cS.kppsGridV);
                            w_kpps_lower = 1.0 - w_kpps_upper;

                            % d. 获取下一期的劳动冲击转移概率向量
                            trans_probs_vec = trans_mat_next_age(ie, :);

                            % e. 将生存下来的质量，按(k',kpps')插值权重和(e')转移概率，精确分配到新位置
                            for ie_next = 1:nw
                                prob_to_enext = trans_probs_vec(ie_next);
                                if prob_to_enext < 1e-12, continue; end
                                
                                mass_to_distribute = mass_surviving * prob_to_enext;

                                % 分配到四个角落 (k_lower/upper, kpps_lower/upper)
                                % 这是标准的离散化后保持期望值不变的分配方法
                                if w_k_lower > 1e-9 && w_kpps_lower > 1e-9
                                    dist_ia_plus_1_next(ik_lower, ikpps_lower, ie_next) = dist_ia_plus_1_next(ik_lower, ikpps_lower, ie_next) + mass_to_distribute * w_k_lower * w_kpps_lower;
                                end
                                if w_k_upper > 1e-9 && w_kpps_lower > 1e-9
                                    dist_ia_plus_1_next(ik_upper, ikpps_lower, ie_next) = dist_ia_plus_1_next(ik_upper, ikpps_lower, ie_next) + mass_to_distribute * w_k_upper * w_kpps_lower;
                                end
                                if w_k_lower > 1e-9 && w_kpps_upper > 1e-9
                                    dist_ia_plus_1_next(ik_lower, ikpps_upper, ie_next) = dist_ia_plus_1_next(ik_lower, ikpps_upper, ie_next) + mass_to_distribute * w_k_lower * w_kpps_upper;
                                end
                                if w_k_upper > 1e-9 && w_kpps_upper > 1e-9
                                    dist_ia_plus_1_next(ik_upper, ikpps_upper, ie_next) = dist_ia_plus_1_next(ik_upper, ikpps_upper, ie_next) + mass_to_distribute * w_k_upper * w_kpps_upper;
                                end
                            end
                        end
                    end
                ...
                % [!!! 核心修正 !!!]
                % 严格按照物理和财富守恒定律进行演化，不进行任何人为的重新缩放。
                Dist(:, :, :, ia+1) = dist_ia_plus_1_next;
            end
            
            % 步骤 3: (可选但关键) 对整个最终分布进行一次总归一化
            % 由于 Z_ss_norm 本身加总为1，且我们的演化是守恒的，
            % 最终 Dist 的总质量理论上也应为1。此步可消除数值累积误差。
            total_mass_final = sum(Dist(:));
            if abs(total_mass_final - 1.0) > 1e-9
                if total_mass_final > 1e-12
