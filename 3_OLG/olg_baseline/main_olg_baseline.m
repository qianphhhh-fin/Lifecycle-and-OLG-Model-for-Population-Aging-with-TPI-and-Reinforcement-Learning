% --- START OF FILE main_olg_baseline.m ---

% OLG 模型 Baseline (固定 PAYG 替代率, VFI w k_pps 状态, PPS缴费规则化):
% 目标: 求解给定 PAYG 替代率 (rho_prime_payg_baseline) 下的均衡
% 定义: rho_prime_payg_baseline = b_payg / avg_worker_gross_wage (PAYG福利 / 平均工人税前总工资) - 固定值
...
clear; % 清除工作区变量
close all; % 关闭所有图形窗口
fprintf('=== OLG 模型 Baseline (固定 Rho_prime_payg, VFI w k_pps, PPS缴费规则化) ===\n');
fprintf('    (Rho_prime_payg_baseline 固定, TR_gov=0, tau_l 内生, theta_payg 有上限)\n');
fprintf('    (VFI 状态: k, k_pps, eps; PPS缴费: 基于年龄组的固定比例规则，有法定上限)\n');

%% 1. 固定模型参数与人口设置
fprintf('\n--- 1. 初始化参数 ---\n');
cS = main_olg_baseline_utils.ParameterValues_HuggettStyle(); % 调用baseline的参数设置函数
paramS = struct(); % 初始化一个结构体用于存储派生参数

% *** Baseline Specific Parameter: Fixed PAYG Replacement Rate ***
cS.rho_prime_payg_baseline = 0.20; % 例如: 设定一个固定的PAYG替代率 (20%)
fprintf('>>> Baseline: 固定 PAYG 替代率 (rho_prime_payg_baseline): %.3f\n', cS.rho_prime_payg_baseline);
% ***************************************************************
...
%% 2. 模拟人口动态至稳态
fprintf('\n--- 2. 模拟人口动态 ---\n');
popS = main_olg_baseline_utils.initPopulation(cS); % 初始化人口结构
popS = main_olg_baseline_utils.populationDynamics(popS, cS); % 模拟人口动态演进
[Z_ss, ~, ~, ~] = main_olg_baseline_utils.detectSteadyStatePopulation(popS, cS); % 检测稳态人口分布
...
%% 3. 预计算劳动供给和禀赋过程
fprintf('\n--- 3. 预计算劳动 ---\n');
[paramS.leLogGridV, paramS.leTrProbM, paramS.leProb1V] = main_olg_baseline_utils.EarningProcess_olgm(cS);
paramS.leGridV = exp(paramS.leLogGridV(:));
paramS.ageEffV_new = cS.ageEffV_new;

eIdxM = main_olg_baseline_utils.LaborEndowSimulation_olgm(cS, paramS);

[~, L_per_capita] = main_olg_baseline_utils.LaborSupply_Huggett(eIdxM, cS, paramS, paramS.ageMassV);
...
%% 4. 求解一般均衡 (给定固定的 rho_prime_payg_baseline)
fprintf('\n--- 4. 求解一般均衡 (固定 rho_prime_payg_baseline=%.3f) ---\n', cS.rho_prime_payg_baseline);

K_global_guess = 15.0; % 总资本存量的初始猜测值
paramS_for_solver = paramS; % 复制参数结构体
paramS_for_solver.suppress_inner_print_header = false; % 允许打印内层K-tau_l循环的表头
paramS_for_solver.suppress_initial_theta_print = false; % 允许打印内层循环初始theta_req的检查信息

fprintf('调用均衡求解器 solve_K_tau_l_for_rho_prime with fixed rho_prime_payg_baseline...\n');
solve_start_time = tic;
[K_eq, tau_l_eq, gbc_residual_eq, eq_found, final_eq_solution_details] = ...
    main_olg_baseline_utils.solve_K_tau_l_for_rho_prime(cS.rho_prime_payg_baseline, K_global_guess, cS, paramS_for_solver, eIdxM);
...
if ~eq_found || isnan(K_eq) || isnan(tau_l_eq)
    error('未能为固定的 rho_prime_payg_baseline = %.3f 找到均衡解。请检查模型参数或初始猜测值。', cS.rho_prime_payg_baseline);
end
if abs(gbc_residual_eq) > cS.gbc_tol_for_internal_loop * 10 % 允许略微宽松一点的最终GBC检查
    warning('最终均衡的GBC残差 (%.2e) 较大。可能均衡解的质量不高。', gbc_residual_eq);
end

% 从解的细节中获取PAYG理论税率 (未考虑上限前)
...
%% 5. 分析和绘制最终均衡结果
fprintf('\n--- 5. 最终均衡结果与绘图 (rho_prime_payg_baseline=%.4f, tau_l_eq=%.4f, TR_gov=0) ---\n', cS.rho_prime_payg_baseline, tau_l_eq);
...
if isfield(final_eq_solution_details, 'theta_payg') && isfinite(final_eq_solution_details.theta_payg)
    ...
    [~, temp_MPL_gross_for_theta_calc] = main_olg_baseline_utils.HHPrices_Huggett(K_eq, paramS.L_per_capita, cS);
    temp_mass_retirees = sum(paramS.ageMassV(cS.aR_new+1:cS.aD_new));
    temp_theta_req = cS.rho_prime_payg_baseline * (temp_mass_retirees / paramS.mass_workers_group);
...
[R_mkt_gross_factor_eq_final, MPL_gross_eq_final] = main_olg_baseline_utils.HHPrices_Huggett(K_eq, paramS.L_per_capita, cS);
...
b_payg_eq_final = cS.rho_prime_payg_baseline * avg_worker_gross_wage_eq_final; % 使用固定的替代率
...
fprintf('调用最终的 HHSolution_VFI_Huggett (Baseline)...\n');
[cPolM_eq, kPolM_eq, cPpsPolM_eq_rule, V] = main_olg_baseline_utils.HHSolution_VFI_Huggett(...
    R_k_net_factor_hh_eq_final, MPL_gross_eq_final, TR_total_eq_final_for_vfi, bV_eq_new_final, paramS_eq, cS);

fprintf('模拟最终均衡的分布 (HHSimulation_olgm Baseline)...\n');
[kHistM_eq, kPpsHistM_eq, cHistM_eq] = main_olg_baseline_utils.HHSimulation_olgm(...
    kPolM_eq, cPpsPolM_eq_rule, cPolM_eq, eIdxM, ...
...
fprintf('目标PAYG替代率 (rho_prime_payg_baseline*): %.4f (b_payg / avg_worker_gross_wage)\n', cS.rho_prime_payg_baseline);
...
fprintf('实际达成替代率 (b_payg / avg_worker_gross_wage): %.4f (应接近 rho_prime_payg_baseline*)\n', achieved_replacement_rate_final);
if abs(achieved_replacement_rate_final - cS.rho_prime_payg_baseline) > 1e-3 && cS.rho_prime_payg_baseline > 1e-9
    warning('最终达成的替代率与目标替代率差异较大 (差异: %.3e)。请检查计算一致性。', abs(achieved_replacement_rate_final - cS.rho_prime_payg_baseline));
end

final_gbc_residual = main_olg_baseline_utils.check_gbc_residual(Actual_K_eq_final, C_eq_final, Y_eq_final, ...
...
if cS.nk > 1 && ~isempty(plot_ikpps_indices)
    figure('Name', ['Baseline: 非PPS储蓄策略 k''(k | k_pps): ' figure_title_suffix_base]);
    hold on;
    ...
    xlabel('当前非PPS资产 k');
    ylabel('下一期非PPS资产 k''');
    title({'Baseline: 非PPS储蓄策略 k''(k | k_{pps})'; figure_title_suffix_base});
    legend(legend_entries_k_prime, 'Location', 'best'); grid on;

    if cS.pps_active
        figure('Name', ['Baseline: PPS缴费策略(规则) c_{pps}(k | k_pps): ' figure_title_suffix_base]);
        hold on;
        ...
        title({'Baseline: PPS缴费策略 (基于规则) c_{pps}(k | k_{pps})'; figure_title_suffix_base});
        legend(legend_entries_cpps, 'Location', 'best'); grid on;
    end

    figure('Name', ['Baseline: 消费策略 c(k | k_pps): ' figure_title_suffix_base]);
    hold on;
    ...
    xlabel('当前非PPS资产 k');
    ylabel('消费量 c');
    title({'Baseline: 消费策略 c(k | k_{pps})'; figure_title_suffix_base});
...
elseif cS.nk > 1 && cS.nkpps == 1
    ...
    figure('Name', ['Baseline: 策略函数 (k变化, k_pps固定): ' figure_title_suffix_k_only]);
    ...
    sgtitle(['Baseline: 策略函数切片: ' figure_title_suffix_k_only]);
else
...
fprintf('\n--- Baseline OLG 模型 (固定 Rho_prime_payg) 分析完成 ---\n');