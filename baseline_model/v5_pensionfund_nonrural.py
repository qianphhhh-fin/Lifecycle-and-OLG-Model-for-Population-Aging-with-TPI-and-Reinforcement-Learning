...
class PensionFundEnv(gym.Env):
    ...
    def __init__(self,params=None): # 
        ...
        self.aa = -1.32157880e+01
        self.b1 = 1.34951371e+00
        self.b2 = -4.33632834e-02
        self.b3 = 5.85163503e-04
        self.b4 = -2.81009579e-06
        self.ar = 0.917415
        self.smay = np.sqrt(0.167829) # 暂时性shock的标准差
        self.smav = np.sqrt(0.084805) # 永久性shock的标准差
        self.ret_fac = 0.7  # 退休后固定支付的基本养老金,为退休前工资的68.27%
        ...
        self.pension_limit = 0.1 # 缴费上限 100%的工资
    ...
    0.82935, 0.81282, 0.79485, 0.77543, 0.75458, 0.7324, 0.70893, 0.68424]
        ...
        self.action_space = spaces.Box(low=np.array([0.1, 0, -0.2, -0.2]),\
                                       high=np.array([1.2, 1.2, 0.3, 1.2]), shape=(4,), dtype=float)
        
        # ===================== 状态空间：=====================
        # 现金余额，永久收入冲击, 个人养老金余额，年龄
        self.observation_space = spaces.Box(
            low=np.array([0, -1e20, 0, -0.5]),
            high=np.array([1e20,  1e20, 1e20, 0.5]),
            shape=(4,),
            dtype=float
        )
...
# === 状态转移方程（退休前） ===       
        if self.age < self.tr:
            ...
            new_wage = max(np.exp(log_rand_wage),0.6) # 最低年收入 2000元
            ...
            pen_amount = self.wage * tda_pct # 个人养老金购买额(实际)
            opt_pen_amount = self.wage * actual_tda_pct # 个人养老金购买额(最优)
            ...
            tax_refund = tax - self.taxation(self.wage - pen_amount) # 购买养老金的节税额
            # if tax>0:
            #     raise
            cash = cash + tax_refund - pen_amount #  tax_refund - pen_amount 对应 Gomes et al. (2009)的 -k_tY_t(1-\tau)
            # if tax_refund>tax:
            #     raise
...
if __name__=="__main__":
    ...
    env = gym.make('pensionfund-v5-nonrural', params=params)
